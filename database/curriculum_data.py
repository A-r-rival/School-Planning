# Auto-generated curriculum data

# ==========================================
# COMMON POOLS
# ==========================================
COMMON_USD_POOL = [
    ("ELC101", "Fotoğrafçılık", 3),
    ("ELC102", "Sinema Tarihi", 3),
    ("ELC103", "İletişim Becerileri", 3),
    ("ELC104", "Yaratıcı Yazarlık", 3),
    ("ELC105", "Bilim Tarihi", 3),
    ("ELC106", "Müzik Kültürü", 3),
    ("ELC107", "İşaret Dili", 3),
    ("ELC108", "Gönüllülük Çalışmaları", 3),
    ("ELC109", "Girişimcilik", 3),
    ("ELC110", "Kariyer Planlama", 3)
]

DEPARTMENTS_DATA = {
    "Bilgisayar Müh": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "MAT103",
            "Analiz I",
            6,
            3,
            2,
            0
        ],
        [
            "INF101",
            "Bilgisayar Bilimine ve Programlamaya Giriş",
            6,
            2,
            0,
            2
        ],
        [
            "INF103",
            "Mantık",
            6,
            2,
            2,
            0
        ],
        [
            "INF107",
            "Bilgisayar Organizasyonu",
            6,
            2,
            0,
            2
        ],
        [
            "DEU121",
            "Teknik Almanca I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG101",
            "İngilizce I",
            2,
            3,
            0,
            0
        ],
        [
            "TUR001",
            "Türkçe I",
            2,
            2,
            0,
            0
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "MAT106",
            "Lineer Cebir",
            6,
            2,
            2,
            1
        ],
        [
            "INF102",
            "Nesnel Programlama",
            6,
            2,
            0,
            2
        ],
        [
            "INF104",
            "Özdevinirler ve Biçimsel Diller",
            6,
            2,
            2,
            0
        ],
        [
            "INF110",
            "İşletim Sistemleri",
            6,
            2,
            0,
            2
        ],
        [
            "DEU122",
            "Teknik Almanca II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG102",
            "İngilizce II",
            2,
            3,
            0,
            0
        ],
        [
            "TUR002",
            "Türkçe II",
            2,
            2,
            0,
            0
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "INF201",
            "Ayrık Yapılar",
            6,
            2,
            2,
            1
        ],
        [
            "INF203",
            "Algoritmalar ve Veri Yapıları I",
            6,
            2,
            0,
            2
        ],
        [
            "INF205",
            "Veritabanı Sistemleri",
            6,
            2,
            0,
            2
        ],
        [
            "INF209",
            "Bilgisayar Ağları",
            6,
            2,
            2,
            0
        ],
        [
            "INF211",
            "Bilgisayar ve Toplum Semineri",
            2,
            1,
            0,
            0
        ],
        [
            "ENG201",
            "İngilizce III",
            2,
            3,
            0,
            0
        ],
        [
            "AIT001",
            "Atatürk İlkeleri ve İnkılap Tarihi I",
            2,
            2,
            0,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "MAT204",
            "Veri Analizinin İstatistiksel Yöntemleri",
            6,
            2,
            2,
            1
        ],
        [
            "INF202",
            "Yazılım Mühendisliği",
            6,
            1,
            0,
            3
        ],
        [
            "INF204",
            "Algoritmalar ve Veri Yapıları II",
            6,
            2,
            0,
            2
        ],
        [
            "INF208",
            "Gömülü Sistemler",
            6,
            2,
            0,
            2
        ],
        [
            "INF210",
            "Bilgisayar Mühendisleri için Etik Semineri",
            2,
            1,
            0,
            0
        ],
        [
            "ENG202",
            "İngilizce IV",
            2,
            3,
            0,
            0
        ],
        [
            "AIT002",
            "Atatürk İlkeleri ve İnkılap Tarihi II",
            2,
            2,
            0,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "SDP",
            "Seçmeli Alan - Proje",
            6,
            0,
            0,
            0
        ],
        [
            "SDIa",
            "Seçmeli I - Uygulamalı Bilgisayar Mühendisliği",
            6,
            0,
            0,
            0
        ],
        [
            "SDIb",
            "Seçmeli I - Bilgisayar Donanımı",
            6,
            0,
            0,
            0
        ],
        [
            "SDIc",
            "Seçmeli I - Bilg. Müh. Kuramsal Temelleri ve Matematik",
            6,
            0,
            0,
            0
        ],
        [
            "USD001",
            "Üniversite Seçmeli Ders Havuzu I",
            2,
            0,
            0,
            0
        ],
        [
            "ISG001",
            "İş Sağlığı ve Güvenliği I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG301",
            "İleri İngilizce I",
            2,
            3,
            0,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "SDIIa",
            "Seçmeli II - Uygulamalı Bilgisayar Müh.",
            12,
            0,
            0,
            0
        ],
        [
            "SDIIb",
            "Seçmeli II - Bilgisayar Donanımı",
            6,
            0,
            0,
            0
        ],
        [
            "SDIIc",
            "Seçmeli II - Bilg. Müh. Kuramsal Temelleri ve Matematik",
            6,
            0,
            0,
            0
        ],
        [
            "USD002",
            "Üniversite Seçmeli Ders Havuzu II",
            2,
            0,
            0,
            0
        ],
        [
            "ISG002",
            "İş Sağlığı ve Güvenliği II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG302",
            "İleri İngilizce II",
            2,
            3,
            0,
            0
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "INF499",
            "Mesleki Alan Stajı",
            6,
            0,
            0,
            0
        ],
        [
            "INF401",
            "Bilimsel Çalışma Yöntemleri",
            6,
            2,
            0,
            0
        ],
        [
            "SDIII",
            "Seçmeli Dersler III",
            18,
            0,
            0,
            0
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "INF492",
            "Bitirme Tezi",
            12,
            0,
            0,
            0
        ],
        [
            "SDIV",
            "Seçmeli Dersler IV",
            18,
            0,
            0,
            0
        ]
    ]
},
        "pool_codes": {
    "SDIII, SDIV": [],
    "SDIIa": [],
    "SDIIb": [],
    "SDIIc": [],
    "SDIId": [],
    "SDIIe": [],
    "SDIa": [],
    "SDIa, SDIIa": [
        "Akıllı Sistemler",
        "Makine Öğrenmesi",
        "Yapay Sinir Ağları",
        "Doğal Dil İşleme",
        "Veri Madenciliği",
        "Veri Analizi Yöntemleri",
        "Bilgi Getirimi ve Çıkarımı",
        "Önerici Sistemler",
        "Derin Jeneratif Modeller",
        "Bilişim Sistemleri Güvenliği",
        "Dağıtık Sistemler",
        "Yazılım Bağlamında Validasyon ve Verifikasyon",
        "Derin Öğrenme",
        "Bilgisayar Grafiği I",
        "Bilgisayar Grafiği II",
        "Pekiştirmeli Öğrenme",
        "Tıbbi Görüntü İşleme",
        "Bilgisayar Görmesinin Temelleri",
        "Oyun Tasarımı",
        "Oyun Motoru Mimarisi",
        "Bilgi Görselleme",
        "Web Mühendisliği",
        "İnsan-Makine İletişimi",
        "Uygulamalı Bilg. Müh: Seçilmiş Konular I",
        "Uyg. Bilg. Müh. Seç. Kon. II - Multimedya İçerik Arama",
        "Uygulamalı Bilg. Müh: Seçilmiş Konular III",
        "Uygulamalı Bilg. Müh: Seçilmiş Konular IV",
        "Uygulamalı Bilg. Müh: Seçilmiş Konular V",
        "Uygulamalı Bilg. Müh: Seçilmiş Konular VI",
        "Yazılım Projesi I",
        "Yazılım Projesi II",
        "Yazılım Projesi III",
        "Tıp Alanında Yapay Zeka",
        "Biyoinfomartiğe Giriş"
    ],
    "SDIb": [],
    "SDIb, SDIIb": [
        "Gerçek Zamanlı Sistemler",
        "Derleyici Tasarımı",
        "Mobil Komünikasyon",
        "GPU Programlama",
        "Görüntü İşlemenin Temelleri",
        "Donanımsal Bilg. Müh: Seçilmiş Konular I",
        "Donanımsal Bilg. Müh: Seçilmiş Konular II",
        "Donanımsal Bilg. Müh: Seçilmiş Konular III",
        "Donanımsal Bilg. Müh: Seçilmiş Konular IV",
        "Donanımsal Bilg. Müh: Seçilmiş Konular V",
        "Donanımsal Bilg. Müh: Seçilmiş Konular VI",
        "Donanım Projesi I",
        "Donanım Projesi II",
        "Donanım Projesi III",
        "Sayısal Tasarım",
        "Elektrik ve Manyetizma",
        "Elektrik Devreleri I",
        "Elektrik Devreleri II",
        "Sinyaller ve Sistemler",
        "Elektronik I: Yarı İletken Elemanlar",
        "Telekomünikasyon",
        "Gömülü Sistemler",
        "VLSI Tasarımına Giriş",
        "Haberleşme Ağları",
        "Dijital Görüntü İşleme",
        "Dijital Sinyal İşleme",
        "Teknik Çizim ve Bilgisayar Destekli Tasarım",
        "Malzeme Teknolojisi I",
        "Malzeme Teknolojisi II",
        "Tasarım Tek. I: Teknik Çizim ve Bilg. Destekli Tasarım",
        "Ölçüm Teknikleri",
        "Endüstriyel Otomasyon Teknolojisi",
        "Endüstriyel Robotik I",
        "Görüntü Tabanlı Otomasyon I",
        "Görüntü Tabanlı Otomasyon II"
    ],
    "SDIc": [],
    "SDIc, SDIIc": [
        "Yapay Zeka",
        "Bilgi Gösterimi ve Akıl Yürütme",
        "Kodlama Teorisi ve Kriptoloji",
        "Algoritmik Geometrinin Temelleri",
        "Algoritma Mühendisliği",
        "Bilg. Müh. Teorik Temelleri: Seçilmiş Konular I",
        "Bilg. Müh. Teorik Temelleri: II",
        "Bilg. Müh. Teorik Temelleri: Seçilmiş Konular III",
        "Bilg. Müh. Teorik Temelleri: Seçilmiş Konular IV",
        "Bilg. Müh. Teorik Temelleri: Seçilmiş Konular V",
        "Bilg. Müh. Teorik Temelleri: Seçilmiş Konular VI",
        "Bilgisayar Destekli İstatistik",
        "Diferansiyel Denklemler ve Nümerik",
        "Bilgisayar Mühendisleri için İleri Matematik",
        "Algoritmalar ve Karmaşıklık Teorisi",
        "Programlama Paradigmaları",
        "Analiz II",
        "Diferansiyel Denklemler",
        "Numerik Matematik",
        "Yöneylem Araştırması I - Doğrusal Modeller",
        "Yöneylem Araştırması II - Rassal Modeller"
    ],
    "SDId": [],
    "SDId, SDIId": [
        "İşletme Enformatiği: Seçilmiş Konular I",
        "İşletme Enformatiği: Seçilmiş Konular II",
        "İşletme Enformatiği: Seçilmiş Konular III",
        "İşletme Enformatiği: Seçilmiş Konular IV",
        "İşletme Enformatiği: Seçilmiş Konular V",
        "İşletme Enformatiği: Seçilmiş Konular VI",
        "Dijital Pazarlama",
        "Karar Teorisi",
        "Örgütsel Davranış",
        "Personel Yönetimi",
        "İşletmesel Veri Analizi",
        "İşletmeye Giriş",
        "Pazarlamanın Temelleri",
        "İşletme Enformatiğinin Temelleri",
        "Operasyon Yönetimi",
        "E-İş",
        "Yöneylem Araştırması",
        "İktisada Giriş",
        "Oyun Teorisi",
        "Uluslararası İktisat 1",
        "Muhasebe ve Bilanço",
        "Üretim ve Lojistik için Enformasyon Sistemleri",
        "Pazarlama",
        "Lojistik Yönetimi ve Teknolojileri",
        "Kalite Yönetimi",
        "Dijital Dönüşüm Yönetimi"
    ],
    "SDIe": [],
    "SDIe, SDIIe": [
        "Mühendisler İçin Hukuk",
        "Girişimcilik",
        "Sosyal Beceriler I",
        "Sosyal Beceriler II",
        "Sosyal Beceriler III",
        "Sosyal Beceriler IV",
        "Sosyal Beceriler V",
        "Sosyal Beceriler VI",
        "Sosyal Beceriler VII",
        "Sosyal Beceriler VIII",
        "Sosyal Beceriler IX",
        "Sosyal Beceriler X",
        "Bilgisayar Mühendisliği Semineri I",
        "Bilgisayar Mühendisliği Semineri II",
        "Mekaniğin Temelleri",
        "Modern Fizik",
        "Biyoloji",
        "Kimya I",
        "Kimya II",
        "Doğa Bilimlerine Giriş",
        "Fizik I",
        "Fizik II",
        "Biyokimya I",
        "Fiziksel Kimya I",
        "Mikrobiyoloji I",
        "Moleküler Biyoteknoloji I"
    ],
    "SDP": [
        "Proje I: Yenilik ve Teknoloji Yönetimi Projesi",
        "Mekatronik Projesi",
        "Yazılım Mühendisliği Projesi",
        "Elektrik ve Elektronik Mühendisliği Projesi"
    ]
},
        "pools": {
    "SDP": [
        [
            "WIN311",
            "Proje I: Yenilik ve Teknoloji Yönetimi Projesi",
            6,
            1,
            0,
            4
        ],
        [
            "MEC319",
            "Mekatronik Projesi",
            6,
            1,
            0,
            4
        ],
        [
            "INF303",
            "Yazılım Mühendisliği Projesi",
            6,
            1,
            0,
            3
        ],
        [
            "ETE491",
            "Elektrik ve Elektronik Mühendisliği Projesi",
            6,
            1,
            0,
            4
        ]
    ],
    "SDIII, SDIV": [],
    "SDIa": [],
    "SDIIa": [],
    "SDIb": [],
    "SDIIb": [],
    "SDIc": [],
    "SDIIc": [],
    "SDId": [],
    "SDIId": [],
    "SDIe": [],
    "SDIIe": [],
    "SDIa, SDIIa": [
        [
            "INF501",
            "Akıllı Sistemler",
            6,
            2,
            0,
            2
        ],
        [
            "INF502",
            "Makine Öğrenmesi",
            6,
            2,
            0,
            2
        ],
        [
            "INF503",
            "Yapay Sinir Ağları",
            6,
            2,
            0,
            2
        ],
        [
            "INF504",
            "Doğal Dil İşleme",
            6,
            2,
            0,
            2
        ],
        [
            "INF505",
            "Veri Madenciliği",
            6,
            2,
            0,
            2
        ],
        [
            "INF506",
            "Veri Analizi Yöntemleri",
            6,
            2,
            0,
            2
        ],
        [
            "INF507",
            "Bilgi Getirimi ve Çıkarımı",
            6,
            2,
            0,
            2
        ],
        [
            "INF508",
            "Önerici Sistemler",
            6,
            2,
            0,
            2
        ],
        [
            "INF509",
            "Derin Jeneratif Modeller",
            6,
            2,
            0,
            2
        ],
        [
            "INF510",
            "Bilişim Sistemleri Güvenliği",
            6,
            2,
            0,
            2
        ],
        [
            "INF511",
            "Dağıtık Sistemler",
            6,
            2,
            0,
            2
        ],
        [
            "INF512",
            "Yazılım Bağlamında Validasyon ve Verifikasyon",
            6,
            2,
            0,
            2
        ],
        [
            "INF513",
            "Derin Öğrenme",
            6,
            2,
            0,
            2
        ],
        [
            "INF514",
            "Bilgisayar Grafiği I",
            6,
            2,
            0,
            2
        ],
        [
            "INF515",
            "Bilgisayar Grafiği II",
            6,
            2,
            0,
            2
        ],
        [
            "INF516",
            "Pekiştirmeli Öğrenme",
            6,
            2,
            0,
            2
        ],
        [
            "INF517",
            "Tıbbi Görüntü İşleme",
            6,
            2,
            0,
            2
        ],
        [
            "INF518",
            "Bilgisayar Görmesinin Temelleri",
            6,
            2,
            0,
            2
        ],
        [
            "INF519",
            "Oyun Tasarımı",
            6,
            2,
            0,
            2
        ],
        [
            "INF520",
            "Oyun Motoru Mimarisi",
            6,
            2,
            0,
            2
        ],
        [
            "INF521",
            "Bilgi Görselleme",
            6,
            2,
            0,
            2
        ],
        [
            "INF522",
            "Web Mühendisliği",
            6,
            2,
            0,
            2
        ],
        [
            "INF523",
            "İnsan-Makine İletişimi",
            6,
            2,
            0,
            2
        ],
        [
            "INF524",
            "Uygulamalı Bilg. Müh: Seçilmiş Konular I",
            6,
            2,
            0,
            2
        ],
        [
            "INF525",
            "Uyg. Bilg. Müh. Seç. Kon. II - Multimedya İçerik Arama",
            6,
            2,
            0,
            2
        ],
        [
            "INF526",
            "Uygulamalı Bilg. Müh: Seçilmiş Konular III",
            6,
            2,
            2,
            0
        ],
        [
            "INF527",
            "Uygulamalı Bilg. Müh: Seçilmiş Konular IV",
            6,
            2,
            2,
            0
        ],
        [
            "INF528",
            "Uygulamalı Bilg. Müh: Seçilmiş Konular V",
            4,
            1,
            0,
            2
        ],
        [
            "INF529",
            "Uygulamalı Bilg. Müh: Seçilmiş Konular VI",
            4,
            1,
            0,
            2
        ],
        [
            "INF530",
            "Yazılım Projesi I",
            6,
            0,
            0,
            4
        ],
        [
            "INF531",
            "Yazılım Projesi II",
            6,
            0,
            0,
            4
        ],
        [
            "INF532",
            "Yazılım Projesi III",
            6,
            0,
            0,
            4
        ],
        [
            "INF533",
            "Tıp Alanında Yapay Zeka",
            6,
            2,
            0,
            2
        ],
        [
            "INF534",
            "Biyoinfomartiğe Giriş",
            6,
            2,
            0,
            2
        ]
    ],
    "SDIb, SDIIb": [
        [
            "INF601",
            "Gerçek Zamanlı Sistemler",
            6,
            2,
            0,
            2
        ],
        [
            "INF602",
            "Derleyici Tasarımı",
            6,
            2,
            0,
            2
        ],
        [
            "INF603",
            "Mobil Komünikasyon",
            6,
            2,
            0,
            2
        ],
        [
            "INF604",
            "GPU Programlama",
            6,
            2,
            0,
            2
        ],
        [
            "INF605",
            "Görüntü İşlemenin Temelleri",
            6,
            2,
            0,
            2
        ],
        [
            "INF606",
            "Donanımsal Bilg. Müh: Seçilmiş Konular I",
            6,
            2,
            0,
            2
        ],
        [
            "INF607",
            "Donanımsal Bilg. Müh: Seçilmiş Konular II",
            6,
            2,
            0,
            2
        ],
        [
            "INF608",
            "Donanımsal Bilg. Müh: Seçilmiş Konular III",
            6,
            2,
            2,
            0
        ],
        [
            "INF609",
            "Donanımsal Bilg. Müh: Seçilmiş Konular IV",
            6,
            2,
            2,
            0
        ],
        [
            "INF610",
            "Donanımsal Bilg. Müh: Seçilmiş Konular V",
            4,
            1,
            0,
            2
        ],
        [
            "INF611",
            "Donanımsal Bilg. Müh: Seçilmiş Konular VI",
            4,
            1,
            2,
            0
        ],
        [
            "INF612",
            "Donanım Projesi I",
            6,
            0,
            0,
            4
        ],
        [
            "INF613",
            "Donanım Projesi II",
            6,
            0,
            0,
            4
        ],
        [
            "INF614",
            "Donanım Projesi III",
            6,
            0,
            0,
            4
        ],
        [
            "ETE101",
            "Sayısal Tasarım",
            6,
            2,
            1,
            1
        ],
        [
            "PHY102",
            "Elektrik ve Manyetizma",
            6,
            3,
            1,
            1
        ],
        [
            "ETE201",
            "Elektrik Devreleri I",
            6,
            3,
            2,
            0
        ],
        [
            "ETE202",
            "Elektrik Devreleri II",
            6,
            3,
            2,
            0
        ],
        [
            "ETE303",
            "Sinyaller ve Sistemler",
            6,
            3,
            1,
            1
        ],
        [
            "ETE311",
            "Elektronik I: Yarı İletken Elemanlar",
            6,
            2,
            2,
            1
        ],
        [
            "ETE372",
            "Telekomünikasyon",
            6,
            3,
            2,
            0
        ],
        [
            "ETE442",
            "Gömülü Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE448",
            "VLSI Tasarımına Giriş",
            6,
            3,
            1,
            1
        ],
        [
            "ETE471",
            "Haberleşme Ağları",
            6,
            3,
            1,
            1
        ],
        [
            "ETE474",
            "Dijital Görüntü İşleme",
            6,
            2,
            1,
            2
        ],
        [
            "ETE475",
            "Dijital Sinyal İşleme",
            6,
            2,
            1,
            2
        ],
        [
            "MAB107",
            "Teknik Çizim ve Bilgisayar Destekli Tasarım",
            6,
            2,
            0,
            4
        ],
        [
            "MAB207",
            "Malzeme Teknolojisi I",
            6,
            3,
            2,
            0
        ],
        [
            "MAB310",
            "Malzeme Teknolojisi II",
            6,
            3,
            1,
            0
        ],
        [
            "MEC107",
            "Tasarım Tek. I: Teknik Çizim ve Bilg. Destekli Tasarım",
            6,
            1,
            2,
            1
        ],
        [
            "MEC208",
            "Ölçüm Teknikleri",
            6,
            2,
            1,
            2
        ],
        [
            "MEC313",
            "Endüstriyel Otomasyon Teknolojisi",
            6,
            3,
            1,
            1
        ],
        [
            "MEC308",
            "Endüstriyel Robotik I",
            6,
            3,
            1,
            1
        ],
        [
            "MEC321",
            "Görüntü Tabanlı Otomasyon I",
            6,
            3,
            1,
            1
        ],
        [
            "MEC324",
            "Görüntü Tabanlı Otomasyon II",
            6,
            3,
            1,
            1
        ]
    ],
    "SDIc, SDIIc": [
        [
            "INF701",
            "Yapay Zeka",
            6,
            2,
            0,
            2
        ],
        [
            "INF702",
            "Bilgi Gösterimi ve Akıl Yürütme",
            6,
            2,
            2,
            0
        ],
        [
            "INF703",
            "Kodlama Teorisi ve Kriptoloji",
            6,
            2,
            2,
            0
        ],
        [
            "INF704",
            "Algoritmik Geometrinin Temelleri",
            6,
            3,
            1,
            0
        ],
        [
            "INF705",
            "Algoritma Mühendisliği",
            6,
            2,
            2,
            0
        ],
        [
            "INF706",
            "Bilg. Müh. Teorik Temelleri: Seçilmiş Konular I",
            6,
            2,
            0,
            2
        ],
        [
            "INF707",
            "Bilg. Müh. Teorik Temelleri: II",
            6,
            2,
            0,
            2
        ],
        [
            "INF708",
            "Bilg. Müh. Teorik Temelleri: Seçilmiş Konular III",
            6,
            2,
            2,
            0
        ],
        [
            "INF709",
            "Bilg. Müh. Teorik Temelleri: Seçilmiş Konular IV",
            6,
            2,
            2,
            0
        ],
        [
            "INF710",
            "Bilg. Müh. Teorik Temelleri: Seçilmiş Konular V",
            4,
            1,
            0,
            2
        ],
        [
            "INF711",
            "Bilg. Müh. Teorik Temelleri: Seçilmiş Konular VI",
            4,
            1,
            2,
            0
        ],
        [
            "INF712",
            "Bilgisayar Destekli İstatistik",
            6,
            2,
            0,
            2
        ],
        [
            "INF713",
            "Diferansiyel Denklemler ve Nümerik",
            6,
            2,
            1,
            1
        ],
        [
            "INF714",
            "Bilgisayar Mühendisleri için İleri Matematik",
            6,
            2,
            2,
            0
        ],
        [
            "INF715",
            "Algoritmalar ve Karmaşıklık Teorisi",
            6,
            2,
            2,
            0
        ],
        [
            "INF716",
            "Programlama Paradigmaları",
            6,
            2,
            0,
            2
        ],
        [
            "MAT108",
            "Analiz II",
            6,
            3,
            2,
            0
        ],
        [
            "MAT201",
            "Diferansiyel Denklemler",
            6,
            2,
            2,
            1
        ],
        [
            "MAT302",
            "Numerik Matematik",
            6,
            3,
            1,
            1
        ],
        [
            "WIN209",
            "Yöneylem Araştırması I - Doğrusal Modeller",
            6,
            2,
            2,
            0
        ],
        [
            "WIN316",
            "Yöneylem Araştırması II - Rassal Modeller",
            6,
            2,
            2,
            0
        ]
    ],
    "SDId, SDIId": [
        [
            "INF801",
            "İşletme Enformatiği: Seçilmiş Konular I",
            6,
            2,
            0,
            2
        ],
        [
            "INF802",
            "İşletme Enformatiği: Seçilmiş Konular II",
            6,
            2,
            0,
            2
        ],
        [
            "INF803",
            "İşletme Enformatiği: Seçilmiş Konular III",
            6,
            2,
            2,
            0
        ],
        [
            "INF804",
            "İşletme Enformatiği: Seçilmiş Konular IV",
            6,
            2,
            2,
            0
        ],
        [
            "INF805",
            "İşletme Enformatiği: Seçilmiş Konular V",
            4,
            1,
            0,
            2
        ],
        [
            "INF806",
            "İşletme Enformatiği: Seçilmiş Konular VI",
            4,
            1,
            2,
            0
        ],
        [
            "BWL007",
            "Dijital Pazarlama",
            6,
            2,
            2,
            0
        ],
        [
            "BWL017",
            "Karar Teorisi",
            6,
            2,
            2,
            0
        ],
        [
            "BWL030",
            "Örgütsel Davranış",
            6,
            2,
            2,
            0
        ],
        [
            "BWL033",
            "Personel Yönetimi",
            6,
            2,
            2,
            0
        ],
        [
            "BWL037",
            "İşletmesel Veri Analizi",
            6,
            2,
            2,
            0
        ],
        [
            "BWL101",
            "İşletmeye Giriş",
            6,
            2,
            2,
            0
        ],
        [
            "BWL201",
            "Pazarlamanın Temelleri",
            6,
            2,
            2,
            0
        ],
        [
            "BWL211",
            "İşletme Enformatiğinin Temelleri",
            3,
            2,
            0,
            0
        ],
        [
            "BWL214",
            "Operasyon Yönetimi",
            5,
            2,
            2,
            0
        ],
        [
            "BWL216",
            "E-İş",
            3,
            2,
            0,
            0
        ],
        [
            "BWL415",
            "Yöneylem Araştırması",
            6,
            2,
            2,
            0
        ],
        [
            "VWL101",
            "İktisada Giriş",
            6,
            2,
            2,
            0
        ],
        [
            "VWL204",
            "Oyun Teorisi",
            5,
            2,
            2,
            0
        ],
        [
            "VWL301",
            "Uluslararası İktisat 1",
            6,
            3,
            1,
            0
        ],
        [
            "WIN204",
            "Muhasebe ve Bilanço",
            6,
            2,
            2,
            1
        ],
        [
            "WIN306",
            "Üretim ve Lojistik için Enformasyon Sistemleri",
            6,
            1,
            0,
            2
        ],
        [
            "WIN309",
            "Pazarlama",
            6,
            2,
            2,
            0
        ],
        [
            "WIN313",
            "Lojistik Yönetimi ve Teknolojileri",
            6,
            2,
            1,
            1
        ],
        [
            "WIN314",
            "Kalite Yönetimi",
            6,
            3,
            1,
            1
        ],
        [
            "WIN351",
            "Dijital Dönüşüm Yönetimi",
            6,
            2,
            2,
            0
        ]
    ],
    "SDIe, SDIIe": [
        [
            "ING406",
            "Mühendisler İçin Hukuk",
            6,
            3,
            1,
            0
        ],
        [
            "ING404",
            "Girişimcilik",
            2,
            2,
            0,
            0
        ],
        [
            "INF901",
            "Sosyal Beceriler I",
            2,
            1,
            0,
            0
        ],
        [
            "INF902",
            "Sosyal Beceriler II",
            3,
            2,
            0,
            0
        ],
        [
            "INF903",
            "Sosyal Beceriler III",
            4,
            2,
            0,
            0
        ],
        [
            "INF904",
            "Sosyal Beceriler IV",
            5,
            2,
            1,
            0
        ],
        [
            "INF905",
            "Sosyal Beceriler V",
            6,
            2,
            2,
            0
        ],
        [
            "INF906",
            "Sosyal Beceriler VI",
            2,
            1,
            0,
            0
        ],
        [
            "INF907",
            "Sosyal Beceriler VII",
            3,
            2,
            0,
            0
        ],
        [
            "INF908",
            "Sosyal Beceriler VIII",
            4,
            2,
            0,
            0
        ],
        [
            "INF909",
            "Sosyal Beceriler IX",
            5,
            2,
            1,
            0
        ],
        [
            "INF910",
            "Sosyal Beceriler X",
            6,
            2,
            2,
            0
        ],
        [
            "INF911",
            "Bilgisayar Mühendisliği Semineri I",
            4,
            2,
            0,
            0
        ],
        [
            "INF912",
            "Bilgisayar Mühendisliği Semineri II",
            4,
            2,
            0,
            0
        ],
        [
            "PHY101",
            "Mekaniğin Temelleri",
            6,
            3,
            1,
            1
        ],
        [
            "PHY103",
            "Modern Fizik",
            6,
            3,
            1,
            1
        ],
        [
            "BIO111",
            "Biyoloji",
            6,
            2,
            1,
            2
        ],
        [
            "CHE111",
            "Kimya I",
            6,
            2,
            1,
            2
        ],
        [
            "CHE112",
            "Kimya II",
            6,
            2,
            1,
            2
        ],
        [
            "NWI107",
            "Doğa Bilimlerine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "PHY111",
            "Fizik I",
            6,
            2,
            1,
            2
        ],
        [
            "PHY112",
            "Fizik II",
            6,
            2,
            1,
            2
        ],
        [
            "MBT211",
            "Biyokimya I",
            6,
            2,
            1,
            2
        ],
        [
            "NWI201",
            "Fiziksel Kimya I",
            6,
            3,
            1,
            1
        ],
        [
            "MBT204",
            "Mikrobiyoloji I",
            6,
            2,
            1,
            2
        ],
        [
            "MBT222",
            "Moleküler Biyoteknoloji I",
            6,
            2,
            1,
            2
        ]
    ]
},
        "pool_metadata": {
    "SDP": {},
    "SDIII, SDIV": {},
    "SDIa": {},
    "SDIIa": {},
    "SDIb": {},
    "SDIIb": {},
    "SDIc": {},
    "SDIIc": {},
    "SDId": {},
    "SDIId": {},
    "SDIe": {},
    "SDIIe": {},
    "SDIa, SDIIa": {},
    "SDIb, SDIIb": {},
    "SDIc, SDIIc": {},
    "SDId, SDIId": {},
    "SDIe, SDIIe": {}
},
    },
    "Elektrik-Elektronik Müh": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "MAT103",
            "Analiz I",
            6,
            3,
            2,
            0
        ],
        [
            "PHY101",
            "Mekaniğin Temelleri",
            6,
            3,
            1,
            1
        ],
        [
            "ETE101",
            "Sayısal Tasarım",
            6,
            2,
            1,
            1
        ],
        [
            "ETE103",
            "Bilg. Bil. ve Prog. Giriş",
            6,
            2,
            0,
            2
        ],
        [
            "ETE091",
            "Elektrik-Elektronik Müh. Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "DEU121",
            "Teknik Almanca I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG101",
            "İngilizce I",
            2,
            3,
            0,
            0
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "MAT108",
            "Analiz II",
            6,
            3,
            2,
            0
        ],
        [
            "PHY102",
            "Elektrik ve Manyetizma",
            6,
            3,
            1,
            1
        ],
        [
            "MAT106",
            "Lineer Cebir",
            6,
            2,
            2,
            1
        ],
        [
            "SDII",
            "Seçmeli Ders Alanı II",
            6,
            0,
            0,
            0
        ],
        [
            "ETE092",
            "Bilimsel Çalışma Yöntemleri",
            2,
            2,
            0,
            0
        ],
        [
            "DEU122",
            "Teknik Almanca II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG102",
            "İngilizce II",
            2,
            3,
            0,
            0
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "MAT201",
            "Diferansiyel Denklemler",
            6,
            2,
            1,
            1
        ],
        [
            "ETE201",
            "Elektrik Devreleri I",
            6,
            4,
            1,
            0
        ],
        [
            "PHY103",
            "Modern Fizik",
            6,
            3,
            1,
            1
        ],
        [
            "ETE207",
            "Malzeme Teknolojisi",
            6,
            3,
            2,
            0
        ],
        [
            "ETE299",
            "Temel Staj",
            2,
            0,
            0,
            0
        ],
        [
            "TUR001",
            "Türkçe I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG201",
            "İngilizce III",
            2,
            3,
            0,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "MAT204",
            "Veri Analizinin İstat. Yönt.",
            6,
            2,
            2,
            1
        ],
        [
            "ETE202",
            "Elektrik Devreleri II",
            6,
            3,
            2,
            0
        ],
        [
            "ETE208",
            "Ölçüm Teknikleri",
            6,
            2,
            1,
            2
        ],
        [
            "ETE292",
            "Proje Yönetimi",
            2,
            1,
            1,
            0
        ],
        [
            "TUR002",
            "Türkçe II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG202",
            "İngilizce IV",
            2,
            3,
            0,
            0
        ],
        [
            "USD000",
            "Üniversite Seçmeli Ders Havuzu",
            6,
            0,
            0,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "ETE321",
            "Elektromanyetik Alanlar",
            6,
            3,
            1,
            0
        ],
        [
            "ETE303",
            "Sinyaller ve Sistemler",
            6,
            3,
            1,
            1
        ],
        [
            "ETE311",
            "Elektronik I: Yarı İletken El.",
            6,
            2,
            2,
            1
        ],
        [
            "ETE331",
            "Elektrik Makineleri I",
            6,
            3,
            1,
            1
        ],
        [
            "SDV",
            "Seçmeli Ders Alanı V",
            6,
            0,
            0,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "ETE304",
            "Kontrol Müh. Temelleri",
            6,
            3,
            1,
            1
        ],
        [
            "ETE372",
            "Telekomünikasyon",
            6,
            3,
            2,
            0
        ],
        [
            "SDT",
            "Seçmeli Ders Alanı T",
            6,
            0,
            0,
            0
        ],
        [
            "SDM",
            "Seçmeli Ders Alanı M",
            6,
            0,
            0,
            0
        ],
        [
            "SDVI",
            "Seçmeli Ders Alanı VI",
            6,
            0,
            0,
            0
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "ETE499",
            "Mesleki Alan Stajı",
            6,
            0,
            0,
            0
        ],
        [
            "ISG001",
            "İş Sağlığı ve Güvenliği I",
            2,
            2,
            0,
            0
        ],
        [
            "AIT001",
            "Atatürk İlkeleri ve İnkılap Tarihi I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG301",
            "İleri İngilizce I",
            2,
            3,
            0,
            0
        ],
        [
            "SDP",
            "Seçmeli Alan - Proje",
            6,
            0,
            0,
            0
        ],
        [
            "SDVII",
            "Seçmeli Ders Alanı VII",
            12,
            0,
            0,
            0
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "ETE492",
            "Lisans Bitirme Çalışması",
            12,
            0,
            0,
            0
        ],
        [
            "ISG002",
            "İş Sağlığı ve Güvenliği II",
            2,
            2,
            0,
            0
        ],
        [
            "AIT002",
            "Atatürk İlkeleri ve İnkılap Tarihi II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG302",
            "İleri İngilizce II",
            2,
            3,
            0,
            0
        ],
        [
            "SDVIII",
            "Seçmeli Ders Alanı VIII",
            12,
            0,
            0,
            0
        ]
    ]
},
        "pool_codes": {
    "SDII": [
        "Mikroişlemciler",
        "Nesnel Programlama"
    ],
    "SDM": [
        "Numerik Matematik",
        "Kompleks Analiz"
    ],
    "SDP": [
        "Elektrik-Elektronik Müh. Projesi",
        "Mekatronik Projesi",
        "Yazılım Mühendisliği Projesi"
    ],
    "SDT": [
        "Elektronik II: Devre Teknolojisi",
        "Sayısal Sinyal İşlemenin Temelleri"
    ],
    "SDV": [
        "Mikroişlemciler",
        "Elektronik II: Devre Teknolojisi",
        "Sayısal Sinyal İşlemenin Temelleri",
        "Elektroteknik: Seçilmiş Konular I",
        "Elektroteknik: Seçilmiş Konular II",
        "Optik Haberleşme Tekniği",
        "İşlemsel Yükselteçler ile Devre Tasarımı",
        "Elektronik Laboratuvarı",
        "RF Ön Uç Bileşenleri ve Devreler",
        "Elektromanyetik Dalgaların Temelleri",
        "Anten Teorisine Giriş",
        "Elektrik Makineleri II",
        "Elektrik Enerjisi Kaynakları",
        "Güç Elektroniği",
        "FPGA Programlama",
        "Gömülü Sistemler",
        "Donanım Betimleme Dili ile Tasarım",
        "Analog Tümdevreler",
        "VLSI Tasarımına Giriş",
        "Doğrusal Olmayan Kontrol Sistemleri",
        "Hibrit Sistemler",
        "Ayrık Zamanlı Kontrol Sistemleri",
        "Çok Girdili Çok Çıktılı Sistemler",
        "Ayrık Olaylı Sistemler",
        "Sistem Tanımlama ve Akıllı Kontrol",
        "Doğrusal Olmayan Dinamik Sistemler",
        "Klinik Veri Yönetimi",
        "Yapay Sinir Ağları",
        "Makine Öğrenmesiyle Biyomedikal Modelleme",
        "Güvenilir Yapay Zeka",
        "Haberleşme Ağları",
        "Sayısal Haberleşme Sistemleri",
        "Sayısal Görüntü İşleme",
        "Sayısal Sinyal İşleme",
        "Bilgi Kuramı",
        "Elektroakustik",
        "Yüksek Frekans Tekniği",
        "Aydınlatma Teknolojisi",
        "Yüksek Gerilim Tekniği",
        "Güç Dağıtım Sistemleri",
        "Akıllı Şebekeler",
        "Yenilenebilir Enerji Teknolojisi",
        "İşletmede Mesleki Eğitim",
        "Elektroteknik: Seçilmiş Konular III",
        "Elektroteknik: Seçilmiş Konular IV",
        "Tıbbi Cihazlarda Kalite Yönetim Sisteminin Kurulması",
        "Tıbbi Cihazlarda Test, Kontrol ve Kalibrasyon",
        "Nesnel Programlama",
        "Akıllı Sistemler",
        "Makine Öğrenmesi",
        "Bilişim Sistemleri Güvenliği",
        "Derin Öğrenme",
        "Bilgisayar Görmesinin Temelleri",
        "İnsan-Makine İletişimi",
        "Yapay Zekâ",
        "Numerik Matematik",
        "Kompleks Analiz",
        "Uygulamalı Kontrol Mühendisliği",
        "Endüstriyel Robotik I",
        "Endüstriyel Otomasyon Teknolojisi",
        "Görüntü Tabanlı Otomasyon I",
        "Görüntü Tabanlı Otomasyon II",
        "Endüstriyel Robotik II"
    ],
    "SDVI": [
        "Mikroişlemciler",
        "Elektronik II: Devre Teknolojisi",
        "Sayısal Sinyal İşlemenin Temelleri",
        "Elektroteknik: Seçilmiş Konular I",
        "Elektroteknik: Seçilmiş Konular II",
        "Optik Haberleşme Tekniği",
        "İşlemsel Yükselteçler ile Devre Tasarımı",
        "Elektronik Laboratuvarı",
        "RF Ön Uç Bileşenleri ve Devreler",
        "Elektromanyetik Dalgaların Temelleri",
        "Anten Teorisine Giriş",
        "Elektrik Makineleri II",
        "Elektrik Enerjisi Kaynakları",
        "Güç Elektroniği",
        "FPGA Programlama",
        "Gömülü Sistemler",
        "Donanım Betimleme Dili ile Tasarım",
        "Analog Tümdevreler",
        "VLSI Tasarımına Giriş",
        "Doğrusal Olmayan Kontrol Sistemleri",
        "Hibrit Sistemler",
        "Ayrık Zamanlı Kontrol Sistemleri",
        "Çok Girdili Çok Çıktılı Sistemler",
        "Ayrık Olaylı Sistemler",
        "Sistem Tanımlama ve Akıllı Kontrol",
        "Doğrusal Olmayan Dinamik Sistemler",
        "Klinik Veri Yönetimi",
        "Yapay Sinir Ağları",
        "Makine Öğrenmesiyle Biyomedikal Modelleme",
        "Güvenilir Yapay Zeka",
        "Haberleşme Ağları",
        "Sayısal Haberleşme Sistemleri",
        "Sayısal Görüntü İşleme",
        "Sayısal Sinyal İşleme",
        "Bilgi Kuramı",
        "Elektroakustik",
        "Yüksek Frekans Tekniği",
        "Aydınlatma Teknolojisi",
        "Yüksek Gerilim Tekniği",
        "Güç Dağıtım Sistemleri",
        "Akıllı Şebekeler",
        "Yenilenebilir Enerji Teknolojisi",
        "İşletmede Mesleki Eğitim",
        "Elektroteknik: Seçilmiş Konular III",
        "Elektroteknik: Seçilmiş Konular IV",
        "Tıbbi Cihazlarda Kalite Yönetim Sisteminin Kurulması",
        "Tıbbi Cihazlarda Test, Kontrol ve Kalibrasyon",
        "Nesnel Programlama",
        "Akıllı Sistemler",
        "Makine Öğrenmesi",
        "Bilişim Sistemleri Güvenliği",
        "Derin Öğrenme",
        "Bilgisayar Görmesinin Temelleri",
        "İnsan-Makine İletişimi",
        "Yapay Zekâ",
        "Numerik Matematik",
        "Kompleks Analiz",
        "Uygulamalı Kontrol Mühendisliği",
        "Endüstriyel Robotik I",
        "Endüstriyel Otomasyon Teknolojisi",
        "Görüntü Tabanlı Otomasyon I",
        "Görüntü Tabanlı Otomasyon II",
        "Endüstriyel Robotik II"
    ],
    "SDVII": [
        "Mikroişlemciler",
        "Elektronik II: Devre Teknolojisi",
        "Sayısal Sinyal İşlemenin Temelleri",
        "Elektroteknik: Seçilmiş Konular I",
        "Elektroteknik: Seçilmiş Konular II",
        "Optik Haberleşme Tekniği",
        "İşlemsel Yükselteçler ile Devre Tasarımı",
        "Elektronik Laboratuvarı",
        "RF Ön Uç Bileşenleri ve Devreler",
        "Elektromanyetik Dalgaların Temelleri",
        "Anten Teorisine Giriş",
        "Elektrik Makineleri II",
        "Elektrik Enerjisi Kaynakları",
        "Güç Elektroniği",
        "FPGA Programlama",
        "Gömülü Sistemler",
        "Donanım Betimleme Dili ile Tasarım",
        "Analog Tümdevreler",
        "VLSI Tasarımına Giriş",
        "Doğrusal Olmayan Kontrol Sistemleri",
        "Hibrit Sistemler",
        "Ayrık Zamanlı Kontrol Sistemleri",
        "Çok Girdili Çok Çıktılı Sistemler",
        "Ayrık Olaylı Sistemler",
        "Sistem Tanımlama ve Akıllı Kontrol",
        "Doğrusal Olmayan Dinamik Sistemler",
        "Klinik Veri Yönetimi",
        "Yapay Sinir Ağları",
        "Makine Öğrenmesiyle Biyomedikal Modelleme",
        "Güvenilir Yapay Zeka",
        "Haberleşme Ağları",
        "Sayısal Haberleşme Sistemleri",
        "Sayısal Görüntü İşleme",
        "Sayısal Sinyal İşleme",
        "Bilgi Kuramı",
        "Elektroakustik",
        "Yüksek Frekans Tekniği",
        "Aydınlatma Teknolojisi",
        "Yüksek Gerilim Tekniği",
        "Güç Dağıtım Sistemleri",
        "Akıllı Şebekeler",
        "Yenilenebilir Enerji Teknolojisi",
        "İşletmede Mesleki Eğitim",
        "Elektroteknik: Seçilmiş Konular III",
        "Elektroteknik: Seçilmiş Konular IV",
        "Tıbbi Cihazlarda Kalite Yönetim Sisteminin Kurulması",
        "Tıbbi Cihazlarda Test, Kontrol ve Kalibrasyon",
        "Nesnel Programlama",
        "Akıllı Sistemler",
        "Makine Öğrenmesi",
        "Bilişim Sistemleri Güvenliği",
        "Derin Öğrenme",
        "Bilgisayar Görmesinin Temelleri",
        "İnsan-Makine İletişimi",
        "Yapay Zekâ",
        "Numerik Matematik",
        "Kompleks Analiz",
        "Uygulamalı Kontrol Mühendisliği",
        "Endüstriyel Robotik I",
        "Endüstriyel Otomasyon Teknolojisi",
        "Görüntü Tabanlı Otomasyon I",
        "Görüntü Tabanlı Otomasyon II",
        "Endüstriyel Robotik II"
    ],
    "SDVIII": [
        "Mikroişlemciler",
        "Elektronik II: Devre Teknolojisi",
        "Sayısal Sinyal İşlemenin Temelleri",
        "Elektroteknik: Seçilmiş Konular I",
        "Elektroteknik: Seçilmiş Konular II",
        "Optik Haberleşme Tekniği",
        "İşlemsel Yükselteçler ile Devre Tasarımı",
        "Elektronik Laboratuvarı",
        "RF Ön Uç Bileşenleri ve Devreler",
        "Elektromanyetik Dalgaların Temelleri",
        "Anten Teorisine Giriş",
        "Elektrik Makineleri II",
        "Elektrik Enerjisi Kaynakları",
        "Güç Elektroniği",
        "FPGA Programlama",
        "Gömülü Sistemler",
        "Donanım Betimleme Dili ile Tasarım",
        "Analog Tümdevreler",
        "VLSI Tasarımına Giriş",
        "Doğrusal Olmayan Kontrol Sistemleri",
        "Hibrit Sistemler",
        "Ayrık Zamanlı Kontrol Sistemleri",
        "Çok Girdili Çok Çıktılı Sistemler",
        "Ayrık Olaylı Sistemler",
        "Sistem Tanımlama ve Akıllı Kontrol",
        "Doğrusal Olmayan Dinamik Sistemler",
        "Klinik Veri Yönetimi",
        "Yapay Sinir Ağları",
        "Makine Öğrenmesiyle Biyomedikal Modelleme",
        "Güvenilir Yapay Zeka",
        "Haberleşme Ağları",
        "Sayısal Haberleşme Sistemleri",
        "Sayısal Görüntü İşleme",
        "Sayısal Sinyal İşleme",
        "Bilgi Kuramı",
        "Elektroakustik",
        "Yüksek Frekans Tekniği",
        "Aydınlatma Teknolojisi",
        "Yüksek Gerilim Tekniği",
        "Güç Dağıtım Sistemleri",
        "Akıllı Şebekeler",
        "Yenilenebilir Enerji Teknolojisi",
        "İşletmede Mesleki Eğitim",
        "Elektroteknik: Seçilmiş Konular III",
        "Elektroteknik: Seçilmiş Konular IV",
        "Tıbbi Cihazlarda Kalite Yönetim Sisteminin Kurulması",
        "Tıbbi Cihazlarda Test, Kontrol ve Kalibrasyon",
        "Nesnel Programlama",
        "Akıllı Sistemler",
        "Makine Öğrenmesi",
        "Bilişim Sistemleri Güvenliği",
        "Derin Öğrenme",
        "Bilgisayar Görmesinin Temelleri",
        "İnsan-Makine İletişimi",
        "Yapay Zekâ",
        "Numerik Matematik",
        "Kompleks Analiz",
        "Uygulamalı Kontrol Mühendisliği",
        "Endüstriyel Robotik I",
        "Endüstriyel Otomasyon Teknolojisi",
        "Görüntü Tabanlı Otomasyon I",
        "Görüntü Tabanlı Otomasyon II",
        "Endüstriyel Robotik II"
    ]
},
        "pools": {
    "SDII": [
        [
            "ETE104",
            "Mikroişlemciler",
            6,
            3,
            1,
            1
        ],
        [
            "INF102",
            "Nesnel Programlama",
            6,
            2,
            0,
            2
        ]
    ],
    "SDM": [
        [
            "MAT302",
            "Numerik Matematik",
            6,
            3,
            1,
            1
        ],
        [
            "MAT392",
            "Kompleks Analiz",
            6,
            3,
            1,
            1
        ]
    ],
    "SDT": [
        [
            "ETE312",
            "Elektronik II: Devre Teknolojisi",
            6,
            4,
            1,
            0
        ],
        [
            "ETE374",
            "Sayısal Sinyal İşlemenin Temelleri",
            6,
            2,
            1,
            2
        ]
    ],
    "SDP": [
        [
            "ETE491",
            "Elektrik-Elektronik Müh. Projesi",
            6,
            1,
            0,
            4
        ],
        [
            "MEC319",
            "Mekatronik Projesi",
            6,
            1,
            0,
            4
        ],
        [
            "INF303",
            "Yazılım Mühendisliği Projesi",
            6,
            1,
            0,
            3
        ]
    ],
    "SDV": [
        [
            "ETE104",
            "Mikroişlemciler",
            6,
            3,
            1,
            1
        ],
        [
            "ETE312",
            "Elektronik II: Devre Teknolojisi",
            6,
            4,
            1,
            0
        ],
        [
            "ETE374",
            "Sayısal Sinyal İşlemenin Temelleri",
            6,
            2,
            1,
            2
        ],
        [
            "ETE393",
            "Elektroteknik: Seçilmiş Konular I",
            6,
            2,
            0,
            2
        ],
        [
            "ETE394",
            "Elektroteknik: Seçilmiş Konular II",
            6,
            2,
            0,
            2
        ],
        [
            "ETE412",
            "Optik Haberleşme Tekniği",
            2,
            2,
            2,
            1
        ],
        [
            "ETE414",
            "İşlemsel Yükselteçler ile Devre Tasarımı",
            6,
            3,
            1,
            1
        ],
        [
            "ETE415",
            "Elektronik Laboratuvarı",
            6,
            2,
            0,
            2
        ],
        [
            "ETE417",
            "RF Ön Uç Bileşenleri ve Devreler",
            6,
            3,
            2,
            0
        ],
        [
            "ETE421",
            "Elektromanyetik Dalgaların Temelleri",
            6,
            3,
            2,
            0
        ],
        [
            "ETE422",
            "Anten Teorisine Giriş",
            6,
            3,
            2,
            0
        ],
        [
            "ETE433",
            "Elektrik Makineleri II",
            6,
            3,
            1,
            1
        ],
        [
            "ETE434",
            "Elektrik Enerjisi Kaynakları",
            6,
            3,
            1,
            1
        ],
        [
            "ETE439",
            "Güç Elektroniği",
            6,
            2,
            2,
            1
        ],
        [
            "ETE441",
            "FPGA Programlama",
            6,
            3,
            1,
            1
        ],
        [
            "ETE442",
            "Gömülü Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE444",
            "Donanım Betimleme Dili ile Tasarım",
            6,
            2,
            2,
            1
        ],
        [
            "ETE447",
            "Analog Tümdevreler",
            6,
            3,
            1,
            1
        ],
        [
            "ETE448",
            "VLSI Tasarımına Giriş",
            6,
            3,
            1,
            1
        ],
        [
            "ETE451",
            "Doğrusal Olmayan Kontrol Sistemleri",
            6,
            3,
            1,
            1
        ],
        [
            "ETE452",
            "Hibrit Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE453",
            "Ayrık Zamanlı Kontrol Sistemleri",
            6,
            3,
            1,
            1
        ],
        [
            "ETE454",
            "Çok Girdili Çok Çıktılı Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE455",
            "Ayrık Olaylı Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE456",
            "Sistem Tanımlama ve Akıllı Kontrol",
            6,
            2,
            1,
            2
        ],
        [
            "ETE459",
            "Doğrusal Olmayan Dinamik Sistemler",
            6,
            2,
            3,
            0
        ],
        [
            "ETE461",
            "Klinik Veri Yönetimi",
            6,
            2,
            1,
            2
        ],
        [
            "ETE464",
            "Yapay Sinir Ağları",
            6,
            2,
            2,
            1
        ],
        [
            "ETE466",
            "Makine Öğrenmesiyle Biyomedikal Modelleme",
            6,
            2,
            1,
            2
        ],
        [
            "ETE467",
            "Güvenilir Yapay Zeka",
            6,
            2,
            1,
            2
        ],
        [
            "ETE471",
            "Haberleşme Ağları",
            6,
            3,
            1,
            1
        ],
        [
            "ETE473",
            "Sayısal Haberleşme Sistemleri",
            6,
            2,
            2,
            1
        ],
        [
            "ETE474",
            "Sayısal Görüntü İşleme",
            6,
            2,
            1,
            2
        ],
        [
            "ETE475",
            "Sayısal Sinyal İşleme",
            6,
            2,
            1,
            2
        ],
        [
            "ETE476",
            "Bilgi Kuramı",
            6,
            3,
            2,
            0
        ],
        [
            "ETE477",
            "Elektroakustik",
            6,
            2,
            1,
            2
        ],
        [
            "ETE479",
            "Yüksek Frekans Tekniği",
            6,
            3,
            2,
            0
        ],
        [
            "ETE481",
            "Aydınlatma Teknolojisi",
            6,
            3,
            1,
            1
        ],
        [
            "ETE482",
            "Yüksek Gerilim Tekniği",
            6,
            3,
            1,
            1
        ],
        [
            "ETE484",
            "Güç Dağıtım Sistemleri",
            6,
            3,
            1,
            1
        ],
        [
            "ETE486",
            "Akıllı Şebekeler",
            6,
            3,
            2,
            0
        ],
        [
            "ETE487",
            "Yenilenebilir Enerji Teknolojisi",
            6,
            3,
            2,
            0
        ],
        [
            "ETE490",
            "İşletmede Mesleki Eğitim",
            15,
            0,
            0,
            0
        ],
        [
            "ETE493",
            "Elektroteknik: Seçilmiş Konular III",
            6,
            2,
            0,
            2
        ],
        [
            "ETE494",
            "Elektroteknik: Seçilmiş Konular IV",
            6,
            2,
            0,
            2
        ],
        [
            "ETE496",
            "Tıbbi Cihazlarda Kalite Yönetim Sisteminin Kurulması",
            6,
            3,
            2,
            0
        ],
        [
            "ETE498",
            "Tıbbi Cihazlarda Test, Kontrol ve Kalibrasyon",
            6,
            3,
            1,
            1
        ],
        [
            "INF102",
            "Nesnel Programlama",
            6,
            2,
            0,
            2
        ],
        [
            "INF501",
            "Akıllı Sistemler",
            6,
            2,
            0,
            2
        ],
        [
            "INF502",
            "Makine Öğrenmesi",
            6,
            2,
            0,
            2
        ],
        [
            "INF510",
            "Bilişim Sistemleri Güvenliği",
            6,
            2,
            0,
            2
        ],
        [
            "INF513",
            "Derin Öğrenme",
            4,
            2,
            0,
            2
        ],
        [
            "INF518",
            "Bilgisayar Görmesinin Temelleri",
            6,
            2,
            0,
            2
        ],
        [
            "INF523",
            "İnsan-Makine İletişimi",
            6,
            2,
            0,
            2
        ],
        [
            "INF701",
            "Yapay Zekâ",
            6,
            2,
            0,
            2
        ],
        [
            "MAT302",
            "Numerik Matematik",
            6,
            3,
            1,
            1
        ],
        [
            "MAT392",
            "Kompleks Analiz",
            6,
            3,
            1,
            1
        ],
        [
            "MEC002",
            "Uygulamalı Kontrol Mühendisliği",
            6,
            1,
            1,
            3
        ],
        [
            "MEC308",
            "Endüstriyel Robotik I",
            6,
            3,
            1,
            1
        ],
        [
            "MEC313",
            "Endüstriyel Otomasyon Teknolojisi",
            6,
            3,
            1,
            1
        ],
        [
            "MEC321",
            "Görüntü Tabanlı Otomasyon I",
            6,
            3,
            1,
            1
        ],
        [
            "MEC324",
            "Görüntü Tabanlı Otomasyon II",
            6,
            3,
            1,
            1
        ],
        [
            "MEC421",
            "Endüstriyel Robotik II",
            6,
            2,
            1,
            2
        ]
    ],
    "SDVI": [
        [
            "ETE104",
            "Mikroişlemciler",
            6,
            3,
            1,
            1
        ],
        [
            "ETE312",
            "Elektronik II: Devre Teknolojisi",
            6,
            4,
            1,
            0
        ],
        [
            "ETE374",
            "Sayısal Sinyal İşlemenin Temelleri",
            6,
            2,
            1,
            2
        ],
        [
            "ETE393",
            "Elektroteknik: Seçilmiş Konular I",
            6,
            2,
            0,
            2
        ],
        [
            "ETE394",
            "Elektroteknik: Seçilmiş Konular II",
            6,
            2,
            0,
            2
        ],
        [
            "ETE412",
            "Optik Haberleşme Tekniği",
            2,
            2,
            2,
            1
        ],
        [
            "ETE414",
            "İşlemsel Yükselteçler ile Devre Tasarımı",
            6,
            3,
            1,
            1
        ],
        [
            "ETE415",
            "Elektronik Laboratuvarı",
            6,
            2,
            0,
            2
        ],
        [
            "ETE417",
            "RF Ön Uç Bileşenleri ve Devreler",
            6,
            3,
            2,
            0
        ],
        [
            "ETE421",
            "Elektromanyetik Dalgaların Temelleri",
            6,
            3,
            2,
            0
        ],
        [
            "ETE422",
            "Anten Teorisine Giriş",
            6,
            3,
            2,
            0
        ],
        [
            "ETE433",
            "Elektrik Makineleri II",
            6,
            3,
            1,
            1
        ],
        [
            "ETE434",
            "Elektrik Enerjisi Kaynakları",
            6,
            3,
            1,
            1
        ],
        [
            "ETE439",
            "Güç Elektroniği",
            6,
            2,
            2,
            1
        ],
        [
            "ETE441",
            "FPGA Programlama",
            6,
            3,
            1,
            1
        ],
        [
            "ETE442",
            "Gömülü Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE444",
            "Donanım Betimleme Dili ile Tasarım",
            6,
            2,
            2,
            1
        ],
        [
            "ETE447",
            "Analog Tümdevreler",
            6,
            3,
            1,
            1
        ],
        [
            "ETE448",
            "VLSI Tasarımına Giriş",
            6,
            3,
            1,
            1
        ],
        [
            "ETE451",
            "Doğrusal Olmayan Kontrol Sistemleri",
            6,
            3,
            1,
            1
        ],
        [
            "ETE452",
            "Hibrit Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE453",
            "Ayrık Zamanlı Kontrol Sistemleri",
            6,
            3,
            1,
            1
        ],
        [
            "ETE454",
            "Çok Girdili Çok Çıktılı Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE455",
            "Ayrık Olaylı Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE456",
            "Sistem Tanımlama ve Akıllı Kontrol",
            6,
            2,
            1,
            2
        ],
        [
            "ETE459",
            "Doğrusal Olmayan Dinamik Sistemler",
            6,
            2,
            3,
            0
        ],
        [
            "ETE461",
            "Klinik Veri Yönetimi",
            6,
            2,
            1,
            2
        ],
        [
            "ETE464",
            "Yapay Sinir Ağları",
            6,
            2,
            2,
            1
        ],
        [
            "ETE466",
            "Makine Öğrenmesiyle Biyomedikal Modelleme",
            6,
            2,
            1,
            2
        ],
        [
            "ETE467",
            "Güvenilir Yapay Zeka",
            6,
            2,
            1,
            2
        ],
        [
            "ETE471",
            "Haberleşme Ağları",
            6,
            3,
            1,
            1
        ],
        [
            "ETE473",
            "Sayısal Haberleşme Sistemleri",
            6,
            2,
            2,
            1
        ],
        [
            "ETE474",
            "Sayısal Görüntü İşleme",
            6,
            2,
            1,
            2
        ],
        [
            "ETE475",
            "Sayısal Sinyal İşleme",
            6,
            2,
            1,
            2
        ],
        [
            "ETE476",
            "Bilgi Kuramı",
            6,
            3,
            2,
            0
        ],
        [
            "ETE477",
            "Elektroakustik",
            6,
            2,
            1,
            2
        ],
        [
            "ETE479",
            "Yüksek Frekans Tekniği",
            6,
            3,
            2,
            0
        ],
        [
            "ETE481",
            "Aydınlatma Teknolojisi",
            6,
            3,
            1,
            1
        ],
        [
            "ETE482",
            "Yüksek Gerilim Tekniği",
            6,
            3,
            1,
            1
        ],
        [
            "ETE484",
            "Güç Dağıtım Sistemleri",
            6,
            3,
            1,
            1
        ],
        [
            "ETE486",
            "Akıllı Şebekeler",
            6,
            3,
            2,
            0
        ],
        [
            "ETE487",
            "Yenilenebilir Enerji Teknolojisi",
            6,
            3,
            2,
            0
        ],
        [
            "ETE490",
            "İşletmede Mesleki Eğitim",
            15,
            0,
            0,
            0
        ],
        [
            "ETE493",
            "Elektroteknik: Seçilmiş Konular III",
            6,
            2,
            0,
            2
        ],
        [
            "ETE494",
            "Elektroteknik: Seçilmiş Konular IV",
            6,
            2,
            0,
            2
        ],
        [
            "ETE496",
            "Tıbbi Cihazlarda Kalite Yönetim Sisteminin Kurulması",
            6,
            3,
            2,
            0
        ],
        [
            "ETE498",
            "Tıbbi Cihazlarda Test, Kontrol ve Kalibrasyon",
            6,
            3,
            1,
            1
        ],
        [
            "INF102",
            "Nesnel Programlama",
            6,
            2,
            0,
            2
        ],
        [
            "INF501",
            "Akıllı Sistemler",
            6,
            2,
            0,
            2
        ],
        [
            "INF502",
            "Makine Öğrenmesi",
            6,
            2,
            0,
            2
        ],
        [
            "INF510",
            "Bilişim Sistemleri Güvenliği",
            6,
            2,
            0,
            2
        ],
        [
            "INF513",
            "Derin Öğrenme",
            4,
            2,
            0,
            2
        ],
        [
            "INF518",
            "Bilgisayar Görmesinin Temelleri",
            6,
            2,
            0,
            2
        ],
        [
            "INF523",
            "İnsan-Makine İletişimi",
            6,
            2,
            0,
            2
        ],
        [
            "INF701",
            "Yapay Zekâ",
            6,
            2,
            0,
            2
        ],
        [
            "MAT302",
            "Numerik Matematik",
            6,
            3,
            1,
            1
        ],
        [
            "MAT392",
            "Kompleks Analiz",
            6,
            3,
            1,
            1
        ],
        [
            "MEC002",
            "Uygulamalı Kontrol Mühendisliği",
            6,
            1,
            1,
            3
        ],
        [
            "MEC308",
            "Endüstriyel Robotik I",
            6,
            3,
            1,
            1
        ],
        [
            "MEC313",
            "Endüstriyel Otomasyon Teknolojisi",
            6,
            3,
            1,
            1
        ],
        [
            "MEC321",
            "Görüntü Tabanlı Otomasyon I",
            6,
            3,
            1,
            1
        ],
        [
            "MEC324",
            "Görüntü Tabanlı Otomasyon II",
            6,
            3,
            1,
            1
        ],
        [
            "MEC421",
            "Endüstriyel Robotik II",
            6,
            2,
            1,
            2
        ]
    ],
    "SDVII": [
        [
            "ETE104",
            "Mikroişlemciler",
            6,
            3,
            1,
            1
        ],
        [
            "ETE312",
            "Elektronik II: Devre Teknolojisi",
            6,
            4,
            1,
            0
        ],
        [
            "ETE374",
            "Sayısal Sinyal İşlemenin Temelleri",
            6,
            2,
            1,
            2
        ],
        [
            "ETE393",
            "Elektroteknik: Seçilmiş Konular I",
            6,
            2,
            0,
            2
        ],
        [
            "ETE394",
            "Elektroteknik: Seçilmiş Konular II",
            6,
            2,
            0,
            2
        ],
        [
            "ETE412",
            "Optik Haberleşme Tekniği",
            2,
            2,
            2,
            1
        ],
        [
            "ETE414",
            "İşlemsel Yükselteçler ile Devre Tasarımı",
            6,
            3,
            1,
            1
        ],
        [
            "ETE415",
            "Elektronik Laboratuvarı",
            6,
            2,
            0,
            2
        ],
        [
            "ETE417",
            "RF Ön Uç Bileşenleri ve Devreler",
            6,
            3,
            2,
            0
        ],
        [
            "ETE421",
            "Elektromanyetik Dalgaların Temelleri",
            6,
            3,
            2,
            0
        ],
        [
            "ETE422",
            "Anten Teorisine Giriş",
            6,
            3,
            2,
            0
        ],
        [
            "ETE433",
            "Elektrik Makineleri II",
            6,
            3,
            1,
            1
        ],
        [
            "ETE434",
            "Elektrik Enerjisi Kaynakları",
            6,
            3,
            1,
            1
        ],
        [
            "ETE439",
            "Güç Elektroniği",
            6,
            2,
            2,
            1
        ],
        [
            "ETE441",
            "FPGA Programlama",
            6,
            3,
            1,
            1
        ],
        [
            "ETE442",
            "Gömülü Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE444",
            "Donanım Betimleme Dili ile Tasarım",
            6,
            2,
            2,
            1
        ],
        [
            "ETE447",
            "Analog Tümdevreler",
            6,
            3,
            1,
            1
        ],
        [
            "ETE448",
            "VLSI Tasarımına Giriş",
            6,
            3,
            1,
            1
        ],
        [
            "ETE451",
            "Doğrusal Olmayan Kontrol Sistemleri",
            6,
            3,
            1,
            1
        ],
        [
            "ETE452",
            "Hibrit Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE453",
            "Ayrık Zamanlı Kontrol Sistemleri",
            6,
            3,
            1,
            1
        ],
        [
            "ETE454",
            "Çok Girdili Çok Çıktılı Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE455",
            "Ayrık Olaylı Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE456",
            "Sistem Tanımlama ve Akıllı Kontrol",
            6,
            2,
            1,
            2
        ],
        [
            "ETE459",
            "Doğrusal Olmayan Dinamik Sistemler",
            6,
            2,
            3,
            0
        ],
        [
            "ETE461",
            "Klinik Veri Yönetimi",
            6,
            2,
            1,
            2
        ],
        [
            "ETE464",
            "Yapay Sinir Ağları",
            6,
            2,
            2,
            1
        ],
        [
            "ETE466",
            "Makine Öğrenmesiyle Biyomedikal Modelleme",
            6,
            2,
            1,
            2
        ],
        [
            "ETE467",
            "Güvenilir Yapay Zeka",
            6,
            2,
            1,
            2
        ],
        [
            "ETE471",
            "Haberleşme Ağları",
            6,
            3,
            1,
            1
        ],
        [
            "ETE473",
            "Sayısal Haberleşme Sistemleri",
            6,
            2,
            2,
            1
        ],
        [
            "ETE474",
            "Sayısal Görüntü İşleme",
            6,
            2,
            1,
            2
        ],
        [
            "ETE475",
            "Sayısal Sinyal İşleme",
            6,
            2,
            1,
            2
        ],
        [
            "ETE476",
            "Bilgi Kuramı",
            6,
            3,
            2,
            0
        ],
        [
            "ETE477",
            "Elektroakustik",
            6,
            2,
            1,
            2
        ],
        [
            "ETE479",
            "Yüksek Frekans Tekniği",
            6,
            3,
            2,
            0
        ],
        [
            "ETE481",
            "Aydınlatma Teknolojisi",
            6,
            3,
            1,
            1
        ],
        [
            "ETE482",
            "Yüksek Gerilim Tekniği",
            6,
            3,
            1,
            1
        ],
        [
            "ETE484",
            "Güç Dağıtım Sistemleri",
            6,
            3,
            1,
            1
        ],
        [
            "ETE486",
            "Akıllı Şebekeler",
            6,
            3,
            2,
            0
        ],
        [
            "ETE487",
            "Yenilenebilir Enerji Teknolojisi",
            6,
            3,
            2,
            0
        ],
        [
            "ETE490",
            "İşletmede Mesleki Eğitim",
            15,
            0,
            0,
            0
        ],
        [
            "ETE493",
            "Elektroteknik: Seçilmiş Konular III",
            6,
            2,
            0,
            2
        ],
        [
            "ETE494",
            "Elektroteknik: Seçilmiş Konular IV",
            6,
            2,
            0,
            2
        ],
        [
            "ETE496",
            "Tıbbi Cihazlarda Kalite Yönetim Sisteminin Kurulması",
            6,
            3,
            2,
            0
        ],
        [
            "ETE498",
            "Tıbbi Cihazlarda Test, Kontrol ve Kalibrasyon",
            6,
            3,
            1,
            1
        ],
        [
            "INF102",
            "Nesnel Programlama",
            6,
            2,
            0,
            2
        ],
        [
            "INF501",
            "Akıllı Sistemler",
            6,
            2,
            0,
            2
        ],
        [
            "INF502",
            "Makine Öğrenmesi",
            6,
            2,
            0,
            2
        ],
        [
            "INF510",
            "Bilişim Sistemleri Güvenliği",
            6,
            2,
            0,
            2
        ],
        [
            "INF513",
            "Derin Öğrenme",
            4,
            2,
            0,
            2
        ],
        [
            "INF518",
            "Bilgisayar Görmesinin Temelleri",
            6,
            2,
            0,
            2
        ],
        [
            "INF523",
            "İnsan-Makine İletişimi",
            6,
            2,
            0,
            2
        ],
        [
            "INF701",
            "Yapay Zekâ",
            6,
            2,
            0,
            2
        ],
        [
            "MAT302",
            "Numerik Matematik",
            6,
            3,
            1,
            1
        ],
        [
            "MAT392",
            "Kompleks Analiz",
            6,
            3,
            1,
            1
        ],
        [
            "MEC002",
            "Uygulamalı Kontrol Mühendisliği",
            6,
            1,
            1,
            3
        ],
        [
            "MEC308",
            "Endüstriyel Robotik I",
            6,
            3,
            1,
            1
        ],
        [
            "MEC313",
            "Endüstriyel Otomasyon Teknolojisi",
            6,
            3,
            1,
            1
        ],
        [
            "MEC321",
            "Görüntü Tabanlı Otomasyon I",
            6,
            3,
            1,
            1
        ],
        [
            "MEC324",
            "Görüntü Tabanlı Otomasyon II",
            6,
            3,
            1,
            1
        ],
        [
            "MEC421",
            "Endüstriyel Robotik II",
            6,
            2,
            1,
            2
        ]
    ],
    "SDVIII": [
        [
            "ETE104",
            "Mikroişlemciler",
            6,
            3,
            1,
            1
        ],
        [
            "ETE312",
            "Elektronik II: Devre Teknolojisi",
            6,
            4,
            1,
            0
        ],
        [
            "ETE374",
            "Sayısal Sinyal İşlemenin Temelleri",
            6,
            2,
            1,
            2
        ],
        [
            "ETE393",
            "Elektroteknik: Seçilmiş Konular I",
            6,
            2,
            0,
            2
        ],
        [
            "ETE394",
            "Elektroteknik: Seçilmiş Konular II",
            6,
            2,
            0,
            2
        ],
        [
            "ETE412",
            "Optik Haberleşme Tekniği",
            2,
            2,
            2,
            1
        ],
        [
            "ETE414",
            "İşlemsel Yükselteçler ile Devre Tasarımı",
            6,
            3,
            1,
            1
        ],
        [
            "ETE415",
            "Elektronik Laboratuvarı",
            6,
            2,
            0,
            2
        ],
        [
            "ETE417",
            "RF Ön Uç Bileşenleri ve Devreler",
            6,
            3,
            2,
            0
        ],
        [
            "ETE421",
            "Elektromanyetik Dalgaların Temelleri",
            6,
            3,
            2,
            0
        ],
        [
            "ETE422",
            "Anten Teorisine Giriş",
            6,
            3,
            2,
            0
        ],
        [
            "ETE433",
            "Elektrik Makineleri II",
            6,
            3,
            1,
            1
        ],
        [
            "ETE434",
            "Elektrik Enerjisi Kaynakları",
            6,
            3,
            1,
            1
        ],
        [
            "ETE439",
            "Güç Elektroniği",
            6,
            2,
            2,
            1
        ],
        [
            "ETE441",
            "FPGA Programlama",
            6,
            3,
            1,
            1
        ],
        [
            "ETE442",
            "Gömülü Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE444",
            "Donanım Betimleme Dili ile Tasarım",
            6,
            2,
            2,
            1
        ],
        [
            "ETE447",
            "Analog Tümdevreler",
            6,
            3,
            1,
            1
        ],
        [
            "ETE448",
            "VLSI Tasarımına Giriş",
            6,
            3,
            1,
            1
        ],
        [
            "ETE451",
            "Doğrusal Olmayan Kontrol Sistemleri",
            6,
            3,
            1,
            1
        ],
        [
            "ETE452",
            "Hibrit Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE453",
            "Ayrık Zamanlı Kontrol Sistemleri",
            6,
            3,
            1,
            1
        ],
        [
            "ETE454",
            "Çok Girdili Çok Çıktılı Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE455",
            "Ayrık Olaylı Sistemler",
            6,
            2,
            2,
            1
        ],
        [
            "ETE456",
            "Sistem Tanımlama ve Akıllı Kontrol",
            6,
            2,
            1,
            2
        ],
        [
            "ETE459",
            "Doğrusal Olmayan Dinamik Sistemler",
            6,
            2,
            3,
            0
        ],
        [
            "ETE461",
            "Klinik Veri Yönetimi",
            6,
            2,
            1,
            2
        ],
        [
            "ETE464",
            "Yapay Sinir Ağları",
            6,
            2,
            2,
            1
        ],
        [
            "ETE466",
            "Makine Öğrenmesiyle Biyomedikal Modelleme",
            6,
            2,
            1,
            2
        ],
        [
            "ETE467",
            "Güvenilir Yapay Zeka",
            6,
            2,
            1,
            2
        ],
        [
            "ETE471",
            "Haberleşme Ağları",
            6,
            3,
            1,
            1
        ],
        [
            "ETE473",
            "Sayısal Haberleşme Sistemleri",
            6,
            2,
            2,
            1
        ],
        [
            "ETE474",
            "Sayısal Görüntü İşleme",
            6,
            2,
            1,
            2
        ],
        [
            "ETE475",
            "Sayısal Sinyal İşleme",
            6,
            2,
            1,
            2
        ],
        [
            "ETE476",
            "Bilgi Kuramı",
            6,
            3,
            2,
            0
        ],
        [
            "ETE477",
            "Elektroakustik",
            6,
            2,
            1,
            2
        ],
        [
            "ETE479",
            "Yüksek Frekans Tekniği",
            6,
            3,
            2,
            0
        ],
        [
            "ETE481",
            "Aydınlatma Teknolojisi",
            6,
            3,
            1,
            1
        ],
        [
            "ETE482",
            "Yüksek Gerilim Tekniği",
            6,
            3,
            1,
            1
        ],
        [
            "ETE484",
            "Güç Dağıtım Sistemleri",
            6,
            3,
            1,
            1
        ],
        [
            "ETE486",
            "Akıllı Şebekeler",
            6,
            3,
            2,
            0
        ],
        [
            "ETE487",
            "Yenilenebilir Enerji Teknolojisi",
            6,
            3,
            2,
            0
        ],
        [
            "ETE490",
            "İşletmede Mesleki Eğitim",
            15,
            0,
            0,
            0
        ],
        [
            "ETE493",
            "Elektroteknik: Seçilmiş Konular III",
            6,
            2,
            0,
            2
        ],
        [
            "ETE494",
            "Elektroteknik: Seçilmiş Konular IV",
            6,
            2,
            0,
            2
        ],
        [
            "ETE496",
            "Tıbbi Cihazlarda Kalite Yönetim Sisteminin Kurulması",
            6,
            3,
            2,
            0
        ],
        [
            "ETE498",
            "Tıbbi Cihazlarda Test, Kontrol ve Kalibrasyon",
            6,
            3,
            1,
            1
        ],
        [
            "INF102",
            "Nesnel Programlama",
            6,
            2,
            0,
            2
        ],
        [
            "INF501",
            "Akıllı Sistemler",
            6,
            2,
            0,
            2
        ],
        [
            "INF502",
            "Makine Öğrenmesi",
            6,
            2,
            0,
            2
        ],
        [
            "INF510",
            "Bilişim Sistemleri Güvenliği",
            6,
            2,
            0,
            2
        ],
        [
            "INF513",
            "Derin Öğrenme",
            4,
            2,
            0,
            2
        ],
        [
            "INF518",
            "Bilgisayar Görmesinin Temelleri",
            6,
            2,
            0,
            2
        ],
        [
            "INF523",
            "İnsan-Makine İletişimi",
            6,
            2,
            0,
            2
        ],
        [
            "INF701",
            "Yapay Zekâ",
            6,
            2,
            0,
            2
        ],
        [
            "MAT302",
            "Numerik Matematik",
            6,
            3,
            1,
            1
        ],
        [
            "MAT392",
            "Kompleks Analiz",
            6,
            3,
            1,
            1
        ],
        [
            "MEC002",
            "Uygulamalı Kontrol Mühendisliği",
            6,
            1,
            1,
            3
        ],
        [
            "MEC308",
            "Endüstriyel Robotik I",
            6,
            3,
            1,
            1
        ],
        [
            "MEC313",
            "Endüstriyel Otomasyon Teknolojisi",
            6,
            3,
            1,
            1
        ],
        [
            "MEC321",
            "Görüntü Tabanlı Otomasyon I",
            6,
            3,
            1,
            1
        ],
        [
            "MEC324",
            "Görüntü Tabanlı Otomasyon II",
            6,
            3,
            1,
            1
        ],
        [
            "MEC421",
            "Endüstriyel Robotik II",
            6,
            2,
            1,
            2
        ]
    ]
},
        "pool_metadata": {
    "SDII": {},
    "SDM": {},
    "SDT": {},
    "SDP": {},
    "SDV": {},
    "SDVI": {},
    "SDVII": {},
    "SDVIII": {}
},
    },
    "Endüstri Müh": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "WIN091",
            "Endüstri Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "WIN103",
            "İşletmeye Giriş",
            6,
            2,
            2,
            0
        ],
        [
            "WIN107",
            "Tasarım Tek. I: Teknik Çizim ve Bilg. Destekli Tasarım",
            6,
            2,
            0,
            4
        ],
        [
            "WIN109",
            "Statik",
            6,
            3,
            2,
            0
        ],
        [
            "MAT103",
            "Analiz I",
            6,
            3,
            2,
            0
        ],
        [
            "DEU121",
            "Teknik Almanca I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG101",
            "İngilizce I",
            2,
            3,
            0,
            0
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "WIN092",
            "Bilimsel Çalışma Yöntemleri",
            2,
            2,
            0,
            0
        ],
        [
            "WIN104",
            "İktisada Giriş",
            6,
            2,
            2,
            0
        ],
        [
            "WIN112",
            "Mukavemet",
            6,
            3,
            2,
            0
        ],
        [
            "MAT106",
            "Lineer Cebir",
            6,
            2,
            2,
            1
        ],
        [
            "MAT108",
            "Analiz II",
            6,
            3,
            2,
            0
        ],
        [
            "DEU118",
            "İşletme Almancası",
            2,
            2,
            0,
            0
        ],
        [
            "ENG102",
            "İngilizce II",
            2,
            3,
            0,
            0
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "WIN203",
            "Bilgisayar Bilimine ve Programlamaya Giriş",
            6,
            2,
            0,
            2
        ],
        [
            "WIN207",
            "Yatırım ve Finansman",
            6,
            2,
            2,
            0
        ],
        [
            "WIN209",
            "Yöneylem Araştırması I - Doğrusal Modeller",
            6,
            2,
            2,
            0
        ],
        [
            "WIN299",
            "Temel Staj",
            2,
            0,
            0,
            0
        ],
        [
            "MAT201",
            "Diferansiyel Denklemler",
            6,
            2,
            1,
            1
        ],
        [
            "ENG201",
            "İngilizce III",
            2,
            3,
            0,
            0
        ],
        [
            "TUR001",
            "Türkçe I",
            2,
            2,
            0,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "WIN204",
            "Muhasebe ve Bilanço",
            6,
            2,
            2,
            1
        ],
        [
            "WIN208",
            "Nesnel Programlama",
            6,
            2,
            0,
            2
        ],
        [
            "WIN212",
            "Elektrik ve Elektronik Mühendisliğinin Temelleri",
            6,
            3,
            1,
            1
        ],
        [
            "WIN292",
            "Proje Yönetimi",
            2,
            1,
            1,
            0
        ],
        [
            "MAT204",
            "Veri Analizinin İstatistiksel Yöntemleri",
            6,
            2,
            2,
            1
        ],
        [
            "ENG202",
            "İngilizce IV",
            2,
            3,
            0,
            0
        ],
        [
            "TUR002",
            "Türkçe II",
            2,
            2,
            0,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "WIN313",
            "Lojistik Yönetimi ve Teknolojileri",
            6,
            2,
            1,
            1
        ],
        [
            "WIN315",
            "Veritabanı Sistemleri",
            6,
            2,
            0,
            2
        ],
        [
            "SDP",
            "Seçmeli Alan - Proje",
            6,
            0,
            0,
            0
        ],
        [
            "SDI",
            "Seçmeli Alan I - İktisadi Bilimler",
            6,
            0,
            0,
            0
        ],
        [
            "ZSDI",
            "Zorunlu Seçmeli Alan I",
            6,
            0,
            0,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "WIN302",
            "Fabrika Yönetimine Giriş",
            6,
            2,
            1,
            1
        ],
        [
            "WIN314",
            "Kalite Yönetimi",
            6,
            3,
            1,
            1
        ],
        [
            "SDII",
            "Seçmeli Alan II - Endüstri Müh. Araştırma Alanı",
            6,
            0,
            0,
            0
        ],
        [
            "ZSDII",
            "Zorunlu Seçmeli Alan II",
            6,
            0,
            0,
            0
        ],
        [
            "USD000",
            "Üniversite Seçmeli Ders Havuzu",
            6,
            0,
            0,
            0
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "WIN403",
            "Proje II: Endüstri Projesi",
            6,
            1,
            0,
            4
        ],
        [
            "WIN499",
            "Mesleki Alan Stajı",
            6,
            0,
            0,
            0
        ],
        [
            "AIT001",
            "Atatürk İlkeleri ve İnkılap Tarihi I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG301",
            "İleri İngilizce I",
            2,
            3,
            0,
            0
        ],
        [
            "ISG001",
            "İş Sağlığı ve Güvenliği I",
            2,
            2,
            0,
            0
        ],
        [
            "ZSDI",
            "Zorunlu Seçmeli Alan I {SECIM:2}",
            12,
            0,
            0,
            0
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "WIN492",
            "Bitirme Tezi",
            12,
            0,
            0,
            0
        ],
        [
            "AIT002",
            "Atatürk İlkeleri ve İnkılap Tarihi II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG302",
            "İleri İngilizce II",
            2,
            3,
            0,
            0
        ],
        [
            "ISG002",
            "İş Sağlığı ve Güvenliği II",
            2,
            2,
            0,
            0
        ],
        [
            "ZSDII",
            "Zorunlu Seçmeli Alan II {SECIM:2}",
            12,
            0,
            0,
            0
        ]
    ]
},
        "pool_codes": {
    "SDI": [
        "Pazarlama ve Üretim Yönetimi",
        "İşletmesel Veri Analizi",
        "Mühendisler için Yöneticilik",
        "İktisadi Tarih"
    ],
    "SDII": [
        "Yöneylem Araştırması II - Rassal Modeller",
        "Mühendisler İçin Hukuk",
        "Para, Banka ve Finans Piyasaları"
    ],
    "SDP": [
        "Proje I: Yenilik ve Teknoloji Yönetimi Projesi",
        "Yazılım Mühendisliği Projesi",
        "Mekatronik Projesi"
    ],
    "ZSDI": [
        "Makine Öğrenmesi",
        "Havayolu Yönetimi",
        "Sürdürülebilir Üretim için Ürün Yaşam Döngüsü",
        "Endüstriyel Enformasyon Tek. ve Sanal Ürün Geliştirme",
        "Üretim Planlama ve Yönetimi",
        "Elektromobilite için Pil Üretim Teknolojisi",
        "Endüstri Mühendisliği: Seçilmiş Konular I",
        "Altı Sigma ve Problem Çözme",
        "Sayısal Tasarım",
        "Elektrik Devreleri I",
        "Algoritmalar ve Veri Yapıları I",
        "Yapay Sinir Ağları",
        "Veri Madenciliği",
        "Bilişim Sistemleri Güvenliği",
        "Web Mühendisliği",
        "İnsan-Makine Etkileşimi",
        "Tasarım Teknikleri II: Mekanik Parça Tasarımı",
        "Malzeme Teknolojisi I",
        "Takım Tezgahları",
        "Termodinamik",
        "İmalat Teknolojisi I",
        "Endüstriyel Otomasyon Teknolojisi"
    ],
    "ZSDII": [
        "Üretim ve Lojistik için Enformasyon Sistemleri",
        "Ergonomi: Metod-Zaman Ölçümü ve İş Bilimi",
        "Makine Öğrenmesi",
        "Endüstriyel Enformasyon Tek. ve Sanal Ürün Geliştirme",
        "Montaj Teknolojisi Temelleri",
        "Elektromobilite için Pil Üretim Teknolojisi",
        "Endüstri Mühendisliği: Seçilmiş Konular II",
        "Havaalanı Yönetimi",
        "Mühendisler için Python",
        "İşletmede Mesleki Eğitim",
        "Yazılım Mühendisliği",
        "Makine Öğrenmesi",
        "Yapay Sinir Ağları",
        "Veri Madenciliği",
        "Bilişim Sistemleri Güvenliği",
        "Web Mühendisliği",
        "İnsan-Makine Etkileşimi",
        "Girişimcilik",
        "Malzeme Teknolojisi I",
        "İmalat Teknolojisinin Temelleri",
        "Araç Teknolojisinin Temelleri",
        "Numerik Matematik",
        "Ölçüm Teknikleri",
        "Endüstriyel Robotik I"
    ]
},
        "pools": {
    "SDP": [
        [
            "WIN311",
            "Proje I: Yenilik ve Teknoloji Yönetimi Projesi",
            6,
            1,
            0,
            4
        ],
        [
            "INF303",
            "Yazılım Mühendisliği Projesi",
            6,
            1,
            0,
            3
        ],
        [
            "MEC319",
            "Mekatronik Projesi",
            6,
            1,
            0,
            4
        ]
    ],
    "SDI": [
        [
            "WIN309",
            "Pazarlama ve Üretim Yönetimi",
            6,
            2,
            2,
            0
        ],
        [
            "WIN317",
            "İşletmesel Veri Analizi",
            6,
            2,
            2,
            0
        ],
        [
            "WIN321",
            "Mühendisler için Yöneticilik",
            6,
            2,
            2,
            0
        ],
        [
            "VWL203",
            "İktisadi Tarih",
            6,
            3,
            0,
            0
        ]
    ],
    "SDII": [
        [
            "WIN316",
            "Yöneylem Araştırması II - Rassal Modeller",
            6,
            2,
            2,
            0
        ],
        [
            "ING406",
            "Mühendisler İçin Hukuk",
            6,
            3,
            1,
            0
        ],
        [
            "VWL210",
            "Para, Banka ve Finans Piyasaları",
            6,
            2,
            2,
            0
        ]
    ],
    "ZSDI": [
        [
            "WIN320",
            "Makine Öğrenmesi",
            6,
            2,
            0,
            2
        ],
        [
            "WIN323",
            "Havayolu Yönetimi",
            6,
            2,
            2,
            0
        ],
        [
            "WIN405",
            "Sürdürülebilir Üretim için Ürün Yaşam Döngüsü",
            6,
            2,
            1,
            2
        ],
        [
            "WIN406",
            "Endüstriyel Enformasyon Tek. ve Sanal Ürün Geliştirme",
            6,
            2,
            1,
            1
        ],
        [
            "WIN407",
            "Üretim Planlama ve Yönetimi",
            6,
            2,
            2,
            0
        ],
        [
            "WIN409",
            "Elektromobilite için Pil Üretim Teknolojisi",
            6,
            2,
            2,
            0
        ],
        [
            "WIN411",
            "Endüstri Mühendisliği: Seçilmiş Konular I",
            6,
            2,
            2,
            0
        ],
        [
            "WIN423",
            "Altı Sigma ve Problem Çözme",
            6,
            2,
            2,
            0
        ],
        [
            "ETE101",
            "Sayısal Tasarım",
            6,
            2,
            1,
            1
        ],
        [
            "ETE201",
            "Elektrik Devreleri I",
            6,
            3,
            2,
            0
        ],
        [
            "INF203",
            "Algoritmalar ve Veri Yapıları I",
            6,
            2,
            0,
            2
        ],
        [
            "INF503",
            "Yapay Sinir Ağları",
            6,
            2,
            0,
            2
        ],
        [
            "INF505",
            "Veri Madenciliği",
            6,
            2,
            0,
            2
        ],
        [
            "INF510",
            "Bilişim Sistemleri Güvenliği",
            6,
            2,
            0,
            2
        ],
        [
            "INF522",
            "Web Mühendisliği",
            6,
            2,
            0,
            2
        ],
        [
            "INF523",
            "İnsan-Makine Etkileşimi",
            6,
            2,
            0,
            2
        ],
        [
            "MAB203",
            "Tasarım Teknikleri II: Mekanik Parça Tasarımı",
            6,
            2,
            1,
            2
        ],
        [
            "MAB207",
            "Malzeme Teknolojisi I",
            6,
            3,
            2,
            0
        ],
        [
            "MAB301",
            "Takım Tezgahları",
            6,
            2,
            1,
            2
        ],
        [
            "MAB303",
            "Termodinamik",
            6,
            3,
            2,
            0
        ],
        [
            "MAB311",
            "İmalat Teknolojisi I",
            6,
            3,
            1,
            1
        ],
        [
            "MEC313",
            "Endüstriyel Otomasyon Teknolojisi",
            6,
            3,
            1,
            1
        ]
    ],
    "ZSDII": [
        [
            "WIN306",
            "Üretim ve Lojistik için Enformasyon Sistemleri",
            6,
            2,
            1,
            2
        ],
        [
            "WIN318",
            "Ergonomi: Metod-Zaman Ölçümü ve İş Bilimi",
            6,
            2,
            0,
            2
        ],
        [
            "WIN320",
            "Makine Öğrenmesi",
            6,
            2,
            0,
            2
        ],
        [
            "WIN406",
            "Endüstriyel Enformasyon Tek. ve Sanal Ürün Geliştirme",
            6,
            2,
            1,
            1
        ],
        [
            "WIN408",
            "Montaj Teknolojisi Temelleri",
            6,
            2,
            2,
            0
        ],
        [
            "WIN409",
            "Elektromobilite için Pil Üretim Teknolojisi",
            6,
            2,
            2,
            0
        ],
        [
            "WIN412",
            "Endüstri Mühendisliği: Seçilmiş Konular II",
            6,
            2,
            2,
            0
        ],
        [
            "WIN422",
            "Havaalanı Yönetimi",
            6,
            2,
            2,
            0
        ],
        [
            "WIN424",
            "Mühendisler için Python",
            6,
            2,
            0,
            2
        ],
        [
            "WIN490",
            "İşletmede Mesleki Eğitim",
            15,
            0,
            0,
            0
        ],
        [
            "INF202",
            "Yazılım Mühendisliği",
            6,
            1,
            0,
            3
        ],
        [
            "INF502",
            "Makine Öğrenmesi",
            6,
            2,
            0,
            2
        ],
        [
            "INF503",
            "Yapay Sinir Ağları",
            6,
            2,
            0,
            2
        ],
        [
            "INF505",
            "Veri Madenciliği",
            6,
            2,
            0,
            2
        ],
        [
            "INF510",
            "Bilişim Sistemleri Güvenliği",
            6,
            2,
            0,
            2
        ],
        [
            "INF522",
            "Web Mühendisliği",
            6,
            2,
            0,
            2
        ],
        [
            "INF523",
            "İnsan-Makine Etkileşimi",
            6,
            2,
            0,
            2
        ],
        [
            "ING404",
            "Girişimcilik",
            2,
            2,
            0,
            0
        ],
        [
            "MAB207",
            "Malzeme Teknolojisi I",
            6,
            3,
            2,
            0
        ],
        [
            "MAB210",
            "İmalat Teknolojisinin Temelleri",
            6,
            3,
            1,
            1
        ],
        [
            "MAB354",
            "Araç Teknolojisinin Temelleri",
            6,
            3,
            1,
            1
        ],
        [
            "MAT302",
            "Numerik Matematik",
            6,
            3,
            1,
            1
        ],
        [
            "MEC208",
            "Ölçüm Teknikleri",
            6,
            2,
            1,
            2
        ],
        [
            "MEC308",
            "Endüstriyel Robotik I",
            6,
            3,
            1,
            1
        ]
    ]
},
        "pool_metadata": {
    "SDP": {},
    "SDI": {},
    "SDII": {},
    "ZSDI": {},
    "ZSDII": {}
},
    },
    "Enerji Bilimi ve Teknolojileri": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "PHY111",
            "Fizik I",
            6,
            2,
            1,
            2
        ],
        [
            "CHE111",
            "Kimya I",
            6,
            2,
            1,
            2
        ],
        [
            "EBT103",
            "Enerji Bilimi ve Teknolojilerine Giriş",
            2,
            2,
            1,
            0
        ],
        [
            "MAT103",
            "Analiz I",
            6,
            3,
            2,
            0
        ],
        [
            "EBT105",
            "Teknik Çizim ve CAD",
            6,
            2,
            0,
            4
        ],
        [
            "DEU121",
            "Teknik Almanca I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG101",
            "İngilizce I",
            2,
            3,
            0,
            0
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "PHY112",
            "Fizik II",
            6,
            2,
            1,
            2
        ],
        [
            "CHE112",
            "Kimya II",
            6,
            2,
            1,
            2
        ],
        [
            "MAT112",
            "Analiz II ve Lineer Cebir",
            6,
            3,
            2,
            0
        ],
        [
            "EBT104",
            "Bilimsel Programlama",
            6,
            2,
            0,
            2
        ],
        [
            "NWI106",
            "Proje Yönetimi",
            2,
            2,
            0,
            0
        ],
        [
            "DEU122",
            "Teknik Almanca II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG102",
            "İngilizce II",
            2,
            3,
            0,
            0
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "EBT201",
            "Yenilenebilir Enerji Teknolojileri",
            6,
            3,
            1,
            0
        ],
        [
            "EBT203",
            "Elektrokimya",
            6,
            3,
            1,
            0
        ],
        [
            "MAT201",
            "Diferansiyel Denklemler",
            6,
            2,
            2,
            1
        ],
        [
            "NWI206",
            "Elektroteknik",
            6,
            2,
            1,
            2
        ],
        [
            "TUR001",
            "Türkçe I",
            2,
            2,
            0,
            0
        ],
        [
            "AIT001",
            "Atatürk İlkeleri ve İnkılap Tarihi I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG201",
            "İngilizce III",
            2,
            3,
            0,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "EBT204",
            "Termodinamik",
            6,
            3,
            2,
            0
        ],
        [
            "EBT206",
            "Katıhal Fiziği",
            6,
            2,
            1,
            0
        ],
        [
            "TUR002",
            "Türkçe II",
            2,
            2,
            0,
            0
        ],
        [
            "AIT002",
            "Atatürk İlkeleri ve İnkılap Tarihi II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG202",
            "İngilizce IV",
            2,
            3,
            0,
            0
        ],
        [
            "ZSD",
            "Zorunlu Seçmeli Ders",
            6,
            2,
            1,
            0
        ],
        [
            "SD",
            "Seçmeli Ders",
            6,
            2,
            1,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "EBT303",
            "Akışkanlar Mekaniği",
            6,
            3,
            2,
            0
        ],
        [
            "EBT305",
            "İstatistik",
            6,
            2,
            2,
            0
        ],
        [
            "ZSD",
            "Zorunlu Seçmeli Ders",
            6,
            2,
            1,
            0
        ],
        [
            "ZSD",
            "Zorunlu Seçmeli Ders",
            6,
            2,
            1,
            0
        ],
        [
            "ZSD",
            "Zorunlu Seçmeli Ders",
            6,
            2,
            1,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "EBT308",
            "Uygulamalı Enerji Bilimi Laboratuvarı",
            6,
            1,
            0,
            5
        ],
        [
            "EBT302",
            "Nümerik Analiz",
            6,
            2,
            1,
            0
        ],
        [
            "EBT306",
            "Isı Transferi",
            6,
            3,
            2,
            0
        ],
        [
            "ZSD",
            "Zorunlu Seçmeli Ders",
            6,
            3,
            0,
            0
        ],
        [
            "ZSD",
            "Zorunlu Seçmeli Ders",
            6,
            2,
            1,
            0
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "EBT401",
            "Proje I",
            6,
            4,
            1,
            0
        ],
        [
            "ISG001",
            "İş Sağlığı ve Güvenliği I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG301",
            "İleri İngilizce I",
            2,
            3,
            0,
            0
        ],
        [
            "ZSD",
            "Zorunlu Seçmeli Ders",
            6,
            2,
            1,
            0
        ],
        [
            "ZSD",
            "Zorunlu Seçmeli Ders",
            6,
            2,
            1,
            0
        ],
        [
            "SD",
            "Seçmeli Ders",
            6,
            2,
            1,
            0
        ],
        [
            "ÜSD",
            "Üniversite Seçmeli Ders",
            2,
            2,
            0,
            0
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "EBT402",
            "Proje II",
            10,
            0,
            6,
            0
        ],
        [
            "EBT404",
            "Seminer",
            2,
            2,
            0,
            0
        ],
        [
            "EBT406",
            "Enerji Yönetimi",
            6,
            3,
            2,
            0
        ],
        [
            "PRK400",
            "Endüstri Stajı Semineri",
            5,
            2,
            0,
            0
        ],
        [
            "ISG002",
            "İş Sağlığı ve Güvenliği II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG302",
            "İleri İngilizce II",
            2,
            3,
            0,
            0
        ],
        [
            "ÜSD",
            "Üniversite Seçmeli Ders",
            3,
            2,
            0,
            0
        ]
    ]
},
        "pool_codes": {
    "SD": [
        "Fiziksel Kimya 2",
        "Veri Analizinin İstatistiksel Yöntemleri",
        "Malzeme Üretim ve İşleme Teknolojileri",
        "Akıllı Şebekeler",
        "İklim Sistemine Giriş",
        "Bilimsel Çalışma Yöntemleri",
        "Elektrik Makineleri",
        "Fonksiyonel Malzemeler",
        "Polimerik Malzemeler"
    ],
    "ZSD": [
        "Hidrojen Enerjisi ve Yakıt Hücreleri",
        "Enerji Sistemlerinde Optimizasyon",
        "Güneş Enerjisi Sistemleri",
        "Rüzgar Enerjisi",
        "Hammadde ve Enerjiye Giriş",
        "Kuantum Enerji Sistemlerine Giriş",
        "Yenilenebilir Enerji – Malzeme, Yapı, İşlev",
        "Sürdürülebilir Enerji",
        "Enerji Depolama Sistemleri",
        "Güneş Hücreleri Fiziği",
        "Nükleer Enerji",
        "İleri Nükleer Enerji",
        "İleri Kuantum Enerji Sistemleri",
        "İnce Film Kaplama Teknolojileri",
        "Soğutma Teknolojisi",
        "Sürdürülebilir Biyoyakıt Teknolojileri",
        "Heterojen Kataliz",
        "Makine Öğrenmesine Giriş",
        "Enerji Ekonomisi ve Politikaları",
        "Enerji Sistem Modellemesi ve Simülasyonu",
        "Kömür İşleme ve Teknolojisi",
        "Temiz Yanma Teknolojileri",
        "Malzemelerin İleri Karakterizasyon Teknikleri"
    ],
    "ÜSD": [
        "Fikri Mülkiyet Hukuku",
        "Felsefe Tarihi",
        "İktisadi Tarihi",
        "Girişimcilik",
        "Bilgisayar Bilimine ve Programlamaya Giriş",
        "Statik",
        "Muhasebe ve Bilanço",
        "Doğa Bilimlerine Giriş"
    ]
},
        "pools": {
    "ZSD": [
        [
            "EBT311",
            "Hidrojen Enerjisi ve Yakıt Hücreleri",
            4,
            2,
            1,
            0
        ],
        [
            "EBT313",
            "Enerji Sistemlerinde Optimizasyon",
            6,
            2,
            1,
            0
        ],
        [
            "EBT301",
            "Güneş Enerjisi Sistemleri",
            6,
            2,
            1,
            1
        ],
        [
            "EBT304",
            "Rüzgar Enerjisi",
            6,
            2,
            1,
            1
        ],
        [
            "EBT307",
            "Hammadde ve Enerjiye Giriş",
            6,
            2,
            1,
            1
        ],
        [
            "EBT309",
            "Kuantum Enerji Sistemlerine Giriş",
            6,
            3,
            1,
            0
        ],
        [
            "EBT310",
            "Yenilenebilir Enerji – Malzeme, Yapı, İşlev",
            6,
            3,
            1,
            0
        ],
        [
            "EBT312",
            "Sürdürülebilir Enerji",
            6,
            3,
            0,
            0
        ],
        [
            "EBT314",
            "Enerji Depolama Sistemleri",
            6,
            2,
            1,
            0
        ],
        [
            "EBT315",
            "Güneş Hücreleri Fiziği",
            6,
            2,
            1,
            0
        ],
        [
            "EBT316",
            "Nükleer Enerji",
            6,
            3,
            2,
            0
        ],
        [
            "EBT317",
            "İleri Nükleer Enerji",
            6,
            3,
            2,
            0
        ],
        [
            "EBT320",
            "İleri Kuantum Enerji Sistemleri",
            6,
            3,
            1,
            0
        ],
        [
            "EBT321",
            "İnce Film Kaplama Teknolojileri",
            6,
            3,
            0,
            0
        ],
        [
            "EBT322",
            "Soğutma Teknolojisi",
            6,
            2,
            2,
            0
        ],
        [
            "EBT323",
            "Sürdürülebilir Biyoyakıt Teknolojileri",
            6,
            2,
            2,
            0
        ],
        [
            "EBT324",
            "Heterojen Kataliz",
            6,
            2,
            2,
            0
        ],
        [
            "EBT357",
            "Makine Öğrenmesine Giriş",
            6,
            3,
            1,
            0
        ],
        [
            "EBT403",
            "Enerji Ekonomisi ve Politikaları",
            6,
            2,
            2,
            0
        ],
        [
            "EBT411",
            "Enerji Sistem Modellemesi ve Simülasyonu",
            6,
            2,
            1,
            0
        ],
        [
            "EBT413",
            "Kömür İşleme ve Teknolojisi",
            6,
            2,
            0,
            2
        ],
        [
            "EBT415",
            "Temiz Yanma Teknolojileri",
            6,
            2,
            1,
            0
        ],
        [
            "NWT408",
            "Malzemelerin İleri Karakterizasyon Teknikleri",
            6,
            2,
            1,
            1
        ]
    ],
    "SD": [
        [
            "NWI202",
            "Fiziksel Kimya 2",
            6,
            3,
            1,
            1
        ],
        [
            "MAT204",
            "Veri Analizinin İstatistiksel Yöntemleri",
            6,
            2,
            2,
            1
        ],
        [
            "NWT302",
            "Malzeme Üretim ve İşleme Teknolojileri",
            6,
            2,
            2,
            1
        ],
        [
            "EBT326",
            "Akıllı Şebekeler",
            6,
            2,
            1,
            0
        ],
        [
            "EBT325",
            "İklim Sistemine Giriş",
            4,
            2,
            1,
            0
        ],
        [
            "NWI401",
            "Bilimsel Çalışma Yöntemleri",
            2,
            2,
            0,
            0
        ],
        [
            "EBT412",
            "Elektrik Makineleri",
            6,
            3,
            2,
            0
        ],
        [
            "MWT405",
            "Fonksiyonel Malzemeler",
            6,
            2,
            1,
            0
        ],
        [
            "MWT307",
            "Polimerik Malzemeler",
            6,
            2,
            1,
            1
        ]
    ],
    "ÜSD": [
        [
            "HUK553",
            "Fikri Mülkiyet Hukuku",
            2,
            2,
            0,
            0
        ],
        [
            "KKW321",
            "Felsefe Tarihi",
            4,
            3,
            0,
            0
        ],
        [
            "VWL203",
            "İktisadi Tarihi",
            6,
            3,
            0,
            0
        ],
        [
            "ING404",
            "Girişimcilik",
            2,
            2,
            0,
            0
        ],
        [
            "INF101",
            "Bilgisayar Bilimine ve Programlamaya Giriş",
            6,
            2,
            0,
            2
        ],
        [
            "MAB109",
            "Statik",
            6,
            3,
            2,
            0
        ],
        [
            "WIN204",
            "Muhasebe ve Bilanço",
            6,
            2,
            2,
            1
        ],
        [
            "NWI107",
            "Doğa Bilimlerine Giriş",
            2,
            2,
            0,
            0
        ]
    ]
},
        "pool_metadata": {
    "ZSD": {},
    "SD": {},
    "ÜSD": {}
},
    },
    "Hukuk Fakültesi": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "HUK151",
            "Hukuka Giriş",
            3,
            0,
            0,
            0
        ],
        [
            "HUK153",
            "Anayasa Hukuku I",
            3,
            0,
            0,
            0
        ],
        [
            "HUK155",
            "Ceza Hukukunun Temel Prensipleri",
            6,
            0,
            0,
            0
        ],
        [
            "HUK157",
            "Hukuki İşlem Teorisi",
            5,
            0,
            0,
            0
        ],
        [
            "HUK159",
            "Anayasa Hukukunun Temel Prensipleri",
            5,
            0,
            0,
            0
        ],
        [
            "HUK161",
            "Medeni Hukuka Giriş ve Başlangıç Hükümleri",
            3,
            0,
            0,
            0
        ],
        [
            "HUK163",
            "Ceza Hukukuna Giriş",
            3,
            0,
            0,
            0
        ],
        [
            "ENG101",
            "İngilizce I",
            2,
            0,
            0,
            0
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "HUK150",
            "Anayasa Hukuku II",
            4,
            0,
            0,
            0
        ],
        [
            "HUK152",
            "Metodoloji",
            4,
            0,
            0,
            0
        ],
        [
            "HUK154",
            "Avrupa Hukuk Tarihi",
            5,
            0,
            0,
            0
        ],
        [
            "HUK156",
            "Anayasa Hukuku Pratik Çalışmaları",
            5,
            0,
            0,
            0
        ],
        [
            "HUK158",
            "Kişiler Hukuku",
            4,
            0,
            0,
            0
        ],
        [
            "HUK160",
            "Türk Hukuk Tarihi",
            2,
            0,
            0,
            0
        ],
        [
            "HUK162",
            "İktisat",
            2,
            0,
            0,
            0
        ],
        [
            "HUK164",
            "Hukuk Almancası",
            2,
            0,
            0,
            0
        ],
        [
            "ENG102",
            "İngilizce II",
            2,
            0,
            0,
            0
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "HUK251",
            "Ceza Hukuku Genel Hükümler I",
            5,
            0,
            0,
            0
        ],
        [
            "HUK253",
            "İdare Hukuku I",
            5,
            0,
            0,
            0
        ],
        [
            "HUK255",
            "Milletlerarası Hukuk I",
            3,
            0,
            0,
            0
        ],
        [
            "HUK257",
            "Alman Sözleşme Hukuku",
            5,
            0,
            0,
            0
        ],
        [
            "HUK259",
            "Hukuk Felsefesi ve Hukuk Sosyolojisi",
            4,
            0,
            0,
            0
        ],
        [
            "HUK261",
            "Borçlar Hukuku Genel Hükümler I",
            6,
            0,
            0,
            0
        ],
        [
            "ENG201",
            "İngilizce III",
            2,
            0,
            0,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "HUK250",
            "Ceza Hukuku Genel Hükümler II",
            5,
            0,
            0,
            0
        ],
        [
            "HUK252",
            "Milletlerarası Hukuk II",
            5,
            0,
            0,
            0
        ],
        [
            "HUK254",
            "Adil Yargılama İlkeleri",
            4,
            0,
            0,
            0
        ],
        [
            "HUK256",
            "İdare Hukuku II",
            5,
            0,
            0,
            0
        ],
        [
            "HUK258",
            "Mali Hukuk",
            3,
            0,
            0,
            0
        ],
        [
            "HUK260",
            "Borçlar Hukuku Genel Hükümler II",
            6,
            0,
            0,
            0
        ],
        [
            "ENG202",
            "İngilizce IV",
            2,
            0,
            0,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "HUK351",
            "Ceza Hukuku Özel Hükümler I",
            3,
            0,
            0,
            0
        ],
        [
            "HUK353",
            "Ticari İşletme Hukuku",
            3,
            0,
            0,
            0
        ],
        [
            "HUK355",
            "Eşya Hukuku I",
            3,
            0,
            0,
            0
        ],
        [
            "HUK357",
            "Medeni Usul Hukuku I",
            3,
            0,
            0,
            0
        ],
        [
            "HUK359",
            "Avrupa Hukuku",
            4,
            0,
            0,
            0
        ],
        [
            "HUK361",
            "Alman İdare Hukuku – Temel Kavramlar",
            4,
            0,
            0,
            0
        ],
        [
            "HUK363",
            "Özel Borç İlişkileri I",
            2,
            0,
            0,
            0
        ],
        [
            "HUK365",
            "İş ve Sosyal Güvenlik Hukuku I",
            2,
            0,
            0,
            0
        ],
        [
            "ENG301",
            "İleri İngilizce I",
            2,
            0,
            0,
            0
        ],
        [
            "TUR001",
            "Türk Dili I",
            2,
            0,
            0,
            0
        ],
        [
            "HUKSD5",
            "Seçimlik Dersler",
            2,
            0,
            0,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "HUK350",
            "Ceza Hukuku Özel Hükümler II",
            3,
            0,
            0,
            0
        ],
        [
            "HUK352",
            "Şirketler Hukuku",
            3,
            0,
            0,
            0
        ],
        [
            "HUK354",
            "Eşya Hukuku II",
            3,
            0,
            0,
            0
        ],
        [
            "HUK356",
            "Medeni Usul Hukuku II",
            3,
            0,
            0,
            0
        ],
        [
            "HUK358",
            "İş ve Sosyal Güvenlik Hukuku II",
            2,
            0,
            0,
            0
        ],
        [
            "HUK360",
            "Devlet Kuramı",
            3,
            0,
            0,
            0
        ],
        [
            "HUK362",
            "Alman Ceza Muhakemesi Hukuku",
            4,
            0,
            0,
            0
        ],
        [
            "HUK364",
            "Özel Borç İlişkileri II",
            3,
            0,
            0,
            0
        ],
        [
            "ENG302",
            "İleri İngilizce II",
            2,
            0,
            0,
            0
        ],
        [
            "TUR002",
            "Türk Dili II",
            2,
            0,
            0,
            0
        ],
        [
            "HUKSD6",
            "Seçimlik Dersler",
            2,
            0,
            0,
            0
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "HUK451",
            "Ceza Muhakemesi Hukuku I",
            4,
            0,
            0,
            0
        ],
        [
            "HUK453",
            "Devletler Özel Hukuku I",
            3,
            0,
            0,
            0
        ],
        [
            "HUK455",
            "Aile ve Çocuk Hukuku",
            3,
            0,
            0,
            0
        ],
        [
            "HUK457",
            "İcra ve İflas Hukuku I",
            4,
            0,
            0,
            0
        ],
        [
            "HUK459",
            "İnsan Hakları Hukuku",
            2,
            0,
            0,
            0
        ],
        [
            "HUK461",
            "Alman Özel İdare Hukuku",
            4,
            0,
            0,
            0
        ],
        [
            "HUK463",
            "Taşıma Hukuku",
            2,
            0,
            0,
            0
        ],
        [
            "HUK465",
            "Kıymetli Evrak Hukuku",
            2,
            0,
            0,
            0
        ],
        [
            "AIT001",
            "Atatürk İlkeleri ve İnkılâp Tarihi I",
            2,
            0,
            0,
            0
        ],
        [
            "HUKSD7",
            "Seçimlik Dersler",
            2,
            0,
            0,
            0
        ],
        [
            "ÜSD000",
            "Üniversite Seçmeli Ders Havuzu",
            2,
            0,
            0,
            0
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "HUK450",
            "Ceza Muhakemesi Hukuku II",
            4,
            0,
            0,
            0
        ],
        [
            "HUK452",
            "İcra ve İflas Hukuku II",
            4,
            0,
            0,
            0
        ],
        [
            "HUK454",
            "Devletler Özel Hukuku II",
            5,
            0,
            0,
            0
        ],
        [
            "HUK456",
            "Miras Hukuku",
            4,
            0,
            0,
            0
        ],
        [
            "HUK458",
            "İdari Yargılama Hukuku",
            2,
            0,
            0,
            0
        ],
        [
            "HUK460",
            "Kamu Maliyesi ve Vergi Hukuku",
            3,
            0,
            0,
            0
        ],
        [
            "HUK462",
            "Seminer",
            2,
            0,
            0,
            0
        ],
        [
            "HUK464",
            "Sigorta Hukuku",
            3,
            0,
            0,
            0
        ],
        [
            "AIT002",
            "Atatürk İlkeleri ve İnkılâp Tarihi II",
            2,
            0,
            0,
            0
        ],
        [
            "HUKSD8",
            "Seçimlik Dersler",
            2,
            0,
            0,
            0
        ]
    ]
},
        "pool_codes": {
    "HUKSD5": [
        "DERSİN ADI",
        "Avrupa Hukuku ve Uluslararası Hukuk I",
        "Ekonomi Hukuku I",
        "İleri Alman Hukuku I"
    ],
    "HUKSD6": [
        "DERSİN ADI",
        "Avrupa Hukuku ve Uluslararası Hukuk II",
        "Ekonomi Hukuku II",
        "İleri Alman Hukuku II"
    ],
    "HUKSD7": [
        "DERSİN ADI",
        "İnfaz Hukuku",
        "Milletlerarası Mal Satım Sözleşmeleri Hukuku",
        "Rekabet Hukuku",
        "Staj Eğitimi",
        "İdarenin Sözleşmelerine ve Kamu İhale Hukukuna Giriş",
        "Türkiye-AB Ortaklık Hukuku",
        "Etik"
    ],
    "HUKSD8": [
        "DERSİN ADI",
        "Avrupa Hukuku ve Uluslararası Hukuk IV",
        "Ekonomi Hukuku IV",
        "İleri Alman Hukuku IV"
    ]
},
        "pools": {
    "HUKSD5": [
        [
            "MODÜL",
            "DERSİN ADI",
            0,
            0,
            0,
            0
        ],
        [
            "HUK561",
            "Avrupa Hukuku ve Uluslararası Hukuk I",
            2,
            0,
            0,
            0
        ],
        [
            "HUK563",
            "Ekonomi Hukuku I",
            2,
            0,
            0,
            0
        ],
        [
            "HUK565",
            "İleri Alman Hukuku I",
            2,
            0,
            0,
            0
        ]
    ],
    "HUKSD6": [
        [
            "MODÜL",
            "DERSİN ADI",
            0,
            0,
            0,
            0
        ],
        [
            "HUK658",
            "Avrupa Hukuku ve Uluslararası Hukuk II",
            2,
            0,
            0,
            0
        ],
        [
            "HUK660",
            "Ekonomi Hukuku II",
            2,
            0,
            0,
            0
        ],
        [
            "HUK662",
            "İleri Alman Hukuku II",
            2,
            0,
            0,
            0
        ]
    ],
    "HUKSD7": [
        [
            "MODÜL",
            "DERSİN ADI",
            0,
            0,
            0,
            0
        ],
        [
            "HUK751",
            "İnfaz Hukuku",
            2,
            0,
            0,
            0
        ],
        [
            "HUK753",
            "Milletlerarası Mal Satım Sözleşmeleri Hukuku",
            2,
            0,
            0,
            0
        ],
        [
            "HUK755",
            "Rekabet Hukuku",
            2,
            0,
            0,
            0
        ],
        [
            "HUK757",
            "Staj Eğitimi",
            2,
            0,
            0,
            0
        ],
        [
            "HUK765",
            "İdarenin Sözleşmelerine ve Kamu İhale Hukukuna Giriş",
            2,
            0,
            0,
            0
        ],
        [
            "HUK767",
            "Türkiye-AB Ortaklık Hukuku",
            2,
            0,
            0,
            0
        ],
        [
            "HUK769",
            "Etik",
            2,
            0,
            0,
            0
        ]
    ],
    "HUKSD8": [
        [
            "MODÜL",
            "DERSİN ADI",
            0,
            0,
            0,
            0
        ],
        [
            "HUK856",
            "Avrupa Hukuku ve Uluslararası Hukuk IV",
            2,
            0,
            0,
            0
        ],
        [
            "HUK858",
            "Ekonomi Hukuku IV",
            2,
            0,
            0,
            0
        ],
        [
            "HUK860",
            "İleri Alman Hukuku IV",
            2,
            0,
            0,
            0
        ]
    ]
},
        "pool_metadata": {
    "HUKSD5": {},
    "HUKSD6": {},
    "HUKSD7": {},
    "HUKSD8": {}
},
    },
    "Kültür ve İletişim Bölümü": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "AIT001",
            "Atatürk İlkeleri ve İnkılap Tarihi I",
            2,
            0,
            0,
            0
        ],
        [
            "ENG101",
            "İngilizce I",
            2,
            0,
            0,
            0
        ],
        [
            "KKW101",
            "Sosyal Bilimlerin Temelleri",
            6,
            0,
            0,
            0
        ],
        [
            "KKW103",
            "İletişim Bilimlerine Giriş",
            6,
            0,
            0,
            0
        ],
        [
            "KKW105",
            "Kültür Bilimlerine Giriş",
            6,
            0,
            0,
            0
        ],
        [
            "KKW121",
            "Çalışma Teknikleri: Bilimsel Çalışmalar",
            4,
            0,
            0,
            0
        ],
        [
            "KKW161",
            "Mesleki Almanca I: Kültür Bilimleri",
            2,
            0,
            0,
            0
        ],
        [
            "TUR001",
            "Türkçe I",
            2,
            0,
            0,
            0
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "AIT002",
            "Atatürk İlkeleri ve İnkılap Tarihi II",
            2,
            0,
            0,
            0
        ],
        [
            "ENG102",
            "İngilizce II",
            2,
            0,
            0,
            0
        ],
        [
            "KKW102",
            "Kültür Bilimleri Araştırma Yöntemleri",
            6,
            0,
            0,
            0
        ],
        [
            "KKW106",
            "Kültür Kuramları",
            5,
            0,
            0,
            0
        ],
        [
            "KKW118",
            "İletişim Bil. Niceliksel ve Niteliksel Araştırma Y.",
            6,
            0,
            0,
            0
        ],
        [
            "KKW150",
            "İletişim Kuramları I",
            5,
            0,
            0,
            0
        ],
        [
            "KKW162",
            "Mesleki Almanca II: İletişim Bilimleri",
            2,
            0,
            0,
            0
        ],
        [
            "TUR002",
            "Türkçe II",
            2,
            0,
            0,
            0
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "ENG201",
            "İngilizce III",
            2,
            0,
            0,
            0
        ],
        [
            "KKW209",
            "Kültür Üretiminin Araçları Olarak Edebiyat ve Sanat",
            4,
            0,
            0,
            0
        ],
        [
            "KKW219",
            "Yeni Medya Teknolojileri",
            3,
            0,
            0,
            0
        ],
        [
            "KKW221",
            "Kültür Tarihi",
            4,
            0,
            0,
            0
        ],
        [
            "KKW251",
            "İletişim Kuramları II",
            5,
            0,
            0,
            0
        ],
        [
            "KKW261",
            "Mesleki Almanca III: Kültürlerarası İletişim",
            2,
            0,
            0,
            0
        ],
        [
            "KKWSDI",
            "Seçmeli Ders I",
            10,
            0,
            0,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "ENG202",
            "İngilizce IV",
            2,
            0,
            0,
            0
        ],
        [
            "KKW212",
            "Kültürel Hafızanın Taşıyıcısı Olarak Edebiyat ve Sanat",
            4,
            0,
            0,
            0
        ],
        [
            "KKW216",
            "Kültür, Toplum ve Gazetecilik",
            4,
            0,
            0,
            0
        ],
        [
            "KKW218",
            "Göç ve Kültür",
            4,
            0,
            0,
            0
        ],
        [
            "KKW226",
            "Kültür ve Küreselleşme",
            4,
            0,
            0,
            0
        ],
        [
            "KKW262",
            "Mesleki Almanca IV: Metin Analizi",
            2,
            0,
            0,
            0
        ],
        [
            "KKWSDII",
            "Seçmeli Ders II",
            10,
            0,
            0,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "ENG301",
            "İleri İngilizce I",
            2,
            0,
            0,
            0
        ],
        [
            "KKW317",
            "Halkla İlişkiler",
            6,
            0,
            0,
            0
        ],
        [
            "KKW329",
            "Dijital Medya Laboratuvarı I",
            4,
            0,
            0,
            0
        ],
        [
            "KKW361",
            "Mesleki Almanca V: Metin Üretimi I",
            2,
            0,
            0,
            0
        ],
        [
            "KKWSDIII",
            "Seçmeli Dersler III",
            16,
            0,
            0,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "ENG302",
            "İleri İngilizce II",
            2,
            0,
            0,
            0
        ],
        [
            "KKW324",
            "Kültürel Bellek",
            6,
            0,
            0,
            0
        ],
        [
            "KKW332",
            "Dijital Medya Laboratuvarı II",
            4,
            0,
            0,
            0
        ],
        [
            "KKW362",
            "Mesleki Almanca VI: Metin Üretimi II",
            2,
            0,
            0,
            0
        ],
        [
            "KKWSDIV",
            "Seçmeli Dersler IV",
            16,
            0,
            0,
            0
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "KKW415",
            "Kültür ve Mekan",
            6,
            0,
            0,
            0
        ],
        [
            "KKW419",
            "Staj",
            6,
            0,
            0,
            0
        ],
        [
            "KKW437",
            "Araştırma Semineri",
            6,
            0,
            0,
            0
        ],
        [
            "KKW439",
            "Bilgi ve İletişim Hukuku",
            4,
            0,
            0,
            0
        ],
        [
            "KKW441",
            "Müzeler ve Koleksiyonlar",
            5,
            0,
            0,
            0
        ],
        [
            "KKWSDV",
            "Seçmeli Dersler V",
            3,
            0,
            0,
            0
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "KKW424",
            "Kültürel ve Yaratıcı Endüstriler",
            4,
            0,
            0,
            0
        ],
        [
            "KKW426",
            "Bitirme Projesi",
            12,
            0,
            0,
            0
        ],
        [
            "KKW428",
            "Kültür ve Medya Ekonomisi",
            4,
            0,
            0,
            0
        ],
        [
            "KKWSDVI",
            "Seçmeli Ders VI",
            4,
            0,
            0,
            0
        ],
        [
            "ÜSD",
            "Üniversite Seçmeli Ders Havuzu",
            6,
            0,
            0,
            0
        ]
    ]
},
        "pool_codes": {
    "KKWSDI": [
        "Almanya'da Göçmen Edebiyatı",
        "Görsel Kültür ve İletişim Araştırmaları",
        "Medya Tarihi",
        "Kültürlerarası İletişim",
        "Medya ve Kültür Bilimleri",
        "Medya Okuryazarlığı",
        "Medya ve Etik",
        "Alımlama ve Medya Etkileri Araştırmaları",
        "Edebiyat ve Metinlerarasılık",
        "Siyasal İletişim ve Medya",
        "Karşılaştırmalı Edebiyat",
        "Kültür Politikası",
        "Estetik",
        "Teori ve Uygulamada Çeviri",
        "Kurumsal İletişim",
        "Dil Felsefesi",
        "Sosyal Medya Yönetimi",
        "Dil ve İletişim"
    ],
    "KKWSDV": [
        "Almanya'da Göçmen Edebiyatı",
        "Görsel Kültür ve İletişim Araştırmaları",
        "Medya Tarihi",
        "Kültürlerarası İletişim",
        "Medya ve Kültür Bilimleri",
        "Medya Okuryazarlığı",
        "Medya ve Etik",
        "Alımlama ve Medya Etkileri Araştırmaları",
        "Edebiyat ve Metinlerarasılık",
        "Siyasal İletişim ve Medya",
        "Karşılaştırmalı Edebiyat",
        "Kültür Politikası",
        "Estetik",
        "Teori ve Uygulamada Çeviri",
        "Kurumsal İletişim",
        "Dil Felsefesi",
        "Sosyal Medya Yönetimi",
        "Dil ve İletişim"
    ],
    "ÜSD": [
        "İnşaat Mühendisliğine Giriş",
        "Uluslararası İşletme",
        "Organizasyon",
        "Pazarlamanın Temelleri",
        "Stratejik Pazarlama",
        "Elektrik-Elektronik Mühendisliğine Giriş",
        "Etik",
        "Girişimcilik",
        "Bilgisayar Bilimine ve Programlamaya Giriş",
        "Statik",
        "Sosyoloji ve Yapay Zeka",
        "Afet Yönetimi",
        "Dijital İnovasyon Yönetimi",
        "Kültürel Miras Yönetimi"
    ]
},
        "pools": {
    "ÜSD": [
        [
            "BAU091",
            "İnşaat Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "BWL034",
            "Uluslararası İşletme",
            6,
            1,
            1,
            0
        ],
        [
            "BWL102",
            "Organizasyon",
            3,
            2,
            0,
            0
        ],
        [
            "BWL201",
            "Pazarlamanın Temelleri",
            5,
            2,
            2,
            0
        ],
        [
            "BWL212",
            "Stratejik Pazarlama",
            5,
            2,
            2,
            0
        ],
        [
            "ETE091",
            "Elektrik-Elektronik Mühendisliğine Giriş",
            2,
            2,
            1,
            0
        ],
        [
            "HUK175",
            "Etik",
            2,
            2,
            0,
            0
        ],
        [
            "ING404",
            "Girişimcilik",
            2,
            2,
            0,
            0
        ],
        [
            "INF101",
            "Bilgisayar Bilimine ve Programlamaya Giriş",
            6,
            2,
            0,
            2
        ],
        [
            "MAB109",
            "Statik",
            6,
            3,
            2,
            0
        ],
        [
            "SOZ027",
            "Sosyoloji ve Yapay Zeka",
            6,
            3,
            0,
            0
        ],
        [
            "ÜSDAFY101",
            "Afet Yönetimi",
            3,
            1,
            1,
            0
        ],
        [
            "ÜSDINF406",
            "Dijital İnovasyon Yönetimi",
            6,
            2,
            2,
            0
        ],
        [
            "ÜSDKKW434",
            "Kültürel Miras Yönetimi",
            6,
            3,
            0,
            0
        ]
    ],
    "KKWSDI": [
        [
            "KKW001",
            "Almanya'da Göçmen Edebiyatı",
            3,
            2,
            0,
            0
        ],
        [
            "KKW220",
            "Görsel Kültür ve İletişim Araştırmaları",
            6,
            3,
            0,
            0
        ],
        [
            "KKW223",
            "Medya Tarihi",
            4,
            2,
            0,
            0
        ],
        [
            "KKW228",
            "Kültürlerarası İletişim",
            4,
            3,
            0,
            0
        ],
        [
            "KKW233",
            "Medya ve Kültür Bilimleri",
            6,
            3,
            0,
            0
        ],
        [
            "KKW302",
            "Medya Okuryazarlığı",
            6,
            2,
            1,
            0
        ],
        [
            "KKW306",
            "Medya ve Etik",
            5,
            3,
            0,
            0
        ],
        [
            "KKW307",
            "Alımlama ve Medya Etkileri Araştırmaları",
            6,
            3,
            0,
            0
        ],
        [
            "KKW325",
            "Edebiyat ve Metinlerarasılık",
            6,
            2,
            1,
            0
        ],
        [
            "KKW341",
            "Siyasal İletişim ve Medya",
            4,
            3,
            0,
            0
        ],
        [
            "KKW342",
            "Karşılaştırmalı Edebiyat",
            5,
            3,
            0,
            0
        ],
        [
            "KKW344",
            "Kültür Politikası",
            5,
            3,
            0,
            0
        ],
        [
            "KKW346",
            "Estetik",
            5,
            3,
            0,
            0
        ],
        [
            "KKW348",
            "Teori ve Uygulamada Çeviri",
            5,
            3,
            0,
            0
        ],
        [
            "KKW422",
            "Kurumsal İletişim",
            4,
            2,
            1,
            0
        ],
        [
            "KKW431",
            "Dil Felsefesi",
            3,
            2,
            0,
            0
        ],
        [
            "KKW433",
            "Sosyal Medya Yönetimi",
            3,
            3,
            0,
            0
        ],
        [
            "KKW443",
            "Dil ve İletişim",
            5,
            3,
            0,
            0
        ]
    ],
    "KKWSDV": [
        [
            "KKW001",
            "Almanya'da Göçmen Edebiyatı",
            3,
            2,
            0,
            0
        ],
        [
            "KKW220",
            "Görsel Kültür ve İletişim Araştırmaları",
            6,
            3,
            0,
            0
        ],
        [
            "KKW223",
            "Medya Tarihi",
            4,
            2,
            0,
            0
        ],
        [
            "KKW228",
            "Kültürlerarası İletişim",
            4,
            3,
            0,
            0
        ],
        [
            "KKW233",
            "Medya ve Kültür Bilimleri",
            6,
            3,
            0,
            0
        ],
        [
            "KKW302",
            "Medya Okuryazarlığı",
            6,
            2,
            1,
            0
        ],
        [
            "KKW306",
            "Medya ve Etik",
            5,
            3,
            0,
            0
        ],
        [
            "KKW307",
            "Alımlama ve Medya Etkileri Araştırmaları",
            6,
            3,
            0,
            0
        ],
        [
            "KKW325",
            "Edebiyat ve Metinlerarasılık",
            6,
            2,
            1,
            0
        ],
        [
            "KKW341",
            "Siyasal İletişim ve Medya",
            4,
            3,
            0,
            0
        ],
        [
            "KKW342",
            "Karşılaştırmalı Edebiyat",
            5,
            3,
            0,
            0
        ],
        [
            "KKW344",
            "Kültür Politikası",
            5,
            3,
            0,
            0
        ],
        [
            "KKW346",
            "Estetik",
            5,
            3,
            0,
            0
        ],
        [
            "KKW348",
            "Teori ve Uygulamada Çeviri",
            5,
            3,
            0,
            0
        ],
        [
            "KKW422",
            "Kurumsal İletişim",
            4,
            2,
            1,
            0
        ],
        [
            "KKW431",
            "Dil Felsefesi",
            3,
            2,
            0,
            0
        ],
        [
            "KKW433",
            "Sosyal Medya Yönetimi",
            3,
            3,
            0,
            0
        ],
        [
            "KKW443",
            "Dil ve İletişim",
            5,
            3,
            0,
            0
        ]
    ]
},
        "pool_metadata": {
    "ÜSD": {},
    "KKWSDI": {},
    "KKWSDV": {}
},
    },
    "Makine Müh": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "MAT103",
            "Analiz I",
            6,
            3,
            2,
            0
        ],
        [
            "MAB107",
            "Teknik Çizim ve CAD",
            6,
            2,
            0,
            0
        ],
        [
            "MAB109",
            "Statik",
            6,
            3,
            2,
            0
        ],
        [
            "MAB105",
            "Bilg. Bil. ve Prog. Giriş",
            6,
            2,
            2,
            0
        ],
        [
            "MAB091",
            "Makine Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "ENG101",
            "İngilizce I",
            2,
            3,
            0,
            0
        ],
        [
            "DEU121",
            "Teknik Almanca I",
            2,
            2,
            0,
            0
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "MAT108",
            "Analiz II",
            6,
            3,
            2,
            0
        ],
        [
            "MAT106",
            "Lineer Cebir",
            6,
            2,
            2,
            0
        ],
        [
            "MAB112",
            "Mukavemet",
            6,
            3,
            2,
            0
        ],
        [
            "MAB108",
            "Tasarım Tek. I: Teknik Çizim II",
            6,
            2,
            0,
            0
        ],
        [
            "MAB092",
            "Bilimsel Çalışma Yöntemleri",
            2,
            2,
            0,
            0
        ],
        [
            "ENG102",
            "İngilizce II",
            2,
            3,
            0,
            0
        ],
        [
            "DEU122",
            "Teknik Almanca II",
            2,
            2,
            0,
            0
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "MAB213",
            "Elektrik ve Elektronik Müh. Temelleri",
            6,
            3,
            1,
            0
        ],
        [
            "MAB203",
            "Tasarım Tek. II: Mekanik Parça Tas.",
            6,
            2,
            1,
            0
        ],
        [
            "MAB205",
            "Kinematik ve Dinamik",
            6,
            3,
            1,
            0
        ],
        [
            "MAB207",
            "Malzeme Teknolojisi I",
            6,
            3,
            2,
            0
        ],
        [
            "MAB209",
            "Temel Staj",
            2,
            0,
            0,
            0
        ],
        [
            "AIT001",
            "Atatürk İlkeleri ve İnkılap Tarihi I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG201",
            "İngilizce III",
            2,
            3,
            0,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "MAB210",
            "İmalat Teknolojisinin Temelleri",
            6,
            3,
            1,
            0
        ],
        [
            "MAB216",
            "Ölçüm Teknikleri",
            6,
            2,
            1,
            0
        ],
        [
            "MAT204",
            "Veri Analizinin İstat. Yönt.",
            6,
            2,
            2,
            0
        ],
        [
            "MAB214",
            "Dif. Denk. ve Sayısal Yönt.",
            6,
            2,
            2,
            0
        ],
        [
            "MAB292",
            "Proje Yönetimi",
            2,
            1,
            1,
            0
        ],
        [
            "AIT002",
            "Atatürk İlkeleri ve İnkılap Tarihi II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG202",
            "İngilizce IV",
            2,
            3,
            0,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "MAB301",
            "Takım Tezgâhları",
            6,
            2,
            1,
            0
        ],
        [
            "MAB317",
            "Titreşim ve Makine Dinamiği",
            6,
            3,
            2,
            0
        ],
        [
            "MAB303",
            "Termodinamik",
            6,
            3,
            2,
            0
        ],
        [
            "SDUx",
            "Zorunlu Seçmeli I - Uzmanlık A, B, C veya D",
            6,
            0,
            0,
            0
        ],
        [
            "ISG001",
            "İş Sağlığı ve Güvenliği I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG301",
            "İleri İngilizce I",
            2,
            3,
            0,
            0
        ],
        [
            "TUR001",
            "Türkçe I",
            2,
            2,
            0,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "MAB312",
            "Isı Transferi",
            6,
            3,
            1,
            0
        ],
        [
            "MAB314",
            "Akışkanlar Mekaniği",
            6,
            3,
            1,
            0
        ],
        [
            "SDK",
            "Zorunlu Seçmeli - Kontrol",
            6,
            0,
            0,
            0
        ],
        [
            "SDP",
            "Seçmeli Ders Alanı - Proje",
            6,
            0,
            0,
            0
        ],
        [
            "TUR002",
            "Türkçe II",
            2,
            2,
            0,
            0
        ],
        [
            "ISG002",
            "İş Sağlığı ve Güvenliği II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG302",
            "İleri İngilizce II",
            2,
            3,
            0,
            0
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "MAB499",
            "Mesleki Alan Stajı",
            6,
            0,
            0,
            0
        ],
        [
            "SUP",
            "Zorunlu Seçmeli - Uygulamalı Proje",
            6,
            0,
            0,
            0
        ],
        [
            "SDUx",
            "Zorunlu Seçmeli - Uzmanlık A, B, C veya D",
            12,
            0,
            0,
            0
        ],
        [
            "USD000",
            "Üniversite Seçmeli Ders Havuzu",
            6,
            0,
            0,
            0
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "MAB492",
            "Bitirme Tezi",
            12,
            0,
            0,
            0
        ],
        [
            "SDUx",
            "Zorunlu Seçmeli - Uzmanlık A, B, C veya D",
            18,
            0,
            0,
            0
        ]
    ]
},
        "pool_codes": {
    "SDK": [
        "Kontrol Mühendisliğinin Temelleri",
        "Kontrol Mühendisliğinin Temelleri"
    ],
    "SDP": [
        "Proje: Üretim Teknikleri",
        "Proje: Tasarım",
        "Proje: Uzay Havacılık",
        "Proje: Taşıt Sistemleri",
        "İnterdisipliner Proje",
        "Endüstri Projesi"
    ],
    "SDUa": [
        "Fabrika Yönetimine Giriş",
        "İmalat Teknolojisi I",
        "İmalat Teknolojisi II",
        "Yüzey Teknolojisi",
        "Proje: Üretim Teknikleri",
        "Üretim: Seçilmiş Konular I",
        "Malzeme Teknolojisi II",
        "İmalat Tesisleri",
        "Kaynak Teknolojisi",
        "Kalite İçin Veri İşleme",
        "Endüstriyel Robotik I",
        "İmalatta Sonlu Elemanlar",
        "Mikroprodüksiyon Teknolojileri",
        "Endüstriyel Otomasyon Tek.",
        "Endüstriyel Robotik II",
        "Elektrik Makineleri I",
        "Üretim: Seçilmiş Konular III",
        "Takım Tezgahları II"
    ],
    "SDUb": [
        "Tasarım Tek. III",
        "Sonlu Elemanlar Yöntemi",
        "Sürekli Ortamlar Mekaniği",
        "Makine Müh. Bilişim Tek.",
        "Proje: Tasarım",
        "Tasarım: Seçilmiş Konular I",
        "Metodik Tasarım",
        "Yorulma Dayanımı",
        "Mekanikte Enerji Yöntemleri",
        "Katı Cisim Simulasyonu",
        "Yapısal Mekanik",
        "Elektrik Makineleri I",
        "Dişli Teorisi ve Mekanizmalar",
        "Tasarım: Seçilmiş Konular II",
        "Tasarım: Seçilmiş Konular III"
    ],
    "SDUc": [
        "Akışkan Makineleri",
        "Hesaplamalı Akışkanlar Dinamiği I",
        "Havacılık Endüstrisine Giriş",
        "Proje: Uzay Havacılık",
        "Uzay Havacılık: Seç. Konular I",
        "Akıllı Sistemler",
        "Makine Öğrenmesi",
        "Yapısal Mekanik",
        "Tasarım Tek. III",
        "Sonlu Elemanlar Yöntemi",
        "Aerodinamik",
        "Sürekli Ortamlar Mekaniği",
        "Uzay Teknolojisinin Temelleri",
        "Hesaplamalı Akışkanlar Dinamiği II",
        "Hesaplamalı Akışkanlar Dinamiği III",
        "Uzay Havacılık: Seç. Konular II",
        "Uzay Havacılık: Seç. Konular III"
    ],
    "SDUd": [
        "Araç Teknolojisinin Temelleri",
        "Dişli Teorisi ve Mekanizmalar",
        "Katı Cisim Simulasyonu",
        "Proje: Taşıt Sistemleri",
        "Taşıt Sist.: Seç. Konular I",
        "Elektrik Makineleri I",
        "Motor Teknolojilerinin Temelleri",
        "Tasarım Tek. III",
        "Sonlu Elemanlar Yöntemi",
        "Malzeme Teknolojisi II",
        "Raylı Taşıt Teknolojisi",
        "Raylı Sistemlerin Temelleri",
        "Taşıt Sist.: Seç. Konular II",
        "Araç Tasarımının Temelleri",
        "Ticari Araç Mühendisliği",
        "Taşıt Sist.: Seç. Konular III"
    ],
    "SDUe": [
        "İşletmede Mesleki Eğitim"
    ],
    "SDUx": []
},
        "pools": {
    "SDK": [
        [
            "MAB308",
            "Kontrol Mühendisliğinin Temelleri",
            6,
            3,
            1,
            0
        ],
        [
            "ETE304",
            "Kontrol Mühendisliğinin Temelleri",
            6,
            3,
            1,
            0
        ]
    ],
    "SDP": [
        [
            "MAB391",
            "Proje: Üretim Teknikleri",
            6,
            1,
            0,
            0
        ],
        [
            "MAB392",
            "Proje: Tasarım",
            6,
            1,
            0,
            0
        ],
        [
            "MAB393",
            "Proje: Uzay Havacılık",
            6,
            1,
            0,
            0
        ],
        [
            "MAB394",
            "Proje: Taşıt Sistemleri",
            6,
            1,
            0,
            0
        ],
        [
            "MAB427",
            "İnterdisipliner Proje",
            6,
            1,
            0,
            0
        ],
        [
            "MAB428",
            "Endüstri Projesi",
            6,
            1,
            0,
            0
        ]
    ],
    "SDUx": [],
    "SDUa": [
        [
            "WIN302",
            "Fabrika Yönetimine Giriş",
            6,
            2,
            1,
            0
        ],
        [
            "MAB311",
            "İmalat Teknolojisi I",
            6,
            3,
            1,
            0
        ],
        [
            "MAB409",
            "İmalat Teknolojisi II",
            6,
            3,
            1,
            0
        ],
        [
            "MAB411",
            "Yüzey Teknolojisi",
            6,
            3,
            2,
            0
        ],
        [
            "MAB391",
            "Proje: Üretim Teknikleri",
            6,
            1,
            0,
            0
        ],
        [
            "MAB381",
            "Üretim: Seçilmiş Konular I",
            6,
            1,
            0,
            0
        ],
        [
            "MAB310",
            "Malzeme Teknolojisi II",
            6,
            3,
            1,
            0
        ],
        [
            "MAB423",
            "İmalat Tesisleri",
            6,
            3,
            2,
            0
        ],
        [
            "MAB404",
            "Kaynak Teknolojisi",
            6,
            3,
            2,
            0
        ],
        [
            "MAB408",
            "Kalite İçin Veri İşleme",
            6,
            3,
            2,
            0
        ],
        [
            "MEC308",
            "Endüstriyel Robotik I",
            6,
            3,
            1,
            0
        ],
        [
            "MAB453",
            "İmalatta Sonlu Elemanlar",
            6,
            3,
            1,
            0
        ],
        [
            "MAB403",
            "Mikroprodüksiyon Teknolojileri",
            6,
            3,
            2,
            0
        ],
        [
            "MEC313",
            "Endüstriyel Otomasyon Tek.",
            6,
            2,
            1,
            0
        ],
        [
            "MEC421",
            "Endüstriyel Robotik II",
            6,
            2,
            1,
            0
        ],
        [
            "ETE331",
            "Elektrik Makineleri I",
            6,
            3,
            1,
            0
        ],
        [
            "MAB465",
            "Üretim: Seçilmiş Konular III",
            6,
            3,
            2,
            0
        ],
        [
            "MAB470",
            "Takım Tezgahları II",
            6,
            3,
            2,
            0
        ]
    ],
    "SDUb": [
        [
            "MAB305",
            "Tasarım Tek. III",
            6,
            3,
            1,
            0
        ],
        [
            "MAB353",
            "Sonlu Elemanlar Yöntemi",
            6,
            3,
            1,
            0
        ],
        [
            "MAB001",
            "Sürekli Ortamlar Mekaniği",
            6,
            2,
            2,
            0
        ],
        [
            "MAB413",
            "Makine Müh. Bilişim Tek.",
            6,
            3,
            2,
            0
        ],
        [
            "MAB392",
            "Proje: Tasarım",
            6,
            1,
            0,
            0
        ],
        [
            "MAB382",
            "Tasarım: Seçilmiş Konular I",
            6,
            3,
            2,
            0
        ],
        [
            "MAB425",
            "Metodik Tasarım",
            6,
            3,
            1,
            0
        ],
        [
            "MAB426",
            "Yorulma Dayanımı",
            6,
            3,
            2,
            0
        ],
        [
            "MAB002",
            "Mekanikte Enerji Yöntemleri",
            6,
            3,
            2,
            0
        ],
        [
            "MAB401",
            "Katı Cisim Simulasyonu",
            6,
            3,
            1,
            0
        ],
        [
            "MAB351",
            "Yapısal Mekanik",
            6,
            3,
            1,
            0
        ],
        [
            "ETE331",
            "Elektrik Makineleri I",
            6,
            3,
            1,
            0
        ],
        [
            "MAB304",
            "Dişli Teorisi ve Mekanizmalar",
            6,
            3,
            1,
            0
        ],
        [
            "MAB462",
            "Tasarım: Seçilmiş Konular II",
            6,
            3,
            1,
            0
        ],
        [
            "MAB466",
            "Tasarım: Seçilmiş Konular III",
            6,
            3,
            2,
            0
        ]
    ],
    "SDUc": [
        [
            "MAB456",
            "Akışkan Makineleri",
            6,
            3,
            1,
            0
        ],
        [
            "MAB355",
            "Hesaplamalı Akışkanlar Dinamiği I",
            6,
            3,
            1,
            0
        ],
        [
            "MAB455",
            "Havacılık Endüstrisine Giriş",
            6,
            3,
            1,
            0
        ],
        [
            "MAB393",
            "Proje: Uzay Havacılık",
            6,
            1,
            0,
            0
        ],
        [
            "MAB383",
            "Uzay Havacılık: Seç. Konular I",
            6,
            3,
            2,
            0
        ],
        [
            "INF501",
            "Akıllı Sistemler",
            6,
            2,
            2,
            0
        ],
        [
            "INF502",
            "Makine Öğrenmesi",
            6,
            2,
            2,
            0
        ],
        [
            "MAB351",
            "Yapısal Mekanik",
            6,
            3,
            1,
            0
        ],
        [
            "MAB305",
            "Tasarım Tek. III",
            6,
            3,
            1,
            0
        ],
        [
            "MAB353",
            "Sonlu Elemanlar Yöntemi",
            6,
            3,
            1,
            0
        ],
        [
            "MAB407",
            "Aerodinamik",
            6,
            3,
            2,
            0
        ],
        [
            "MAB001",
            "Sürekli Ortamlar Mekaniği",
            6,
            2,
            2,
            0
        ],
        [
            "MAB406",
            "Uzay Teknolojisinin Temelleri",
            6,
            3,
            2,
            0
        ],
        [
            "MAB417",
            "Hesaplamalı Akışkanlar Dinamiği II",
            6,
            3,
            1,
            0
        ],
        [
            "MAB415",
            "Hesaplamalı Akışkanlar Dinamiği III",
            6,
            3,
            1,
            0
        ],
        [
            "MAB463",
            "Uzay Havacılık: Seç. Konular II",
            6,
            3,
            2,
            0
        ],
        [
            "MAB467",
            "Uzay Havacılık: Seç. Konular III",
            6,
            3,
            2,
            0
        ]
    ],
    "SDUd": [
        [
            "MAB354",
            "Araç Teknolojisinin Temelleri",
            6,
            3,
            1,
            0
        ],
        [
            "MAB304",
            "Dişli Teorisi ve Mekanizmalar",
            6,
            3,
            2,
            0
        ],
        [
            "MAB401",
            "Katı Cisim Simulasyonu",
            6,
            3,
            1,
            0
        ],
        [
            "MAB394",
            "Proje: Taşıt Sistemleri",
            6,
            1,
            0,
            0
        ],
        [
            "MAB384",
            "Taşıt Sist.: Seç. Konular I",
            6,
            3,
            2,
            0
        ],
        [
            "ETE331",
            "Elektrik Makineleri I",
            6,
            3,
            1,
            0
        ],
        [
            "MAB405",
            "Motor Teknolojilerinin Temelleri",
            6,
            3,
            1,
            0
        ],
        [
            "MAB305",
            "Tasarım Tek. III",
            6,
            3,
            1,
            0
        ],
        [
            "MAB353",
            "Sonlu Elemanlar Yöntemi",
            6,
            3,
            1,
            0
        ],
        [
            "MAB310",
            "Malzeme Teknolojisi II",
            6,
            3,
            1,
            0
        ],
        [
            "MAB421",
            "Raylı Taşıt Teknolojisi",
            6,
            3,
            1,
            0
        ],
        [
            "MAB422",
            "Raylı Sistemlerin Temelleri",
            6,
            3,
            1,
            0
        ],
        [
            "MAB464",
            "Taşıt Sist.: Seç. Konular II",
            6,
            3,
            2,
            0
        ],
        [
            "MAB356",
            "Araç Tasarımının Temelleri",
            6,
            3,
            2,
            0
        ],
        [
            "MAB357",
            "Ticari Araç Mühendisliği",
            6,
            3,
            2,
            0
        ],
        [
            "MAB468",
            "Taşıt Sist.: Seç. Konular III",
            6,
            3,
            2,
            0
        ]
    ],
    "SDUe": [
        [
            "MAB490",
            "İşletmede Mesleki Eğitim",
            18,
            0,
            0,
            0
        ]
    ]
},
        "pool_metadata": {
    "SDK": {},
    "SDP": {},
    "SDUx": {},
    "SDUa": {},
    "SDUb": {},
    "SDUc": {},
    "SDUd": {},
    "SDUe": {}
},
    },
    "Malzeme Bilimi ve Teknolojileri": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "PHY111",
            "Fizik I",
            6,
            2,
            1,
            2
        ],
        [
            "CHE111",
            "Kimya I",
            6,
            2,
            1,
            2
        ],
        [
            "BIO111",
            "Biyoloji",
            6,
            2,
            1,
            2
        ],
        [
            "MAT103",
            "Analiz I",
            6,
            3,
            2,
            0
        ],
        [
            "MWT101",
            "Malzeme Bilimi: Kariyere Giriş ve Etik",
            2,
            2,
            0,
            0
        ],
        [
            "DEU121",
            "Teknik Almanca I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG101",
            "İngilizce I",
            2,
            3,
            0,
            0
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "PHY112",
            "Fizik II",
            6,
            2,
            1,
            2
        ],
        [
            "CHE112",
            "Kimya II",
            6,
            2,
            1,
            2
        ],
        [
            "MAT112",
            "Analiz II ve Lineer Cebir",
            6,
            3,
            2,
            0
        ],
        [
            "NWI102",
            "Programlamaya Giriş",
            6,
            2,
            0,
            2
        ],
        [
            "NWI106",
            "Proje Yönetimi",
            2,
            2,
            0,
            0
        ],
        [
            "DEU122",
            "Teknik Almanca II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG102",
            "İngilizce II",
            2,
            3,
            0,
            0
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "NWI201",
            "Fiziksel Kimya I",
            6,
            2,
            1,
            2
        ],
        [
            "MWT205",
            "Malzeme Biliminin Temelleri",
            6,
            3,
            1,
            0
        ],
        [
            "TUR001",
            "Türkçe I",
            2,
            2,
            0,
            0
        ],
        [
            "AIT001",
            "Atatürk İlkeleri ve İnkılap Tarihi I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG201",
            "İngilizce III",
            2,
            3,
            0,
            0
        ],
        [
            "ZSD I",
            "Zorunlu Seçmeli Ders I {SECIM:2}",
            6,
            0,
            0,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "NWI202",
            "Fiziksel Kimya II",
            6,
            3,
            1,
            1
        ],
        [
            "NWI204",
            "Ölçüm Teknikleri",
            6,
            2,
            1,
            1
        ],
        [
            "MWT202",
            "Katıların Termodinamiği",
            6,
            2,
            2,
            1
        ],
        [
            "TUR002",
            "Türkçe II",
            2,
            2,
            0,
            0
        ],
        [
            "AIT002",
            "Atatürk İlkeleri ve İnkılap Tarihi II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG202",
            "İngilizce IV",
            2,
            3,
            0,
            0
        ],
        [
            "ZSD II",
            "Zorunlu Seçmeli Ders II",
            6,
            0,
            0,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "MAT201",
            "Diferansiyel Denklemler",
            6,
            2,
            2,
            1
        ],
        [
            "MWT301",
            "Reel Kristaller ve Özellikleri",
            6,
            3,
            1,
            1
        ],
        [
            "MWT303",
            "Teknik Mekanik",
            6,
            3,
            1,
            1
        ],
        [
            "MWT305",
            "Malzeme Biliminde Karakterizasyon Metotları",
            6,
            3,
            1,
            0
        ],
        [
            "ZSD III",
            "Zorunlu Seçmeli Ders III",
            6,
            0,
            0,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "NWI302",
            "İstatistiksel ve Sayısal Yöntemler",
            6,
            2,
            2,
            0
        ],
        [
            "MWT302",
            "Malzeme Üretim ve İşleme Teknolojileri",
            6,
            2,
            2,
            1
        ],
        [
            "MWT304",
            "Malzemelerin Mekanik Özellikleri",
            6,
            3,
            1,
            1
        ],
        [
            "ZSD IV",
            "Zorunlu Seçmeli Ders IV {SECIM:2}",
            6,
            0,
            0,
            0
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "MWT401",
            "Proje I",
            6,
            1,
            0,
            4
        ],
        [
            "NWI401",
            "Bilimsel Çalışma Yöntemleri",
            2,
            2,
            0,
            0
        ],
        [
            "ISG001",
            "İş Sağlığı ve İş Güvenliği I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG301",
            "İleri İngilizce I",
            2,
            3,
            0,
            0
        ],
        [
            "ZSD V",
            "Zorunlu Seçmeli Ders V {SECIM:3}",
            6,
            0,
            0,
            0
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "MWT402",
            "Proje II",
            12,
            1,
            0,
            7
        ],
        [
            "MWT404",
            "Seminer",
            4,
            2,
            0,
            0
        ],
        [
            "PRK400",
            "Endüstri Stajı Semineri",
            4,
            2,
            0,
            0
        ],
        [
            "ISG002",
            "İş Sağlığı ve İş Güvenliği II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG302",
            "İleri İngilizce II",
            2,
            3,
            0,
            0
        ],
        [
            "ÜSD",
            "Üniversite Seçmeli Dersi",
            3,
            0,
            0,
            0
        ],
        [
            "ZSD VI",
            "Zorunlu Sosyal Seçmeli Ders VI",
            3,
            0,
            0,
            0
        ]
    ]
},
        "pool_codes": {
    "ZSD I": [],
    "ZSDI": [
        "Sayısal Tasarım",
        "Statik",
        "Malzeme Kimyası",
        "Malzeme Yapısı",
        "Tasarım Teknikleri I: Teknik Çizim ve CAD"
    ],
    "ZSDII": [
        "Elektrotekniğe Giriş",
        "Yoğun Maddenin Fonksiyonel Özellikleri"
    ],
    "ZSDIII": [
        "Malzeme Teknolojisi",
        "Malzeme Muayenesi"
    ],
    "ZSDIV": [
        "Yapı Malzemeleri",
        "Toz Metalurjisi",
        "Biyomalzemeler",
        "Kompozit Malzemeler",
        "Malzemelerin İleri Karakterizasyon Teknikleri"
    ],
    "ZSDV": [
        "Polimerik Malzemeler",
        "Metalik Malzemeler",
        "Malzeme Muayenesi",
        "Reel Kristallerde Difüzyon ve Taşınım",
        "Fonksiyonel Malzemeler",
        "Yoğun Madde Fiziği",
        "Yenilenebilir Enerji Teknolojileri"
    ],
    "ZSDVI": [
        "Temel Hukuk"
    ]
},
        "pools": {
    "ZSD I": [],
    "ZSDI": [
        [
            "ETE101",
            "Sayısal Tasarım",
            6,
            2,
            1,
            1
        ],
        [
            "MEC109",
            "Statik",
            6,
            3,
            2,
            0
        ],
        [
            "MWT203",
            "Malzeme Kimyası",
            6,
            2,
            1,
            1
        ],
        [
            "MWT201",
            "Malzeme Yapısı",
            6,
            2,
            2,
            1
        ],
        [
            "NWI203",
            "Tasarım Teknikleri I: Teknik Çizim ve CAD",
            6,
            1,
            2,
            1
        ]
    ],
    "ZSDII": [
        [
            "NWI206",
            "Elektrotekniğe Giriş",
            6,
            2,
            1,
            2
        ],
        [
            "MWT406",
            "Yoğun Maddenin Fonksiyonel Özellikleri",
            6,
            2,
            1,
            1
        ]
    ],
    "ZSDIII": [
        [
            "MEC207",
            "Malzeme Teknolojisi",
            6,
            3,
            1,
            1
        ],
        [
            "MWT311",
            "Malzeme Muayenesi",
            6,
            2,
            1,
            2
        ]
    ],
    "ZSDIV": [
        [
            "MWT306",
            "Yapı Malzemeleri",
            6,
            2,
            1,
            1
        ],
        [
            "MWT308",
            "Toz Metalurjisi",
            6,
            2,
            1,
            1
        ],
        [
            "MWT310",
            "Biyomalzemeler",
            6,
            3,
            0,
            2
        ],
        [
            "MWT312",
            "Kompozit Malzemeler",
            6,
            2,
            1,
            1
        ],
        [
            "MWT408",
            "Malzemelerin İleri Karakterizasyon Teknikleri",
            6,
            2,
            1,
            1
        ]
    ],
    "ZSDV": [
        [
            "MWT307",
            "Polimerik Malzemeler",
            6,
            2,
            0,
            2
        ],
        [
            "MWT309",
            "Metalik Malzemeler",
            6,
            2,
            1,
            1
        ],
        [
            "MWT311",
            "Malzeme Muayenesi",
            6,
            2,
            1,
            2
        ],
        [
            "MWT403",
            "Reel Kristallerde Difüzyon ve Taşınım",
            6,
            2,
            1,
            1
        ],
        [
            "MWT405",
            "Fonksiyonel Malzemeler",
            6,
            2,
            1,
            1
        ],
        [
            "NWI405",
            "Yoğun Madde Fiziği",
            6,
            3,
            1,
            0
        ],
        [
            "NWI407",
            "Yenilenebilir Enerji Teknolojileri",
            6,
            2,
            1,
            1
        ]
    ],
    "ZSDVI": [
        [
            "NWI300",
            "Temel Hukuk",
            3,
            3,
            0,
            0
        ]
    ]
},
        "pool_metadata": {
    "ZSD I": {},
    "ZSDI": {},
    "ZSDII": {},
    "ZSDIII": {},
    "ZSDIV": {},
    "ZSDV": {},
    "ZSDVI": {}
},
    },
    "Mekatronik Müh": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "MAT103",
            "Analiz I",
            6,
            3,
            2,
            0
        ],
        [
            "MEC107",
            "Tasarım Tek. I",
            6,
            2,
            0,
            4
        ],
        [
            "MEC109",
            "Statik",
            6,
            3,
            2,
            0
        ],
        [
            "MEC105",
            "Bilg. Bil. Giris",
            6,
            2,
            0,
            2
        ],
        [
            "MEC091",
            "Mühendisliğe Giris",
            2,
            2,
            0,
            0
        ],
        [
            "DEU121",
            "Teknik Almanca I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG101",
            "İngilizce I",
            2,
            3,
            0,
            0
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "MAT108",
            "Analiz II",
            6,
            3,
            2,
            0
        ],
        [
            "PHY102",
            "Elektrik ve Manyetizma",
            6,
            3,
            1,
            1
        ],
        [
            "MEC112",
            "Mukavemet",
            6,
            3,
            2,
            0
        ],
        [
            "MAT106",
            "Lineer Cebir",
            6,
            2,
            2,
            1
        ],
        [
            "MEC092",
            "Bilimsel Yöntemler",
            2,
            2,
            0,
            0
        ],
        [
            "DEU122",
            "Teknik Almanca II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG102",
            "İngilizce II",
            2,
            3,
            0,
            0
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "MAT201",
            "Diferansiyel Denklemler",
            6,
            2,
            1,
            1
        ],
        [
            "MEC213",
            "Elektrik Devreleri I",
            6,
            3,
            1,
            2
        ],
        [
            "MEC209",
            "Kinematik ve Dinamik",
            6,
            3,
            1,
            1
        ],
        [
            "MEC207",
            "Malzeme Teknolojisi I",
            6,
            3,
            1,
            1
        ],
        [
            "MEC299",
            "Temel Staj",
            2,
            0,
            0,
            0
        ],
        [
            "TUR001",
            "Türkçe I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG201",
            "İngilizce III",
            2,
            3,
            0,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "MAT204",
            "İstatistik",
            6,
            2,
            2,
            1
        ],
        [
            "MEC214",
            "Elektrik Devreleri II",
            6,
            3,
            1,
            2
        ],
        [
            "MEC208",
            "Ölçüm Teknikleri",
            6,
            2,
            1,
            2
        ],
        [
            "MEC218",
            "Nesnel Programlama",
            6,
            2,
            0,
            2
        ],
        [
            "MEC292",
            "Project Management",
            2,
            1,
            1,
            0
        ],
        [
            "TUR002",
            "Türkçe II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG202",
            "İngilizce IV",
            2,
            3,
            0,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "MEC313",
            "End. Otomasyon Tek.",
            6,
            3,
            1,
            1
        ],
        [
            "MEC311",
            "Sinyaller ve Sistemler",
            6,
            3,
            1,
            1
        ],
        [
            "MEC317",
            "Algoritma ve Veri Y.",
            6,
            2,
            0,
            2
        ],
        [
            "MEC319",
            "Mechatronics Project",
            6,
            1,
            0,
            4
        ],
        [
            "ZSD I",
            "Zorunlu Seçmeli Ders I",
            6,
            0,
            0,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "MEC308",
            "Endüstriyel Robotik I",
            6,
            3,
            1,
            1
        ],
        [
            "MEC302",
            "Kontrol Müh. Temelleri",
            6,
            3,
            1,
            1
        ],
        [
            "SDP I",
            "Elective Project I",
            6,
            0,
            0,
            0
        ],
        [
            "ZSD II",
            "Zorunlu Seçmeli Ders II",
            6,
            0,
            0,
            0
        ],
        [
            "USD000",
            "Üniversite Seçmeli",
            6,
            0,
            0,
            0
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "MEC499",
            "Mesleki Alan Stajı",
            6,
            0,
            0,
            0
        ],
        [
            "ISG001",
            "İş Sağlığı ve Güvenliği I",
            2,
            2,
            0,
            0
        ],
        [
            "AIT001",
            "Atatürk İlkeleri I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG301",
            "İleri İngilizce I",
            2,
            3,
            0,
            0
        ],
        [
            "SDP II",
            "Elective Project II",
            6,
            0,
            0,
            0
        ],
        [
            "ZSD III",
            "Zorunlu Seçmeli Ders III",
            12,
            0,
            0,
            0
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "MEC492",
            "Bitirme Tezi",
            12,
            0,
            0,
            0
        ],
        [
            "ISG002",
            "İş Sağlığı ve Güv. II",
            2,
            2,
            0,
            0
        ],
        [
            "AIT002",
            "Atatürk İlkeleri II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG302",
            "İleri İngilizce II",
            2,
            3,
            0,
            0
        ],
        [
            "ZSD IV",
            "Zorunlu Seçmeli Ders IV",
            12,
            0,
            0,
            0
        ]
    ]
},
        "pool_codes": {
    "SDP I": [
        "Robotics Project I",
        "Production Auto. Project I",
        "Intelligent Sys. Project I"
    ],
    "SDP II": [
        "Robotics Project II",
        "Production Auto. Project II",
        "Intelligent Sys. Project II"
    ],
    "ZSD": [
        "Image-Based Auto. I",
        "Uygulamalı Kontrol Müh.",
        "Image-Based Auto. II",
        "Electronic Components",
        "Manufacturing Tech. I",
        "Industrial Robotics II",
        "Veritabanı Sistemleri",
        "Machine Tools",
        "Thermodynamics I",
        "Software Engineering",
        "Sys. ID & Intelligent Ctrl",
        "Bilg.-Makine Etkileşimi",
        "Electrical Machines I",
        "Microprocessors",
        "Fabrika Yönetimine Giriş",
        "Makine Öğrenmesi",
        "Sayısal Tasarım",
        "Electronics I",
        "CS Selected Topics I",
        "CS Selected Topics II",
        "EE Selected Topics I",
        "EE Selected Topics II",
        "Makine Müh. Seçilmiş K. I",
        "Makine Müh. Seçilmiş K. II",
        "Embedded Systems",
        "Mechatronics Topics II",
        "Power Electronics",
        "Materials Technology II",
        "Ind. Info Tech",
        "Quality Management",
        "Montaj Teknolojisi Temelleri",
        "Fluid Mechanics",
        "Heat Transfer",
        "FPGA Programming",
        "VLSI Tasarımına Giriş",
        "Intelligent Systems",
        "Neural Networks",
        "Deep Learning",
        "Artificial Intelligence"
    ]
},
        "pools": {
    "SDP I": [
        [
            "MEC423",
            "Robotics Project I",
            6,
            1,
            0,
            4
        ],
        [
            "MEC425",
            "Production Auto. Project I",
            6,
            1,
            0,
            4
        ],
        [
            "MEC427",
            "Intelligent Sys. Project I",
            6,
            1,
            0,
            4
        ]
    ],
    "SDP II": [
        [
            "MEC424",
            "Robotics Project II",
            6,
            1,
            0,
            4
        ],
        [
            "MEC426",
            "Production Auto. Project II",
            6,
            1,
            0,
            4
        ],
        [
            "MEC428",
            "Intelligent Sys. Project II",
            6,
            1,
            0,
            4
        ]
    ],
    "ZSD": [
        [
            "MEC321",
            "Image-Based Auto. I",
            6,
            2,
            1,
            1
        ],
        [
            "MEC002",
            "Uygulamalı Kontrol Müh.",
            6,
            1,
            1,
            3
        ],
        [
            "MEC324",
            "Image-Based Auto. II",
            6,
            2,
            1,
            1
        ],
        [
            "MEC312",
            "Electronic Components",
            6,
            2,
            1,
            1
        ],
        [
            "MAB311",
            "Manufacturing Tech. I",
            6,
            3,
            1,
            1
        ],
        [
            "MEC421",
            "Industrial Robotics II",
            6,
            2,
            1,
            2
        ],
        [
            "INF205",
            "Veritabanı Sistemleri",
            6,
            2,
            0,
            2
        ],
        [
            "MAB301",
            "Machine Tools",
            6,
            2,
            1,
            2
        ],
        [
            "MAB202",
            "Thermodynamics I",
            6,
            3,
            2,
            0
        ],
        [
            "INF202",
            "Software Engineering",
            6,
            1,
            0,
            3
        ],
        [
            "ETE456",
            "Sys. ID & Intelligent Ctrl",
            6,
            2,
            1,
            2
        ],
        [
            "INF523",
            "Bilg.-Makine Etkileşimi",
            6,
            2,
            0,
            2
        ],
        [
            "ETE331",
            "Electrical Machines I",
            6,
            3,
            1,
            1
        ],
        [
            "ETE104",
            "Microprocessors",
            6,
            3,
            1,
            1
        ],
        [
            "WIN302",
            "Fabrika Yönetimine Giriş",
            6,
            2,
            1,
            1
        ],
        [
            "INF502",
            "Makine Öğrenmesi",
            6,
            2,
            0,
            2
        ],
        [
            "ETE101",
            "Sayısal Tasarım",
            6,
            2,
            1,
            1
        ],
        [
            "ETE311",
            "Electronics I",
            6,
            2,
            2,
            1
        ],
        [
            "MEC030",
            "CS Selected Topics I",
            6,
            2,
            0,
            2
        ],
        [
            "MEC031",
            "CS Selected Topics II",
            3,
            1,
            0,
            2
        ],
        [
            "MEC032",
            "EE Selected Topics I",
            6,
            2,
            0,
            2
        ],
        [
            "MEC033",
            "EE Selected Topics II",
            3,
            1,
            0,
            2
        ],
        [
            "MEC034",
            "Makine Müh. Seçilmiş K. I",
            6,
            2,
            0,
            2
        ],
        [
            "MEC035",
            "Makine Müh. Seçilmiş K. II",
            3,
            1,
            0,
            2
        ],
        [
            "MEC036",
            "Embedded Systems",
            6,
            2,
            0,
            2
        ],
        [
            "MEC037",
            "Mechatronics Topics II",
            3,
            1,
            0,
            2
        ],
        [
            "ETE439",
            "Power Electronics",
            6,
            2,
            2,
            1
        ],
        [
            "MAB310",
            "Materials Technology II",
            6,
            3,
            1,
            1
        ],
        [
            "WIN406",
            "Ind. Info Tech",
            6,
            2,
            1,
            1
        ],
        [
            "WIN314",
            "Quality Management",
            6,
            3,
            1,
            1
        ],
        [
            "WIN408",
            "Montaj Teknolojisi Temelleri",
            6,
            2,
            2,
            0
        ],
        [
            "MAB309",
            "Fluid Mechanics",
            6,
            3,
            1,
            1
        ],
        [
            "MAB312",
            "Heat Transfer",
            6,
            3,
            1,
            1
        ],
        [
            "ETE441",
            "FPGA Programming",
            6,
            3,
            1,
            1
        ],
        [
            "ETE448",
            "VLSI Tasarımına Giriş",
            6,
            3,
            1,
            1
        ],
        [
            "INF501",
            "Intelligent Systems",
            6,
            2,
            0,
            2
        ],
        [
            "INF503",
            "Neural Networks",
            6,
            2,
            0,
            2
        ],
        [
            "INF513",
            "Deep Learning",
            6,
            2,
            0,
            2
        ],
        [
            "INF701",
            "Artificial Intelligence",
            6,
            2,
            0,
            2
        ]
    ]
},
        "pool_metadata": {
    "SDP I": {},
    "SDP II": {},
    "ZSD": {}
},
    },
    "Moleküler Biyoteknoloji": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "BIO111",
            "Biyoloji",
            6,
            2,
            1,
            2
        ],
        [
            "CHE111",
            "Kimya I",
            6,
            2,
            1,
            2
        ],
        [
            "DEU121",
            "Teknik Almanca I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG101",
            "İngilizce I",
            2,
            3,
            0,
            0
        ],
        [
            "MAT103",
            "Analiz I",
            6,
            3,
            2,
            0
        ],
        [
            "MBT101",
            "Moleküler Biyoteknoloji'ye Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "PHY111",
            "Fizik I",
            6,
            2,
            1,
            2
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "CHE112",
            "Kimya II",
            6,
            2,
            1,
            2
        ],
        [
            "DEU122",
            "Teknik Almanca II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG102",
            "İngilizce II",
            2,
            3,
            0,
            0
        ],
        [
            "MAT112",
            "Analiz II ve Lineer Cebir",
            6,
            3,
            2,
            0
        ],
        [
            "NWI102",
            "Programlamaya Giriş",
            6,
            2,
            0,
            2
        ],
        [
            "NWI106",
            "Proje Yönetimi",
            2,
            2,
            0,
            0
        ],
        [
            "PHY112",
            "Fizik II",
            6,
            2,
            1,
            2
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "AIT001",
            "Atatürk İlkeleri ve İnkılap Tarihi I",
            2,
            2,
            0,
            0
        ],
        [
            "ENG201",
            "İngilizce III",
            2,
            3,
            0,
            0
        ],
        [
            "MAT201",
            "Diferansiyel Denklemler",
            6,
            3,
            2,
            0
        ],
        [
            "MBT201",
            "Hücre Biyolojisi",
            6,
            3,
            1,
            1
        ],
        [
            "MBT211",
            "Biyokimya I",
            6,
            2,
            1,
            2
        ],
        [
            "NWI201",
            "Fiziksel Kimya I",
            6,
            3,
            1,
            1
        ],
        [
            "TUR001",
            "Türkçe I",
            2,
            2,
            0,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "AIT002",
            "Atatürk İlkeleri ve İnkılap Tarihi II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG202",
            "İngilizce IV",
            2,
            3,
            0,
            0
        ],
        [
            "MBT204",
            "Mikrobiyoloji I",
            6,
            2,
            1,
            2
        ],
        [
            "MBT206",
            "Biyoenformatik",
            6,
            2,
            0,
            2
        ],
        [
            "MBT212",
            "Biyokimya II",
            6,
            3,
            0,
            2
        ],
        [
            "MBT222",
            "Moleküler Biyoteknoloji I",
            6,
            2,
            1,
            2
        ],
        [
            "TUR002",
            "Türkçe II",
            2,
            2,
            0,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "ENG301",
            "İleri İngilizce I",
            2,
            3,
            0,
            0
        ],
        [
            "ISG001",
            "İş Sağlığı ve Güvenliği I",
            2,
            2,
            0,
            0
        ],
        [
            "MBT323",
            "Moleküler Biyoteknoloji II",
            6,
            2,
            1,
            2
        ],
        [
            "SDBIO",
            "Seçmeli Dersler - Bio I",
            18,
            0,
            0,
            0
        ],
        [
            "ÜSD",
            "Üniversite Seçmeli Ders Havuzu",
            2,
            0,
            0,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "ENG302",
            "İleri İngilizce II",
            2,
            3,
            0,
            0
        ],
        [
            "ISG002",
            "İş Sağlığı ve İş Güvenliği II",
            2,
            2,
            0,
            0
        ],
        [
            "MBT324",
            "Moleküler Biyoteknoloji III",
            5,
            2,
            0,
            2
        ],
        [
            "MBT332",
            "Biyoteknoloji İşletmeleri Sistem Teknikleri I",
            6,
            2,
            1,
            0
        ],
        [
            "MBT364",
            "Biyoanalitik",
            6,
            3,
            0,
            2
        ],
        [
            "NWI300",
            "Temel Hukuk",
            3,
            3,
            0,
            0
        ],
        [
            "SDBIOII",
            "Seçmeli Dersler - Bio II",
            6,
            0,
            0,
            0
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "MBT441",
            "Proje I",
            6,
            0,
            4,
            0
        ],
        [
            "PRK401",
            "Staj",
            6,
            0,
            0,
            0
        ],
        [
            "SDBIOIII",
            "Seçmeli Dersler - Bio III",
            12,
            0,
            0,
            0
        ],
        [
            "SDMATI",
            "Seçmeli Dersler - Mat I",
            6,
            0,
            0,
            0
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "MBT442",
            "Proje II",
            12,
            0,
            0,
            6
        ],
        [
            "SDBIOIV",
            "Seçmeli Dersler - Bio IV",
            12,
            0,
            0,
            0
        ],
        [
            "SDMATII",
            "Seçmeli Dersler - Mat II",
            6,
            0,
            0,
            0
        ]
    ]
},
        "pool_codes": {
    "SDBIO": [],
    "SDBIOII": [
        "Biyofiziksel Kimya",
        "Biyoelektrokimya",
        "Biyokataliz",
        "Fiziksel Kimya II"
    ],
    "SDBIOIII": [
        "Biyoteknoloji İşletmeleri için Sistem Teknikleri II",
        "Hücre-Malzeme Etkileşimleri",
        "Doku Mühendisliği",
        "Biyosensörler",
        "Nanobiyoteknoloji",
        "Hedeflendirilmiş İlaç Taşıma"
    ],
    "SDBIOIV": [
        "Popülasyon Genetiği",
        "Etken Madde Araştırmaları",
        "Doğal Madde Üretimi",
        "Egzersizin Moleküler Biyolojisi",
        "Biyomalzemeler"
    ],
    "SDMAT": [],
    "SDMATI": [
        "Biyomatematik",
        "Biyofizik"
    ],
    "SDMATII": [
        "Nicel Biyoloji",
        "Biyoistatistik",
        "Duyu Organlarının Biyofiziği",
        "Biyomedikal İşaret işleme ve Yapay Zeka Uygulamaları",
        "İstatistiksel ve Sayısal Yöntemler",
        "Biyobilimler için MATLAB"
    ],
    "ÜSD": [
        "Mikrobiyoloji II",
        "İmmünoloji",
        "Ekoloji",
        "Evrim Biyolojisi",
        "Biyobilimler için Organik Kimya",
        "İnşaat Mühendisliğine Giriş",
        "Pazarlamanın Temelleri",
        "Stratejik Pazarlama",
        "Elektrik-Elektronik Mühendisliğine Giriş",
        "Sayısal Tasarım",
        "Nesnel Programlama",
        "Makine Mühendisliğine Giriş",
        "Statik",
        "Malzeme Teknolojisi I",
        "İktisadi Tarih",
        "Endüstri Mühendisliğine Giriş",
        "Muhasebe ve Bilanço"
    ]
},
        "pools": {
    "SDBIO": [],
    "SDMAT": [],
    "ÜSD": [
        [
            "MBT361",
            "Mikrobiyoloji II",
            6,
            3,
            0,
            2
        ],
        [
            "MBT363",
            "İmmünoloji",
            6,
            3,
            2,
            0
        ],
        [
            "MBT365",
            "Ekoloji",
            6,
            3,
            0,
            2
        ],
        [
            "MBT367",
            "Evrim Biyolojisi",
            6,
            3,
            0,
            2
        ],
        [
            "NWI301",
            "Biyobilimler için Organik Kimya",
            6,
            3,
            3,
            0
        ],
        [
            "BAU091",
            "İnşaat Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "BWL201",
            "Pazarlamanın Temelleri",
            5,
            2,
            2,
            0
        ],
        [
            "BWL212",
            "Stratejik Pazarlama",
            5,
            2,
            2,
            0
        ],
        [
            "ETE091",
            "Elektrik-Elektronik Mühendisliğine Giriş",
            2,
            2,
            1,
            0
        ],
        [
            "ETE101",
            "Sayısal Tasarım",
            6,
            2,
            1,
            1
        ],
        [
            "INF102",
            "Nesnel Programlama",
            6,
            2,
            0,
            2
        ],
        [
            "MAB091",
            "Makine Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "MAB109",
            "Statik",
            6,
            3,
            2,
            0
        ],
        [
            "MAB207",
            "Malzeme Teknolojisi I",
            6,
            3,
            2,
            0
        ],
        [
            "VWL203",
            "İktisadi Tarih",
            6,
            3,
            0,
            0
        ],
        [
            "WIN091",
            "Endüstri Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "WIN204",
            "Muhasebe ve Bilanço",
            6,
            2,
            2,
            1
        ]
    ],
    "SDBIOII": [
        [
            "MBT202",
            "Biyofiziksel Kimya",
            6,
            3,
            1,
            0
        ],
        [
            "MBT326",
            "Biyoelektrokimya",
            6,
            2,
            0,
            2
        ],
        [
            "MBT336",
            "Biyokataliz",
            6,
            2,
            2,
            0
        ],
        [
            "NWI202",
            "Fiziksel Kimya II",
            6,
            3,
            1,
            1
        ]
    ],
    "SDBIOIII": [
        [
            "MBT433",
            "Biyoteknoloji İşletmeleri için Sistem Teknikleri II",
            6,
            3,
            0,
            2
        ],
        [
            "MBT471",
            "Hücre-Malzeme Etkileşimleri",
            6,
            3,
            0,
            2
        ],
        [
            "MBT473",
            "Doku Mühendisliği",
            6,
            3,
            0,
            2
        ],
        [
            "MBT475",
            "Biyosensörler",
            6,
            3,
            0,
            2
        ],
        [
            "MBT477",
            "Nanobiyoteknoloji",
            6,
            3,
            0,
            2
        ],
        [
            "MBT479",
            "Hedeflendirilmiş İlaç Taşıma",
            6,
            3,
            0,
            2
        ]
    ],
    "SDBIOIV": [
        [
            "MBT474",
            "Popülasyon Genetiği",
            6,
            3,
            2,
            0
        ],
        [
            "MBT476",
            "Etken Madde Araştırmaları",
            6,
            3,
            0,
            2
        ],
        [
            "MBT478",
            "Doğal Madde Üretimi",
            6,
            3,
            0,
            2
        ],
        [
            "MBT480",
            "Egzersizin Moleküler Biyolojisi",
            6,
            3,
            2,
            0
        ],
        [
            "MWT310",
            "Biyomalzemeler",
            6,
            3,
            0,
            2
        ]
    ],
    "SDMATI": [
        [
            "MBT453",
            "Biyomatematik",
            6,
            2,
            2,
            0
        ],
        [
            "MBT455",
            "Biyofizik",
            6,
            2,
            2,
            0
        ]
    ],
    "SDMATII": [
        [
            "MBT456",
            "Nicel Biyoloji",
            6,
            2,
            2,
            0
        ],
        [
            "MBT458",
            "Biyoistatistik",
            6,
            2,
            2,
            0
        ],
        [
            "MBT460",
            "Duyu Organlarının Biyofiziği",
            6,
            2,
            2,
            0
        ],
        [
            "MBT488",
            "Biyomedikal İşaret işleme ve Yapay Zeka Uygulamaları",
            6,
            2,
            2,
            0
        ],
        [
            "NWI302",
            "İstatistiksel ve Sayısal Yöntemler",
            6,
            2,
            2,
            0
        ],
        [
            "NWI402",
            "Biyobilimler için MATLAB",
            6,
            2,
            0,
            2
        ]
    ]
},
        "pool_metadata": {
    "SDBIO": {},
    "SDMAT": {},
    "ÜSD": {},
    "SDBIOII": {},
    "SDBIOIII": {},
    "SDBIOIV": {},
    "SDMATI": {},
    "SDMATII": {}
},
    },
    "Siyaset Bilimi ve Uluslararası İlişkiler": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "AIT001",
            "Atatürk İlkeleri ve İnkılap Tarihi I",
            2,
            0,
            0,
            0
        ],
        [
            "DEU121",
            "Siyaset Bilimi için Almanca I",
            3,
            0,
            0,
            0
        ],
        [
            "ENG103",
            "İngilizce I",
            3,
            0,
            0,
            0
        ],
        [
            "POL101",
            "Siyaset Bilimine Giriş",
            5,
            0,
            0,
            0
        ],
        [
            "POL105",
            "Uluslararası İlişkiler Tarihi",
            5,
            0,
            0,
            0
        ],
        [
            "POL107",
            "Bilimsel Çalışmaya Giriş",
            5,
            0,
            0,
            0
        ],
        [
            "TUR001",
            "Türkçe I",
            2,
            0,
            0,
            0
        ],
        [
            "VWL191",
            "İktisada Giriş I",
            5,
            0,
            0,
            0
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "AIT002",
            "Atatürk İlkeleri ve İnkılap Tarihi II",
            2,
            0,
            0,
            0
        ],
        [
            "DEU122",
            "Siyaset Bilimi için Almanca II",
            3,
            0,
            0,
            0
        ],
        [
            "ENG104",
            "İngilizce II",
            3,
            0,
            0,
            0
        ],
        [
            "POL104",
            "Siyasal Düşünceler Tarihi",
            5,
            0,
            0,
            0
        ],
        [
            "POL106",
            "Uluslararası İlişkilere Giriş",
            5,
            0,
            0,
            0
        ],
        [
            "POL108",
            "Sosyolojiye Giriş",
            5,
            0,
            0,
            0
        ],
        [
            "TUR002",
            "Türkçe II",
            2,
            0,
            0,
            0
        ],
        [
            "VWL192",
            "İktisada Giriş II",
            5,
            0,
            0,
            0
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "ENG203",
            "İngilizce III",
            3,
            0,
            0,
            0
        ],
        [
            "HUK921",
            "Hukuka Giriş",
            4,
            0,
            0,
            0
        ],
        [
            "POL203",
            "Sosyal Bilimlerde Nitel Yöntemler",
            5,
            0,
            0,
            0
        ],
        [
            "POL205",
            "Uluslararası İlişkiler Teorileri",
            5,
            0,
            0,
            0
        ],
        [
            "POL211",
            "Karşılaştırmalı Siyaset Bilimi",
            5,
            0,
            0,
            0
        ],
        [
            "POLSDI",
            "Seçmeli Ders Alanı I",
            8,
            0,
            0,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "ENG204",
            "İngilizce IV",
            2,
            0,
            0,
            0
        ],
        [
            "POL204",
            "Sosyal Bilimlerde Nicel Yöntemler",
            5,
            0,
            0,
            0
        ],
        [
            "POL208",
            "Siyasal Teori",
            5,
            0,
            0,
            0
        ],
        [
            "POL212",
            "Türk Siyasal Hayatı",
            5,
            0,
            0,
            0
        ],
        [
            "POLSDII",
            "Seçmeli Ders Alanı II",
            13,
            0,
            0,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "ENG301",
            "İleri İngilizce I",
            2,
            0,
            0,
            0
        ],
        [
            "POL303",
            "Uluslararası Ekonomi Politik",
            5,
            0,
            0,
            0
        ],
        [
            "POL309",
            "Türkiye ve Avrupa İlişkilerinde Güncel Meseleler",
            3,
            0,
            0,
            0
        ],
        [
            "POL311",
            "Dış Politika Analizi",
            5,
            0,
            0,
            0
        ],
        [
            "POL313",
            "Türk Dış Politikası I",
            5,
            0,
            0,
            0
        ],
        [
            "POLSDIII",
            "Seçmeli Ders Alanı III",
            5,
            0,
            0,
            0
        ],
        [
            "ÜSD000",
            "Üniversite Seçmeli Ders Havuzu",
            5,
            0,
            0,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "ENG302",
            "İleri İngilizce II",
            2,
            0,
            0,
            0
        ],
        [
            "POL302",
            "Sosyal Bilimler İçin İstatistik",
            5,
            0,
            0,
            0
        ],
        [
            "POL304",
            "Avrupa Birliği'nin Siyasi ve İdari Yapısı",
            5,
            0,
            0,
            0
        ],
        [
            "POL306",
            "Almanya Siyasi Tarihi",
            5,
            0,
            0,
            0
        ],
        [
            "POL312",
            "Uluslararası Örgütler",
            5,
            0,
            0,
            0
        ],
        [
            "POL314",
            "Türk Dış Politikası II",
            5,
            0,
            0,
            0
        ],
        [
            "POL316",
            "Araştırma Semineri",
            3,
            0,
            0,
            0
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "POL403",
            "Alman Dış Politikası",
            5,
            0,
            0,
            0
        ],
        [
            "POL405",
            "Avrupa Entegrasyon Teorileri",
            5,
            0,
            0,
            0
        ],
        [
            "POLSDIV",
            "Seçmeli Ders Alanı IV",
            20,
            0,
            0,
            0
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "POL402",
            "Bitirme Ödevi",
            12,
            0,
            0,
            0
        ],
        [
            "POL404",
            "Bitirme Kolokyumu",
            5,
            0,
            0,
            0
        ],
        [
            "POL406",
            "Güncel Uluslararası Sorunlar",
            3,
            0,
            0,
            0
        ],
        [
            "POLSDV",
            "Seçmeli Ders Alanı V",
            10,
            0,
            0,
            0
        ]
    ]
},
        "pool_codes": {
    "POLSDI": [
        "Girişimcilik",
        "Dijital Pazarlama",
        "Örgütsel Davranış",
        "Kültürlerarası Yetkinlik Geliştirme",
        "Personel Yönetimi",
        "Uluslararası İşletme",
        "İşletmeye Giriş",
        "Pazarlamanın Temelleri",
        "Proje Yönetimi",
        "Stratejik Pazarlama",
        "E-İş",
        "İş Etiği",
        "Anayasa Hukuku II",
        "Anayasa Hukuku I",
        "Anayasa Hukukunun Temel Prensipleri",
        "Uluslararası Hukuk II",
        "İdare Hukuku I",
        "Uluslararası Hukuk I",
        "İdare Hukuku II",
        "Avrupa Hukuku",
        "Alman İdare Hukuku - Temel Kavramlar",
        "Devletler Özel Hukuku I",
        "Borçlar Hukuku",
        "İş Hukuku",
        "Ticaret Hukuku",
        "İletişim Bilimlerine Giriş",
        "Kültür Bilimlerine Giriş",
        "Kültür Kuramları",
        "Göç ve Kültür",
        "Kültür ve Küreselleşme",
        "Kültürlerarası İletişim",
        "Halkla İlişkiler",
        "Kültürel Bellek",
        "Çok Düzlemli Sistemlerde Politik Saha Analizi",
        "Karşılaştırmalı Parti ve Seçim Sistemleri",
        "Siyaset Sosyolojisi",
        "Kamu Yönetimine Giriş",
        "ABD Dış Politikası",
        "Latin Amerika Siyaseti",
        "Güncel Ortadoğu ve Kuzey Afrika Siyaseti",
        "Orta Asya ve Kafkasya Bölgesel Siyaseti",
        "Rus Siyaseti",
        "Dünya Siyasetinde Doğu Asya",
        "Dünya Siyasetinde Afrika",
        "Güneydoğu Avrupa Siyaseti",
        "Güncel Avrupa Siyaseti",
        "Karşılaştırmalı Avrupa Siyaseti",
        "AB Dış ve Güvenlik Siyaseti",
        "Almanya, Türkiye ve AB için Güncel Politik Sorunlar",
        "Türk-Alman İlişkileri: Ekonomi, Siyaset ve Toplum",
        "Sekülerleşme Teorileri",
        "Küreselleşme",
        "Irkçılık, Antisemitizm ve İslamofobi",
        "Toplumsal Cinsiyet ve Siyaset",
        "Siyaset Bilimi Stajı",
        "Türk Dış Politikasında Güncel Meseleler",
        "Avrupa'da Federalizm - Avrupa Federalizmi",
        "Siyaset Teorisinde Meseleler",
        "Değişen Küresel Sistemde Bölgesel Güçler",
        "Türk-Alman Diplomatik İlişkiler Tarihi",
        "Osmanlı Siyasi Tarihi II",
        "Türk Tarihi",
        "Siyaset Psikolojisi",
        "Türk Devletleri Teşkilatı ve Türk Devletleri Bütünleşmesi",
        "20. Yüzyıl Avrupa Siyasi Düşüncesi",
        "Göç, Diaspora ve Mülteciler",
        "Milliyetçilik Teorileri",
        "Dünya Siyaseti ve Sinema",
        "Uluslararası Enerji Siyaseti",
        "Küresel İklim Siyaseti",
        "Almanya ve Türkiye Karşılaştırmalı Siyasal Sistemler",
        "Kamu Yönetiminde Güncel Meseleler",
        "Yerel Yönetimler",
        "Kentleşme ve Çevre Sorunları",
        "Uluslararası İktisat I",
        "Uluslararası İktisat II",
        "İktisadi Düşünceler Tarihi",
        "Türkiye Ekonomisinin Gelişimi",
        "Dünya Ekonomisinin Güncel Sorunları",
        "Demografi ve Göç İktisadı",
        "Avrupa Bütünleşmesinin İktisadı"
    ],
    "POLSDV": [
        "Girişimcilik",
        "Dijital Pazarlama",
        "Örgütsel Davranış",
        "Kültürlerarası Yetkinlik Geliştirme",
        "Personel Yönetimi",
        "Uluslararası İşletme",
        "İşletmeye Giriş",
        "Pazarlamanın Temelleri",
        "Proje Yönetimi",
        "Stratejik Pazarlama",
        "E-İş",
        "İş Etiği",
        "Anayasa Hukuku II",
        "Anayasa Hukuku I",
        "Anayasa Hukukunun Temel Prensipleri",
        "Uluslararası Hukuk II",
        "İdare Hukuku I",
        "Uluslararası Hukuk I",
        "İdare Hukuku II",
        "Avrupa Hukuku",
        "Alman İdare Hukuku - Temel Kavramlar",
        "Devletler Özel Hukuku I",
        "Borçlar Hukuku",
        "İş Hukuku",
        "Ticaret Hukuku",
        "İletişim Bilimlerine Giriş",
        "Kültür Bilimlerine Giriş",
        "Kültür Kuramları",
        "Göç ve Kültür",
        "Kültür ve Küreselleşme",
        "Kültürlerarası İletişim",
        "Halkla İlişkiler",
        "Kültürel Bellek",
        "Çok Düzlemli Sistemlerde Politik Saha Analizi",
        "Karşılaştırmalı Parti ve Seçim Sistemleri",
        "Siyaset Sosyolojisi",
        "Kamu Yönetimine Giriş",
        "ABD Dış Politikası",
        "Latin Amerika Siyaseti",
        "Güncel Ortadoğu ve Kuzey Afrika Siyaseti",
        "Orta Asya ve Kafkasya Bölgesel Siyaseti",
        "Rus Siyaseti",
        "Dünya Siyasetinde Doğu Asya",
        "Dünya Siyasetinde Afrika",
        "Güneydoğu Avrupa Siyaseti",
        "Güncel Avrupa Siyaseti",
        "Karşılaştırmalı Avrupa Siyaseti",
        "AB Dış ve Güvenlik Siyaseti",
        "Almanya, Türkiye ve AB için Güncel Politik Sorunlar",
        "Türk-Alman İlişkileri: Ekonomi, Siyaset ve Toplum",
        "Sekülerleşme Teorileri",
        "Küreselleşme",
        "Irkçılık, Antisemitizm ve İslamofobi",
        "Toplumsal Cinsiyet ve Siyaset",
        "Siyaset Bilimi Stajı",
        "Türk Dış Politikasında Güncel Meseleler",
        "Avrupa'da Federalizm - Avrupa Federalizmi",
        "Siyaset Teorisinde Meseleler",
        "Değişen Küresel Sistemde Bölgesel Güçler",
        "Türk-Alman Diplomatik İlişkiler Tarihi",
        "Osmanlı Siyasi Tarihi II",
        "Türk Tarihi",
        "Siyaset Psikolojisi",
        "Türk Devletleri Teşkilatı ve Türk Devletleri Bütünleşmesi",
        "20. Yüzyıl Avrupa Siyasi Düşüncesi",
        "Göç, Diaspora ve Mülteciler",
        "Milliyetçilik Teorileri",
        "Dünya Siyaseti ve Sinema",
        "Uluslararası Enerji Siyaseti",
        "Küresel İklim Siyaseti",
        "Almanya ve Türkiye Karşılaştırmalı Siyasal Sistemler",
        "Kamu Yönetiminde Güncel Meseleler",
        "Yerel Yönetimler",
        "Kentleşme ve Çevre Sorunları",
        "Uluslararası İktisat I",
        "Uluslararası İktisat II",
        "İktisadi Düşünceler Tarihi",
        "Türkiye Ekonomisinin Gelişimi",
        "Dünya Ekonomisinin Güncel Sorunları",
        "Demografi ve Göç İktisadı",
        "Avrupa Bütünleşmesinin İktisadı"
    ]
},
        "pools": {
    "POLSDI": [
        [
            "BWL002",
            "Girişimcilik",
            6,
            2,
            2,
            0
        ],
        [
            "BWL007",
            "Dijital Pazarlama",
            6,
            2,
            2,
            0
        ],
        [
            "BWL030",
            "Örgütsel Davranış",
            6,
            2,
            2,
            0
        ],
        [
            "BWL032",
            "Kültürlerarası Yetkinlik Geliştirme",
            6,
            2,
            2,
            0
        ],
        [
            "BWL033",
            "Personel Yönetimi",
            6,
            2,
            2,
            0
        ],
        [
            "BWL034",
            "Uluslararası İşletme",
            6,
            2,
            2,
            0
        ],
        [
            "BWL101",
            "İşletmeye Giriş",
            6,
            2,
            2,
            0
        ],
        [
            "BWL201",
            "Pazarlamanın Temelleri",
            5,
            2,
            2,
            0
        ],
        [
            "BWL206",
            "Proje Yönetimi",
            6,
            2,
            2,
            0
        ],
        [
            "BWL212",
            "Stratejik Pazarlama",
            5,
            2,
            2,
            0
        ],
        [
            "BWL216",
            "E-İş",
            3,
            2,
            0,
            0
        ],
        [
            "BWL417",
            "İş Etiği",
            3,
            1,
            1,
            0
        ],
        [
            "HUK150",
            "Anayasa Hukuku II",
            4,
            3,
            0,
            0
        ],
        [
            "HUK153",
            "Anayasa Hukuku I",
            3,
            2,
            0,
            0
        ],
        [
            "HUK159",
            "Anayasa Hukukunun Temel Prensipleri",
            5,
            2,
            0,
            0
        ],
        [
            "HUK252",
            "Uluslararası Hukuk II",
            5,
            3,
            0,
            0
        ],
        [
            "HUK253",
            "İdare Hukuku I",
            5,
            3,
            1,
            0
        ],
        [
            "HUK255",
            "Uluslararası Hukuk I",
            3,
            2,
            0,
            0
        ],
        [
            "HUK256",
            "İdare Hukuku II",
            5,
            3,
            0,
            0
        ],
        [
            "HUK359",
            "Avrupa Hukuku",
            4,
            3,
            0,
            0
        ],
        [
            "HUK361",
            "Alman İdare Hukuku - Temel Kavramlar",
            4,
            3,
            0,
            0
        ],
        [
            "HUK453",
            "Devletler Özel Hukuku I",
            4,
            3,
            0,
            0
        ],
        [
            "HUK913",
            "Borçlar Hukuku",
            3,
            2,
            0,
            0
        ],
        [
            "HUK915",
            "İş Hukuku",
            3,
            2,
            0,
            0
        ],
        [
            "HUK916",
            "Ticaret Hukuku",
            3,
            2,
            0,
            0
        ],
        [
            "KKW103",
            "İletişim Bilimlerine Giriş",
            6,
            3,
            0,
            0
        ],
        [
            "KKW105",
            "Kültür Bilimlerine Giriş",
            6,
            3,
            0,
            0
        ],
        [
            "KKW106",
            "Kültür Kuramları",
            5,
            3,
            0,
            0
        ],
        [
            "KKW218",
            "Göç ve Kültür",
            4,
            2,
            0,
            0
        ],
        [
            "KKW226",
            "Kültür ve Küreselleşme",
            4,
            2,
            0,
            0
        ],
        [
            "KKW228",
            "Kültürlerarası İletişim",
            4,
            3,
            0,
            0
        ],
        [
            "KKW317",
            "Halkla İlişkiler",
            6,
            2,
            1,
            0
        ],
        [
            "KKW324",
            "Kültürel Bellek",
            6,
            3,
            0,
            0
        ],
        [
            "POL210",
            "Çok Düzlemli Sistemlerde Politik Saha Analizi",
            5,
            3,
            0,
            0
        ],
        [
            "POL401",
            "Karşılaştırmalı Parti ve Seçim Sistemleri",
            5,
            3,
            0,
            0
        ],
        [
            "POL501",
            "Siyaset Sosyolojisi",
            5,
            3,
            0,
            0
        ],
        [
            "POL502",
            "Kamu Yönetimine Giriş",
            5,
            3,
            0,
            0
        ],
        [
            "POL503",
            "ABD Dış Politikası",
            5,
            3,
            0,
            0
        ],
        [
            "POL504",
            "Latin Amerika Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL505",
            "Güncel Ortadoğu ve Kuzey Afrika Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL506",
            "Orta Asya ve Kafkasya Bölgesel Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL507",
            "Rus Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL508",
            "Dünya Siyasetinde Doğu Asya",
            5,
            3,
            0,
            0
        ],
        [
            "POL509",
            "Dünya Siyasetinde Afrika",
            5,
            3,
            0,
            0
        ],
        [
            "POL510",
            "Güneydoğu Avrupa Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL511",
            "Güncel Avrupa Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL512",
            "Karşılaştırmalı Avrupa Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL513",
            "AB Dış ve Güvenlik Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL514",
            "Almanya, Türkiye ve AB için Güncel Politik Sorunlar",
            5,
            3,
            0,
            0
        ],
        [
            "POL515",
            "Türk-Alman İlişkileri: Ekonomi, Siyaset ve Toplum",
            5,
            3,
            0,
            0
        ],
        [
            "POL516",
            "Sekülerleşme Teorileri",
            5,
            3,
            0,
            0
        ],
        [
            "POL517",
            "Küreselleşme",
            5,
            3,
            0,
            0
        ],
        [
            "POL518",
            "Irkçılık, Antisemitizm ve İslamofobi",
            5,
            3,
            0,
            0
        ],
        [
            "POL519",
            "Toplumsal Cinsiyet ve Siyaset",
            5,
            3,
            0,
            0
        ],
        [
            "POL520",
            "Siyaset Bilimi Stajı",
            2,
            0,
            0,
            0
        ],
        [
            "POL521",
            "Türk Dış Politikasında Güncel Meseleler",
            5,
            3,
            0,
            0
        ],
        [
            "POL522",
            "Avrupa'da Federalizm - Avrupa Federalizmi",
            5,
            3,
            0,
            0
        ],
        [
            "POL523",
            "Siyaset Teorisinde Meseleler",
            5,
            3,
            0,
            0
        ],
        [
            "POL524",
            "Değişen Küresel Sistemde Bölgesel Güçler",
            5,
            3,
            0,
            0
        ],
        [
            "POL526",
            "Türk-Alman Diplomatik İlişkiler Tarihi",
            5,
            3,
            0,
            0
        ],
        [
            "POL553",
            "Osmanlı Siyasi Tarihi II",
            5,
            3,
            0,
            0
        ],
        [
            "POL554",
            "Türk Tarihi",
            5,
            3,
            0,
            0
        ],
        [
            "POL555",
            "Siyaset Psikolojisi",
            5,
            3,
            0,
            0
        ],
        [
            "POL556",
            "Türk Devletleri Teşkilatı ve Türk Devletleri Bütünleşmesi",
            5,
            3,
            0,
            0
        ],
        [
            "POL601",
            "20. Yüzyıl Avrupa Siyasi Düşüncesi",
            5,
            3,
            0,
            0
        ],
        [
            "POL602",
            "Göç, Diaspora ve Mülteciler",
            5,
            3,
            0,
            0
        ],
        [
            "POL603",
            "Milliyetçilik Teorileri",
            5,
            3,
            0,
            0
        ],
        [
            "POL604",
            "Dünya Siyaseti ve Sinema",
            5,
            3,
            0,
            0
        ],
        [
            "POL605",
            "Uluslararası Enerji Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL606",
            "Küresel İklim Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL701",
            "Almanya ve Türkiye Karşılaştırmalı Siyasal Sistemler",
            5,
            3,
            0,
            0
        ],
        [
            "POL707",
            "Kamu Yönetiminde Güncel Meseleler",
            5,
            3,
            0,
            0
        ],
        [
            "POL709",
            "Yerel Yönetimler",
            5,
            3,
            0,
            0
        ],
        [
            "POL711",
            "Kentleşme ve Çevre Sorunları",
            5,
            3,
            0,
            0
        ],
        [
            "VWL301",
            "Uluslararası İktisat I",
            6,
            3,
            1,
            0
        ],
        [
            "VWL302",
            "Uluslararası İktisat II",
            6,
            3,
            1,
            0
        ],
        [
            "VWL304",
            "İktisadi Düşünceler Tarihi",
            6,
            3,
            0,
            0
        ],
        [
            "VWL401",
            "Türkiye Ekonomisinin Gelişimi",
            6,
            3,
            0,
            0
        ],
        [
            "VWL403",
            "Dünya Ekonomisinin Güncel Sorunları",
            6,
            3,
            0,
            0
        ],
        [
            "VWL426",
            "Demografi ve Göç İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL428",
            "Avrupa Bütünleşmesinin İktisadı",
            5,
            2,
            0,
            0
        ]
    ],
    "POLSDV": [
        [
            "BWL002",
            "Girişimcilik",
            6,
            2,
            2,
            0
        ],
        [
            "BWL007",
            "Dijital Pazarlama",
            6,
            2,
            2,
            0
        ],
        [
            "BWL030",
            "Örgütsel Davranış",
            6,
            2,
            2,
            0
        ],
        [
            "BWL032",
            "Kültürlerarası Yetkinlik Geliştirme",
            6,
            2,
            2,
            0
        ],
        [
            "BWL033",
            "Personel Yönetimi",
            6,
            2,
            2,
            0
        ],
        [
            "BWL034",
            "Uluslararası İşletme",
            6,
            2,
            2,
            0
        ],
        [
            "BWL101",
            "İşletmeye Giriş",
            6,
            2,
            2,
            0
        ],
        [
            "BWL201",
            "Pazarlamanın Temelleri",
            5,
            2,
            2,
            0
        ],
        [
            "BWL206",
            "Proje Yönetimi",
            6,
            2,
            2,
            0
        ],
        [
            "BWL212",
            "Stratejik Pazarlama",
            5,
            2,
            2,
            0
        ],
        [
            "BWL216",
            "E-İş",
            3,
            2,
            0,
            0
        ],
        [
            "BWL417",
            "İş Etiği",
            3,
            1,
            1,
            0
        ],
        [
            "HUK150",
            "Anayasa Hukuku II",
            4,
            3,
            0,
            0
        ],
        [
            "HUK153",
            "Anayasa Hukuku I",
            3,
            2,
            0,
            0
        ],
        [
            "HUK159",
            "Anayasa Hukukunun Temel Prensipleri",
            5,
            2,
            0,
            0
        ],
        [
            "HUK252",
            "Uluslararası Hukuk II",
            5,
            3,
            0,
            0
        ],
        [
            "HUK253",
            "İdare Hukuku I",
            5,
            3,
            1,
            0
        ],
        [
            "HUK255",
            "Uluslararası Hukuk I",
            3,
            2,
            0,
            0
        ],
        [
            "HUK256",
            "İdare Hukuku II",
            5,
            3,
            0,
            0
        ],
        [
            "HUK359",
            "Avrupa Hukuku",
            4,
            3,
            0,
            0
        ],
        [
            "HUK361",
            "Alman İdare Hukuku - Temel Kavramlar",
            4,
            3,
            0,
            0
        ],
        [
            "HUK453",
            "Devletler Özel Hukuku I",
            4,
            3,
            0,
            0
        ],
        [
            "HUK913",
            "Borçlar Hukuku",
            3,
            2,
            0,
            0
        ],
        [
            "HUK915",
            "İş Hukuku",
            3,
            2,
            0,
            0
        ],
        [
            "HUK916",
            "Ticaret Hukuku",
            3,
            2,
            0,
            0
        ],
        [
            "KKW103",
            "İletişim Bilimlerine Giriş",
            6,
            3,
            0,
            0
        ],
        [
            "KKW105",
            "Kültür Bilimlerine Giriş",
            6,
            3,
            0,
            0
        ],
        [
            "KKW106",
            "Kültür Kuramları",
            5,
            3,
            0,
            0
        ],
        [
            "KKW218",
            "Göç ve Kültür",
            4,
            2,
            0,
            0
        ],
        [
            "KKW226",
            "Kültür ve Küreselleşme",
            4,
            2,
            0,
            0
        ],
        [
            "KKW228",
            "Kültürlerarası İletişim",
            4,
            3,
            0,
            0
        ],
        [
            "KKW317",
            "Halkla İlişkiler",
            6,
            2,
            1,
            0
        ],
        [
            "KKW324",
            "Kültürel Bellek",
            6,
            3,
            0,
            0
        ],
        [
            "POL210",
            "Çok Düzlemli Sistemlerde Politik Saha Analizi",
            5,
            3,
            0,
            0
        ],
        [
            "POL401",
            "Karşılaştırmalı Parti ve Seçim Sistemleri",
            5,
            3,
            0,
            0
        ],
        [
            "POL501",
            "Siyaset Sosyolojisi",
            5,
            3,
            0,
            0
        ],
        [
            "POL502",
            "Kamu Yönetimine Giriş",
            5,
            3,
            0,
            0
        ],
        [
            "POL503",
            "ABD Dış Politikası",
            5,
            3,
            0,
            0
        ],
        [
            "POL504",
            "Latin Amerika Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL505",
            "Güncel Ortadoğu ve Kuzey Afrika Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL506",
            "Orta Asya ve Kafkasya Bölgesel Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL507",
            "Rus Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL508",
            "Dünya Siyasetinde Doğu Asya",
            5,
            3,
            0,
            0
        ],
        [
            "POL509",
            "Dünya Siyasetinde Afrika",
            5,
            3,
            0,
            0
        ],
        [
            "POL510",
            "Güneydoğu Avrupa Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL511",
            "Güncel Avrupa Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL512",
            "Karşılaştırmalı Avrupa Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL513",
            "AB Dış ve Güvenlik Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL514",
            "Almanya, Türkiye ve AB için Güncel Politik Sorunlar",
            5,
            3,
            0,
            0
        ],
        [
            "POL515",
            "Türk-Alman İlişkileri: Ekonomi, Siyaset ve Toplum",
            5,
            3,
            0,
            0
        ],
        [
            "POL516",
            "Sekülerleşme Teorileri",
            5,
            3,
            0,
            0
        ],
        [
            "POL517",
            "Küreselleşme",
            5,
            3,
            0,
            0
        ],
        [
            "POL518",
            "Irkçılık, Antisemitizm ve İslamofobi",
            5,
            3,
            0,
            0
        ],
        [
            "POL519",
            "Toplumsal Cinsiyet ve Siyaset",
            5,
            3,
            0,
            0
        ],
        [
            "POL520",
            "Siyaset Bilimi Stajı",
            2,
            0,
            0,
            0
        ],
        [
            "POL521",
            "Türk Dış Politikasında Güncel Meseleler",
            5,
            3,
            0,
            0
        ],
        [
            "POL522",
            "Avrupa'da Federalizm - Avrupa Federalizmi",
            5,
            3,
            0,
            0
        ],
        [
            "POL523",
            "Siyaset Teorisinde Meseleler",
            5,
            3,
            0,
            0
        ],
        [
            "POL524",
            "Değişen Küresel Sistemde Bölgesel Güçler",
            5,
            3,
            0,
            0
        ],
        [
            "POL526",
            "Türk-Alman Diplomatik İlişkiler Tarihi",
            5,
            3,
            0,
            0
        ],
        [
            "POL553",
            "Osmanlı Siyasi Tarihi II",
            5,
            3,
            0,
            0
        ],
        [
            "POL554",
            "Türk Tarihi",
            5,
            3,
            0,
            0
        ],
        [
            "POL555",
            "Siyaset Psikolojisi",
            5,
            3,
            0,
            0
        ],
        [
            "POL556",
            "Türk Devletleri Teşkilatı ve Türk Devletleri Bütünleşmesi",
            5,
            3,
            0,
            0
        ],
        [
            "POL601",
            "20. Yüzyıl Avrupa Siyasi Düşüncesi",
            5,
            3,
            0,
            0
        ],
        [
            "POL602",
            "Göç, Diaspora ve Mülteciler",
            5,
            3,
            0,
            0
        ],
        [
            "POL603",
            "Milliyetçilik Teorileri",
            5,
            3,
            0,
            0
        ],
        [
            "POL604",
            "Dünya Siyaseti ve Sinema",
            5,
            3,
            0,
            0
        ],
        [
            "POL605",
            "Uluslararası Enerji Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL606",
            "Küresel İklim Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "POL701",
            "Almanya ve Türkiye Karşılaştırmalı Siyasal Sistemler",
            5,
            3,
            0,
            0
        ],
        [
            "POL707",
            "Kamu Yönetiminde Güncel Meseleler",
            5,
            3,
            0,
            0
        ],
        [
            "POL709",
            "Yerel Yönetimler",
            5,
            3,
            0,
            0
        ],
        [
            "POL711",
            "Kentleşme ve Çevre Sorunları",
            5,
            3,
            0,
            0
        ],
        [
            "VWL301",
            "Uluslararası İktisat I",
            6,
            3,
            1,
            0
        ],
        [
            "VWL302",
            "Uluslararası İktisat II",
            6,
            3,
            1,
            0
        ],
        [
            "VWL304",
            "İktisadi Düşünceler Tarihi",
            6,
            3,
            0,
            0
        ],
        [
            "VWL401",
            "Türkiye Ekonomisinin Gelişimi",
            6,
            3,
            0,
            0
        ],
        [
            "VWL403",
            "Dünya Ekonomisinin Güncel Sorunları",
            6,
            3,
            0,
            0
        ],
        [
            "VWL426",
            "Demografi ve Göç İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL428",
            "Avrupa Bütünleşmesinin İktisadı",
            5,
            2,
            0,
            0
        ]
    ]
},
        "pool_metadata": {
    "POLSDI": {},
    "POLSDV": {}
},
    },
    "Sosyoloji": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "SOZ101",
            "Sosyolojiye Giriş I",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ103",
            "Sosyolojinin Felsefi Temelleri",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ111",
            "Çalışma Teknikleri: Bilimsel Çalışmalar",
            4,
            0,
            0,
            0
        ],
        [
            "SOZ151",
            "Küresel Uygarlıklar Tarihi",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ161",
            "Mesleki Almanca I",
            2,
            0,
            0,
            0
        ],
        [
            "AIT001",
            "Atatürk İlkeleri ve İnkılap Tarihi I",
            2,
            0,
            0,
            0
        ],
        [
            "TUR001",
            "Türkçe I",
            2,
            0,
            0,
            0
        ],
        [
            "ENG101",
            "İngilizce I",
            2,
            0,
            0,
            0
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "SOZ102",
            "Sosyolojiye Giriş II",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ132",
            "Sosyal Bilimlerde Mantık ve Argümantasyon",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ142",
            "Sosyal ve Kültürel Antropoloji",
            4,
            0,
            0,
            0
        ],
        [
            "SOZ152",
            "Türkiye’nin Toplumsal Yapısı",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ162",
            "Mesleki Almanca II",
            2,
            0,
            0,
            0
        ],
        [
            "AIT002",
            "Atatürk İlkeleri ve İnkılap Tarihi II",
            2,
            0,
            0,
            0
        ],
        [
            "TUR002",
            "Türkçe II",
            2,
            0,
            0,
            0
        ],
        [
            "ENG102",
            "İngilizce II",
            2,
            0,
            0,
            0
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "SOZ201",
            "Sosyoloji Kuramları I",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ215",
            "Toplum Bilimlerinde Nitel Araştırma Yöntemleri",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ217",
            "İstatistik",
            5,
            0,
            0,
            0
        ],
        [
            "SOZ231",
            "Göç Sosyolojisi",
            5,
            0,
            0,
            0
        ],
        [
            "SOZ241",
            "Kent Sosyolojisi",
            4,
            0,
            0,
            0
        ],
        [
            "SOZ261",
            "Mesleki Almanca III",
            2,
            0,
            0,
            0
        ],
        [
            "ENG201",
            "İngilizce III",
            2,
            0,
            0,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "SOZ202",
            "Sosyoloji Kuramları II",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ216",
            "Toplum Bilimlerinde Nicel Araştırma Yöntemleri",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ218",
            "Toplumsal Yapılar ve Değişim",
            4,
            0,
            0,
            0
        ],
        [
            "SOZ222",
            "Toplumsal Cinsiyet Sosyolojisi",
            4,
            0,
            0,
            0
        ],
        [
            "SOZ262",
            "Mesleki Almanca IV",
            2,
            0,
            0,
            0
        ],
        [
            "ENG202",
            "İngilizce IV",
            2,
            0,
            0,
            0
        ],
        [
            "SOZSDI",
            "Seçmeli Ders",
            6,
            0,
            0,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "SOZ311",
            "İleri Araştırma Yöntemleri I",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ361",
            "Mesleki Almanca V",
            2,
            0,
            0,
            0
        ],
        [
            "ENG301",
            "İleri İngilizce I",
            2,
            0,
            0,
            0
        ],
        [
            "ÜSD001",
            "Üniversite Seçmeli Ders Havuzu I",
            2,
            0,
            0,
            0
        ],
        [
            "SOZSDII",
            "Seçmeli Ders",
            6,
            0,
            0,
            0
        ],
        [
            "SOZSDII",
            "Seçmeli Ders",
            6,
            0,
            0,
            0
        ],
        [
            "SOZSDII",
            "Seçmeli Ders",
            6,
            0,
            0,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "SOZ312",
            "İleri Araştırma Yöntemleri II",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ362",
            "Mesleki Almanca VI",
            2,
            0,
            0,
            0
        ],
        [
            "ENG302",
            "İleri İngilizce II",
            2,
            0,
            0,
            0
        ],
        [
            "ÜSD002",
            "Üniversite Seçmeli Ders Havuzu II",
            2,
            0,
            0,
            0
        ],
        [
            "SOZSDIII",
            "Seçmeli Ders",
            6,
            0,
            0,
            0
        ],
        [
            "SOZSDIII",
            "Seçmeli Ders",
            6,
            0,
            0,
            0
        ],
        [
            "SOZSDIII",
            "Seçmeli Ders",
            6,
            0,
            0,
            0
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "SOZ419",
            "Staj",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ421",
            "Türk-Alman İlişkileri",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ431",
            "Türkiye Sosyoloji Tarihi",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ437",
            "Araştırma Semineri",
            6,
            0,
            0,
            0
        ],
        [
            "SOZSDIV",
            "Seçmeli Ders",
            6,
            0,
            0,
            0
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "SOZ422",
            "Ulusaşırı Türk-Alman Alanları",
            6,
            0,
            0,
            0
        ],
        [
            "SOZ426",
            "Bitirme Projesi",
            12,
            0,
            0,
            0
        ],
        [
            "SOZSDV",
            "Seçmeli Ders",
            6,
            0,
            0,
            0
        ],
        [
            "SOZSDV",
            "Seçmeli Ders",
            6,
            0,
            0,
            0
        ]
    ]
},
        "pool_codes": {
    "SOZSDI": [
        "Sanat Sosyolojisi",
        "Teknoloji Bağımlılığı",
        "Küreselleşme",
        "Gelecek Sosyolojisi",
        "Organizasyon Sosyolojisi",
        "Sosyolojide Güncel Tartışmalar",
        "Almanya'da Güncel Azınlık Tartışmaları",
        "Avrupa’da Kapitalizmin Doğuşu",
        "Avrupa’da Sosyal Politika",
        "Avrupa’da Toplumsal Dönüşümler",
        "Beden Sosyolojisi",
        "Kültür Sosyolojisi",
        "Kurumlar Sosyolojisi",
        "Tarihsel Sosyoloji",
        "Ayrımcılık Sosyolojisi",
        "Çalışma Sosyolojisi",
        "Eleştirel Kuram ve Toplum",
        "Tüketim Toplumu",
        "Toplumsal Hareketler",
        "Sağlık Sosyolojisi",
        "Toplumsal Ekoloji",
        "Mekan Sosyolojisi"
    ],
    "SOZSDII": [
        "Görsel Sosyoloji",
        "Gençlik Sosyolojisi",
        "Sosyolojik Kuramların Dilleri",
        "Sosyoloji ve Yapay Zeka",
        "Ekonomi Sosyolojisi",
        "Türkiye'de Kapitalizmin Gelişimi",
        "Türkiye’de Göç Çalışmaları",
        "Türkiye’de Toplumsal Dönüşümler",
        "Bir Metropolün Sosyolojisi: İstanbul",
        "Duygular Sosyolojisi",
        "Din Sosyolojisi",
        "Gündelik Hayat Sosyolojisi",
        "Siyaset Sosyolojisi",
        "Afet Sosyolojisi",
        "Eğitim Sosyolojisi",
        "Hayvanlar ve Toplum",
        "Öznellikler ve Özneleşme",
        "Kolektif ve Toplumsal Hafıza",
        "Engellilik Sosyolojisi",
        "Proje Tasarımı",
        "Güncel Etnografi Okumaları"
    ],
    "SOZSDIII": [
        "Sanat Sosyolojisi",
        "Teknoloji Bağımlılığı",
        "Küreselleşme",
        "Gelecek Sosyolojisi",
        "Organizasyon Sosyolojisi",
        "Sosyolojide Güncel Tartışmalar",
        "Almanya'da Güncel Azınlık Tartışmaları",
        "Avrupa’da Kapitalizmin Doğuşu",
        "Avrupa’da Sosyal Politika",
        "Avrupa’da Toplumsal Dönüşümler",
        "Beden Sosyolojisi",
        "Kültür Sosyolojisi",
        "Kurumlar Sosyolojisi",
        "Tarihsel Sosyoloji",
        "Ayrımcılık Sosyolojisi",
        "Çalışma Sosyolojisi",
        "Eleştirel Kuram ve Toplum",
        "Tüketim Toplumu",
        "Toplumsal Hareketler",
        "Sağlık Sosyolojisi",
        "Toplumsal Ekoloji",
        "Mekan Sosyolojisi"
    ],
    "SOZSDIV": [
        "Görsel Sosyoloji",
        "Gençlik Sosyolojisi",
        "Sosyolojik Kuramların Dilleri",
        "Sosyoloji ve Yapay Zeka",
        "Ekonomi Sosyolojisi",
        "Türkiye'de Kapitalizmin Gelişimi",
        "Türkiye’de Göç Çalışmaları",
        "Türkiye’de Toplumsal Dönüşümler",
        "Bir Metropolün Sosyolojisi: İstanbul",
        "Duygular Sosyolojisi",
        "Din Sosyolojisi",
        "Gündelik Hayat Sosyolojisi",
        "Siyaset Sosyolojisi",
        "Afet Sosyolojisi",
        "Eğitim Sosyolojisi",
        "Hayvanlar ve Toplum",
        "Öznellikler ve Özneleşme",
        "Kolektif ve Toplumsal Hafıza",
        "Engellilik Sosyolojisi",
        "Proje Tasarımı",
        "Güncel Etnografi Okumaları"
    ],
    "SOZSDV": [
        "Sanat Sosyolojisi",
        "Teknoloji Bağımlılığı",
        "Küreselleşme",
        "Gelecek Sosyolojisi",
        "Organizasyon Sosyolojisi",
        "Sosyolojide Güncel Tartışmalar",
        "Almanya'da Güncel Azınlık Tartışmaları",
        "Avrupa’da Kapitalizmin Doğuşu",
        "Avrupa’da Sosyal Politika",
        "Avrupa’da Toplumsal Dönüşümler",
        "Beden Sosyolojisi",
        "Kültür Sosyolojisi",
        "Kurumlar Sosyolojisi",
        "Tarihsel Sosyoloji",
        "Ayrımcılık Sosyolojisi",
        "Çalışma Sosyolojisi",
        "Eleştirel Kuram ve Toplum",
        "Tüketim Toplumu",
        "Toplumsal Hareketler",
        "Sağlık Sosyolojisi",
        "Toplumsal Ekoloji",
        "Mekan Sosyolojisi"
    ]
},
        "pools": {
    "SOZSDI": [
        [
            "SOZ022",
            "Sanat Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ024",
            "Teknoloji Bağımlılığı",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ026",
            "Küreselleşme",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ028",
            "Gelecek Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ030",
            "Organizasyon Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ032",
            "Sosyolojide Güncel Tartışmalar",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ042",
            "Almanya'da Güncel Azınlık Tartışmaları",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ044",
            "Avrupa’da Kapitalizmin Doğuşu",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ046",
            "Avrupa’da Sosyal Politika",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ048",
            "Avrupa’da Toplumsal Dönüşümler",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ052",
            "Beden Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ054",
            "Kültür Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ056",
            "Kurumlar Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ058",
            "Tarihsel Sosyoloji",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ060",
            "Ayrımcılık Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ062",
            "Çalışma Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ064",
            "Eleştirel Kuram ve Toplum",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ066",
            "Tüketim Toplumu",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ068",
            "Toplumsal Hareketler",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ070",
            "Sağlık Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ072",
            "Toplumsal Ekoloji",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ078",
            "Mekan Sosyolojisi",
            6,
            3,
            0,
            0
        ]
    ],
    "SOZSDII": [
        [
            "SOZ021",
            "Görsel Sosyoloji",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ023",
            "Gençlik Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ025",
            "Sosyolojik Kuramların Dilleri",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ027",
            "Sosyoloji ve Yapay Zeka",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ029",
            "Ekonomi Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ033",
            "Türkiye'de Kapitalizmin Gelişimi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ035",
            "Türkiye’de Göç Çalışmaları",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ037",
            "Türkiye’de Toplumsal Dönüşümler",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ039",
            "Bir Metropolün Sosyolojisi: İstanbul",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ051",
            "Duygular Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ053",
            "Din Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ055",
            "Gündelik Hayat Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ057",
            "Siyaset Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ059",
            "Afet Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ063",
            "Eğitim Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ065",
            "Hayvanlar ve Toplum",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ067",
            "Öznellikler ve Özneleşme",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ069",
            "Kolektif ve Toplumsal Hafıza",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ071",
            "Engellilik Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ073",
            "Proje Tasarımı",
            6,
            1,
            2,
            0
        ],
        [
            "SOZ075",
            "Güncel Etnografi Okumaları",
            6,
            3,
            0,
            0
        ]
    ],
    "SOZSDIII": [
        [
            "SOZ022",
            "Sanat Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ024",
            "Teknoloji Bağımlılığı",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ026",
            "Küreselleşme",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ028",
            "Gelecek Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ030",
            "Organizasyon Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ032",
            "Sosyolojide Güncel Tartışmalar",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ042",
            "Almanya'da Güncel Azınlık Tartışmaları",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ044",
            "Avrupa’da Kapitalizmin Doğuşu",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ046",
            "Avrupa’da Sosyal Politika",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ048",
            "Avrupa’da Toplumsal Dönüşümler",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ052",
            "Beden Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ054",
            "Kültür Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ056",
            "Kurumlar Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ058",
            "Tarihsel Sosyoloji",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ060",
            "Ayrımcılık Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ062",
            "Çalışma Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ064",
            "Eleştirel Kuram ve Toplum",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ066",
            "Tüketim Toplumu",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ068",
            "Toplumsal Hareketler",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ070",
            "Sağlık Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ072",
            "Toplumsal Ekoloji",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ078",
            "Mekan Sosyolojisi",
            6,
            3,
            0,
            0
        ]
    ],
    "SOZSDIV": [
        [
            "SOZ021",
            "Görsel Sosyoloji",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ023",
            "Gençlik Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ025",
            "Sosyolojik Kuramların Dilleri",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ027",
            "Sosyoloji ve Yapay Zeka",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ029",
            "Ekonomi Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ033",
            "Türkiye'de Kapitalizmin Gelişimi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ035",
            "Türkiye’de Göç Çalışmaları",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ037",
            "Türkiye’de Toplumsal Dönüşümler",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ039",
            "Bir Metropolün Sosyolojisi: İstanbul",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ051",
            "Duygular Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ053",
            "Din Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ055",
            "Gündelik Hayat Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ057",
            "Siyaset Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ059",
            "Afet Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ063",
            "Eğitim Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ065",
            "Hayvanlar ve Toplum",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ067",
            "Öznellikler ve Özneleşme",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ069",
            "Kolektif ve Toplumsal Hafıza",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ071",
            "Engellilik Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ073",
            "Proje Tasarımı",
            6,
            1,
            2,
            0
        ],
        [
            "SOZ075",
            "Güncel Etnografi Okumaları",
            6,
            3,
            0,
            0
        ]
    ],
    "SOZSDV": [
        [
            "SOZ022",
            "Sanat Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ024",
            "Teknoloji Bağımlılığı",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ026",
            "Küreselleşme",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ028",
            "Gelecek Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ030",
            "Organizasyon Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ032",
            "Sosyolojide Güncel Tartışmalar",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ042",
            "Almanya'da Güncel Azınlık Tartışmaları",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ044",
            "Avrupa’da Kapitalizmin Doğuşu",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ046",
            "Avrupa’da Sosyal Politika",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ048",
            "Avrupa’da Toplumsal Dönüşümler",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ052",
            "Beden Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ054",
            "Kültür Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ056",
            "Kurumlar Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ058",
            "Tarihsel Sosyoloji",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ060",
            "Ayrımcılık Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ062",
            "Çalışma Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ064",
            "Eleştirel Kuram ve Toplum",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ066",
            "Tüketim Toplumu",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ068",
            "Toplumsal Hareketler",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ070",
            "Sağlık Sosyolojisi",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ072",
            "Toplumsal Ekoloji",
            6,
            3,
            0,
            0
        ],
        [
            "SOZ078",
            "Mekan Sosyolojisi",
            6,
            3,
            0,
            0
        ]
    ]
},
        "pool_metadata": {
    "SOZSDI": {},
    "SOZSDII": {},
    "SOZSDIII": {},
    "SOZSDIV": {},
    "SOZSDV": {}
},
    },
    "Üniversites Seçmeli Dersler (ÜSD)": {
        "curriculum": {},
        "pool_codes": {
    "ÜSD": [
        "Mikrobiyoloji II",
        "İmmünoloji",
        "Ekoloji",
        "Evrim Biyolojisi",
        "Biyobilimler için Organik Kimya",
        "İnşaat Mühendisliğine Giriş",
        "Pazarlamanın Temelleri",
        "Stratejik Pazarlama",
        "Elektrik-Elektronik Mühendisliğine Giriş",
        "Sayısal Tasarım",
        "Nesnel Programlama",
        "Makine Mühendisliğine Giriş",
        "Statik",
        "Malzeme Teknolojisi I",
        "İktisadi Tarih",
        "Endüstri Mühendisliğine Giriş",
        "Muhasebe ve Bilanço",
        "Fikri Mülkiyet Hukuku",
        "Felsefe Tarihi",
        "Girişimcilik",
        "Bilgisayar Bilimine ve Programlamaya Giriş",
        "Doğa Bilimlerine Giriş"
    ]
},
        "pools": {
    "ÜSD": [
        [
            "MBT361",
            "Mikrobiyoloji II",
            6,
            3,
            0,
            2
        ],
        [
            "MBT363",
            "İmmünoloji",
            6,
            3,
            2,
            0
        ],
        [
            "MBT365",
            "Ekoloji",
            6,
            3,
            0,
            2
        ],
        [
            "MBT367",
            "Evrim Biyolojisi",
            6,
            3,
            0,
            2
        ],
        [
            "NWI301",
            "Biyobilimler için Organik Kimya",
            6,
            3,
            3,
            0
        ],
        [
            "BAU091",
            "İnşaat Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "BWL201",
            "Pazarlamanın Temelleri",
            5,
            2,
            2,
            0
        ],
        [
            "BWL212",
            "Stratejik Pazarlama",
            5,
            2,
            2,
            0
        ],
        [
            "ETE091",
            "Elektrik-Elektronik Mühendisliğine Giriş",
            2,
            2,
            1,
            0
        ],
        [
            "ETE101",
            "Sayısal Tasarım",
            6,
            2,
            1,
            1
        ],
        [
            "INF102",
            "Nesnel Programlama",
            6,
            2,
            0,
            2
        ],
        [
            "MAB091",
            "Makine Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "MAB109",
            "Statik",
            6,
            3,
            2,
            0
        ],
        [
            "MAB207",
            "Malzeme Teknolojisi I",
            6,
            3,
            2,
            0
        ],
        [
            "VWL203",
            "İktisadi Tarih",
            6,
            3,
            0,
            0
        ],
        [
            "WIN091",
            "Endüstri Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "WIN204",
            "Muhasebe ve Bilanço",
            6,
            2,
            2,
            1
        ],
        [
            "HUK553",
            "Fikri Mülkiyet Hukuku",
            2,
            2,
            0,
            0
        ],
        [
            "KKW321",
            "Felsefe Tarihi",
            4,
            3,
            0,
            0
        ],
        [
            "ING404",
            "Girişimcilik",
            2,
            2,
            0,
            0
        ],
        [
            "INF101",
            "Bilgisayar Bilimine ve Programlamaya Giriş",
            6,
            2,
            0,
            2
        ],
        [
            "NWI107",
            "Doğa Bilimlerine Giriş",
            2,
            2,
            0,
            0
        ]
    ]
},
        "pool_metadata": {
    "ÜSD": {}
},
    },
    "İktisat": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "VWL101",
            "İktisada Giriş 1",
            6,
            2,
            6,
            0
        ],
        [
            "VWL103",
            "Matematik 1",
            5,
            2,
            5,
            0
        ],
        [
            "VWL105",
            "İstatistik 1",
            5,
            2,
            5,
            0
        ],
        [
            "HUK931",
            "Hukuka Giriş",
            3,
            0,
            3,
            0
        ],
        [
            "BWL181",
            "İşletmeye Giriş",
            6,
            2,
            6,
            0
        ],
        [
            "DEU111",
            "İktisatçılar için Almanca 1",
            3,
            0,
            3,
            0
        ],
        [
            "ENG101",
            "İngilizce 1",
            2,
            0,
            2,
            0
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "VWL102",
            "İktisada Giriş 2",
            6,
            2,
            6,
            0
        ],
        [
            "VWL104",
            "Matematik 2",
            5,
            2,
            5,
            0
        ],
        [
            "VWL106",
            "İstatistik 2",
            5,
            2,
            5,
            0
        ],
        [
            "VWL108",
            "Bilimsel Çalışma Yöntemleri",
            3,
            0,
            3,
            0
        ],
        [
            "VWL110",
            "Bilgisayar Programlaması ve Kullanımına Giriş",
            3,
            2,
            3,
            0
        ],
        [
            "VWL112",
            "Toplum Bilimine Giriş",
            3,
            0,
            3,
            0
        ],
        [
            "DEU112",
            "İktisatçılar için Almanca 2",
            3,
            0,
            3,
            0
        ],
        [
            "ENG102",
            "İngilizce 2",
            2,
            0,
            2,
            0
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "VWL201",
            "Mikro İktisat",
            6,
            2,
            6,
            0
        ],
        [
            "BWL183",
            "Muhasebenin Temelleri",
            5,
            2,
            5,
            0
        ],
        [
            "VWL203",
            "İktisadi Tarih",
            6,
            0,
            6,
            0
        ],
        [
            "VWL205",
            "Ekonometri I",
            6,
            2,
            6,
            0
        ],
        [
            "POL191",
            "Siyaset Bilimine Giriş",
            5,
            0,
            5,
            0
        ],
        [
            "ENG201",
            "İngilizce 3",
            2,
            0,
            2,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "VWL202",
            "Makro İktisat",
            6,
            2,
            6,
            0
        ],
        [
            "VWL204",
            "Oyun Teorisi",
            5,
            2,
            5,
            0
        ],
        [
            "VWL206",
            "Ekonometri II",
            6,
            2,
            6,
            0
        ],
        [
            "VWL208",
            "Çevre ve Doğal Kaynaklar İktisadı",
            5,
            0,
            5,
            0
        ],
        [
            "VWL210",
            "Para, Banka ve Finans Piyasaları",
            6,
            2,
            6,
            0
        ],
        [
            "ENG202",
            "İngilizce 4",
            2,
            0,
            2,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "VWL301",
            "Uluslararası İktisat I",
            6,
            1,
            6,
            0
        ],
        [
            "VWL303",
            "Kamu Maliyesi ve Kamu İktisadı",
            5,
            0,
            5,
            0
        ],
        [
            "TUR001",
            "Türkçe 1",
            2,
            0,
            2,
            0
        ],
        [
            "ENG301",
            "İleri İngilizce 1",
            2,
            0,
            2,
            0
        ],
        [
            "VWLSD",
            "Seçimlik Ders I",
            15,
            0,
            15,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "VWL304",
            "İktisadi Düşünceler Tarihi",
            5,
            0,
            5,
            0
        ],
        [
            "VWL306",
            "İktisadi Büyüme ve Kalkınma",
            5,
            1,
            5,
            0
        ],
        [
            "VWL308",
            "Sanayi ve Yenilik İktisadı",
            5,
            0,
            5,
            0
        ],
        [
            "VWL310",
            "Seminer",
            5,
            0,
            5,
            0
        ],
        [
            "TUR002",
            "Türkçe 2",
            2,
            0,
            2,
            0
        ],
        [
            "ENG302",
            "İleri İngilizce 2",
            2,
            0,
            2,
            0
        ],
        [
            "VWLSD",
            "Seçimlik Ders",
            6,
            0,
            6,
            0
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "VWLSD",
            "Seçimlik Ders VII",
            24,
            0,
            24,
            0
        ],
        [
            "AIT001",
            "Atatürk İlkeleri ve İnkilapları Tarihi 1",
            2,
            0,
            2,
            0
        ],
        [
            "ÜSD",
            "Üniversite Seçmeli Ders Havuzu",
            4,
            0,
            4,
            0
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "VWL410",
            "Bitirme Tezi ve Kollogyum",
            12,
            0,
            12,
            0
        ],
        [
            "AIT002",
            "Atatürk İlkeleri ve İnkılapları Tarihi II",
            2,
            0,
            2,
            0
        ],
        [
            "VWLSD",
            "Seçimlik Ders XI",
            16,
            0,
            16,
            0
        ]
    ]
},
        "pool_codes": {
    "VWLSD": [
        "Uluslararası İktisat II",
        "Davranışsal İktisat",
        "SAP Uygulamaları",
        "Para Teorisi ve Politikası",
        "Tarım İktisadı",
        "Çalışma İktisadı",
        "Eğitim İktisadı",
        "Dijital Ekonomi",
        "Enerji İktisadı",
        "Cinsiyet İktisadı",
        "Sağlık İktisadı",
        "Kurumsal İktisat",
        "Türkiye Ekonomisinin Gelişimi",
        "Dünya Ekonomisinin Güncel Sorunları",
        "Staj",
        "Uluslararası Finans",
        "İslami Finans",
        "İslam İktisadı",
        "Kültür ve Sanat İktisadı",
        "Turizm İktisadı",
        "Finansal Aracılık ve Finans Mühendisliği",
        "Sunum ve Görselleştirme",
        "Bilimde Sözlü ve Yazılı İletişim",
        "Ekonomistler İçin İngilizce",
        "Metin Madenciliği",
        "Uluslararası Ekonomi Politik",
        "Medeni Hukuka Giriş ve Başlangıç Hükümleri",
        "Yatırım ve Finansman",
        "Pazarlamanın Temelleri",
        "Yöneylem Araştırması",
        "Demografi ve Göç İktisadı",
        "Avrupa Bütünleşmesinin İktisadı",
        "Coğrafya ve Ulaştırma İktisadı",
        "Spor İktisadı",
        "Bölgesel İktisat",
        "İktisatta Güncel Meseleler 1",
        "İktisatta Güncel Meseleler 2",
        "Akademik Yazma Becerileri",
        "İktisatta Güncel Meseleler 3",
        "İktisatta Güncel Meseleler 4",
        "İktisatta Güncel Meseleler 5",
        "İktisatta Güncel Meseleler 6",
        "Ampirik Ekonomi Politikası",
        "R ile Ampirik İktisat",
        "Ampirik İktisat ve İktisat Tarihinde Güncel Konular",
        "Ekonometri ve İktisadi Tarih",
        "Stratejik Pazarlama",
        "Pazar Araştırması",
        "Organizasyon",
        "Dijital Pazarlama",
        "İş Oyunu",
        "İş Etiği",
        "E-iş",
        "Karar Teorisi",
        "Kültürlerarası İletişim",
        "Veritabanı Sistemleri",
        "Dijital Tedarik Zinciri Yönetimi",
        "Veri Madenciliği",
        "Çok Değişkenli İstatistik Yöntemler",
        "İşletmesel Veri Analizi",
        "İşletme Finansı",
        "Akıllı Sistemler",
        "Veri Analizi Yöntemleri",
        "Yapay Sinir Ağları",
        "Makine Öğrenmesi",
        "Derin Öğrenme",
        "Kültürlerarası Yeterlilik Gelişimi",
        "Oyun Tasarımı",
        "Kültürel ve Yaratıcı Sanayiler",
        "Uluslararası Örgütler",
        "Kültür Endüstrisinde Etkinlik Yönetimi",
        "Almanya, Türkiye ve AB için Güncel Siyasal Sorunlar",
        "Türkiye-Avrupa İlişkilerinde Güncel Meseleler",
        "Küresel İklim Politikası",
        "Türk-Alman İlişkileri: Ekonomi, Politika ve Toplum",
        "Türk Ekonomisinin Kurumları",
        "Küreselleşme",
        "Uluslararası Enerji Siyaseti",
        "Kentleşme Sorunları",
        "Marka Yönetimi",
        "Kurumsal 2.0"
    ],
    "ÜSD": [
        "Akademik İngilizce",
        "İngilizce Etkili İletişim Becerileri",
        "Türkiye'de Gündelik Hayatın Tarihi",
        "Yabancılar İçin Türkçe I",
        "Yabancılar İçin Akademik Türkçe I",
        "Kariyer Planlama",
        "Fikri Mülkiyet Hukuku",
        "Etik",
        "Felsefe Tarihi",
        "İktisadi Tarih",
        "Osmanlıca I",
        "Pazarlamanın Temelleri",
        "Stratejik Yönetim",
        "Bilgisayar Bilimine ve Programlamaya Giriş",
        "Elektrik Elektronik Mühendisliğine Giriş",
        "Sayısal Tasarım",
        "Endüstri Mühendisliğine Giriş",
        "İnşaat Mühendisliğine Giriş",
        "Makine Mühendisliğine Giriş",
        "Statik",
        "Malzeme Teknolojisi I",
        "Doğal Afetler",
        "Doğa Bilimlerinde Girişimcilik",
        "Kuantum Enerjisi Sistemlerine Giriş",
        "Doğa Bilimlerinde Yapay Zeka"
    ]
},
        "pools": {
    "VWLSD": [
        [
            "VWL302",
            "Uluslararası İktisat II",
            6,
            3,
            1,
            0
        ],
        [
            "VWL305",
            "Davranışsal İktisat",
            5,
            3,
            0,
            0
        ],
        [
            "VWL321",
            "SAP Uygulamaları",
            5,
            2,
            2,
            0
        ],
        [
            "VWL323",
            "Para Teorisi ve Politikası",
            5,
            2,
            0,
            0
        ],
        [
            "VWL325",
            "Tarım İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL327",
            "Çalışma İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL329",
            "Eğitim İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL331",
            "Dijital Ekonomi",
            5,
            2,
            0,
            0
        ],
        [
            "VWL322",
            "Enerji İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL324",
            "Cinsiyet İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL326",
            "Sağlık İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL328",
            "Kurumsal İktisat",
            5,
            2,
            0,
            0
        ],
        [
            "VWL419",
            "Türkiye Ekonomisinin Gelişimi",
            6,
            3,
            0,
            0
        ],
        [
            "VWL421",
            "Dünya Ekonomisinin Güncel Sorunları",
            6,
            3,
            0,
            0
        ],
        [
            "VWL423",
            "Staj",
            6,
            0,
            0,
            0
        ],
        [
            "VWL425",
            "Uluslararası Finans",
            5,
            2,
            0,
            0
        ],
        [
            "VWL427",
            "İslami Finans",
            5,
            2,
            0,
            0
        ],
        [
            "VWL429",
            "İslam İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL431",
            "Kültür ve Sanat İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL433",
            "Turizm İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL435",
            "Finansal Aracılık ve Finans Mühendisliği",
            5,
            2,
            0,
            0
        ],
        [
            "VWL437",
            "Sunum ve Görselleştirme",
            6,
            2,
            2,
            0
        ],
        [
            "VWL441",
            "Bilimde Sözlü ve Yazılı İletişim",
            6,
            2,
            2,
            0
        ],
        [
            "VWL443",
            "Ekonomistler İçin İngilizce",
            6,
            2,
            2,
            0
        ],
        [
            "VWL445",
            "Metin Madenciliği",
            6,
            2,
            2,
            0
        ],
        [
            "VWL447",
            "Uluslararası Ekonomi Politik",
            5,
            3,
            0,
            0
        ],
        [
            "VWL449",
            "Medeni Hukuka Giriş ve Başlangıç Hükümleri",
            5,
            2,
            0,
            0
        ],
        [
            "VWL451",
            "Yatırım ve Finansman",
            6,
            2,
            2,
            0
        ],
        [
            "VWL453",
            "Pazarlamanın Temelleri",
            5,
            2,
            0,
            0
        ],
        [
            "VWL455",
            "Yöneylem Araştırması",
            6,
            2,
            0,
            0
        ],
        [
            "VWL426",
            "Demografi ve Göç İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL428",
            "Avrupa Bütünleşmesinin İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL430",
            "Coğrafya ve Ulaştırma İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL432",
            "Spor İktisadı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL434",
            "Bölgesel İktisat",
            5,
            2,
            0,
            0
        ],
        [
            "VWL436",
            "İktisatta Güncel Meseleler 1",
            5,
            2,
            2,
            0
        ],
        [
            "VWL438",
            "İktisatta Güncel Meseleler 2",
            5,
            2,
            2,
            0
        ],
        [
            "VWL439",
            "Akademik Yazma Becerileri",
            6,
            2,
            2,
            0
        ],
        [
            "VWL440",
            "İktisatta Güncel Meseleler 3",
            5,
            2,
            2,
            0
        ],
        [
            "VWL442",
            "İktisatta Güncel Meseleler 4",
            5,
            2,
            2,
            0
        ],
        [
            "VWL444",
            "İktisatta Güncel Meseleler 5",
            5,
            2,
            2,
            0
        ],
        [
            "VWL446",
            "İktisatta Güncel Meseleler 6",
            5,
            2,
            2,
            0
        ],
        [
            "VWL448",
            "Ampirik Ekonomi Politikası",
            5,
            2,
            0,
            0
        ],
        [
            "VWL450",
            "R ile Ampirik İktisat",
            5,
            2,
            2,
            0
        ],
        [
            "VWL452",
            "Ampirik İktisat ve İktisat Tarihinde Güncel Konular",
            5,
            2,
            0,
            0
        ],
        [
            "VWL454",
            "Ekonometri ve İktisadi Tarih",
            5,
            2,
            0,
            0
        ],
        [
            "VWL456",
            "Stratejik Pazarlama",
            5,
            2,
            0,
            0
        ],
        [
            "VWL457",
            "Pazar Araştırması",
            6,
            2,
            2,
            0
        ],
        [
            "VWL458",
            "Organizasyon",
            5,
            2,
            0,
            0
        ],
        [
            "VWL459",
            "Dijital Pazarlama",
            6,
            2,
            2,
            0
        ],
        [
            "VWL460",
            "İş Oyunu",
            5,
            4,
            0,
            0
        ],
        [
            "VWL461",
            "İş Etiği",
            3,
            1,
            1,
            0
        ],
        [
            "VWL462",
            "E-iş",
            5,
            2,
            0,
            0
        ],
        [
            "VWL463",
            "Karar Teorisi",
            6,
            2,
            2,
            0
        ],
        [
            "VWL464",
            "Kültürlerarası İletişim",
            5,
            2,
            0,
            0
        ],
        [
            "VWL465",
            "Veritabanı Sistemleri",
            6,
            2,
            2,
            0
        ],
        [
            "VWL466",
            "Dijital Tedarik Zinciri Yönetimi",
            6,
            2,
            2,
            0
        ],
        [
            "VWL467",
            "Veri Madenciliği",
            6,
            2,
            2,
            0
        ],
        [
            "VWL468",
            "Çok Değişkenli İstatistik Yöntemler",
            6,
            2,
            2,
            0
        ],
        [
            "VWL469",
            "İşletmesel Veri Analizi",
            6,
            2,
            2,
            0
        ],
        [
            "VWL470",
            "İşletme Finansı",
            6,
            2,
            2,
            0
        ],
        [
            "VWL471",
            "Akıllı Sistemler",
            6,
            2,
            2,
            0
        ],
        [
            "VWL472",
            "Veri Analizi Yöntemleri",
            6,
            2,
            2,
            0
        ],
        [
            "VWL473",
            "Yapay Sinir Ağları",
            6,
            2,
            2,
            0
        ],
        [
            "VWL474",
            "Makine Öğrenmesi",
            6,
            2,
            2,
            0
        ],
        [
            "VWL475",
            "Derin Öğrenme",
            6,
            2,
            2,
            0
        ],
        [
            "VWL476",
            "Kültürlerarası Yeterlilik Gelişimi",
            6,
            2,
            2,
            0
        ],
        [
            "VWL477",
            "Oyun Tasarımı",
            5,
            2,
            0,
            0
        ],
        [
            "VWL478",
            "Kültürel ve Yaratıcı Sanayiler",
            4,
            2,
            1,
            0
        ],
        [
            "VWL480",
            "Uluslararası Örgütler",
            5,
            3,
            0,
            0
        ],
        [
            "VWL481",
            "Kültür Endüstrisinde Etkinlik Yönetimi",
            4,
            3,
            0,
            0
        ],
        [
            "VWL482",
            "Almanya, Türkiye ve AB için Güncel Siyasal Sorunlar",
            5,
            3,
            0,
            0
        ],
        [
            "VWL483",
            "Türkiye-Avrupa İlişkilerinde Güncel Meseleler",
            3,
            2,
            0,
            0
        ],
        [
            "VWL484",
            "Küresel İklim Politikası",
            5,
            3,
            0,
            0
        ],
        [
            "VWL485",
            "Türk-Alman İlişkileri: Ekonomi, Politika ve Toplum",
            5,
            3,
            0,
            0
        ],
        [
            "VWL486",
            "Türk Ekonomisinin Kurumları",
            5,
            2,
            0,
            0
        ],
        [
            "VWL487",
            "Küreselleşme",
            5,
            3,
            0,
            0
        ],
        [
            "VWL489",
            "Uluslararası Enerji Siyaseti",
            5,
            3,
            0,
            0
        ],
        [
            "VWL491",
            "Kentleşme Sorunları",
            5,
            3,
            0,
            0
        ],
        [
            "VWL493",
            "Marka Yönetimi",
            6,
            2,
            2,
            0
        ],
        [
            "VWL495",
            "Kurumsal 2.0",
            6,
            2,
            2,
            0
        ]
    ],
    "ÜSD": [
        [
            "ÜSDENG111",
            "Akademik İngilizce",
            4,
            3,
            0,
            0
        ],
        [
            "ÜSDENG120",
            "İngilizce Etkili İletişim Becerileri",
            3,
            3,
            0,
            0
        ],
        [
            "ÜSDTGH111",
            "Türkiye'de Gündelik Hayatın Tarihi",
            2,
            2,
            0,
            0
        ],
        [
            "ÜSDTUR111",
            "Yabancılar İçin Türkçe I",
            2,
            2,
            0,
            0
        ],
        [
            "ÜSDTUR009",
            "Yabancılar İçin Akademik Türkçe I",
            2,
            2,
            0,
            0
        ],
        [
            "ÜSDKAR123",
            "Kariyer Planlama",
            2,
            2,
            0,
            0
        ],
        [
            "HUK553",
            "Fikri Mülkiyet Hukuku",
            2,
            2,
            0,
            0
        ],
        [
            "HUK175",
            "Etik",
            2,
            2,
            0,
            0
        ],
        [
            "KKW321",
            "Felsefe Tarihi",
            4,
            3,
            0,
            0
        ],
        [
            "VWL203",
            "İktisadi Tarih",
            3,
            6,
            0,
            0
        ],
        [
            "POL550",
            "Osmanlıca I",
            3,
            3,
            0,
            0
        ],
        [
            "BWL201",
            "Pazarlamanın Temelleri",
            4,
            5,
            0,
            0
        ],
        [
            "BWL309",
            "Stratejik Yönetim",
            3,
            4,
            0,
            0
        ],
        [
            "INF101",
            "Bilgisayar Bilimine ve Programlamaya Giriş",
            6,
            2,
            0,
            0
        ],
        [
            "ETE091",
            "Elektrik Elektronik Mühendisliğine Giriş",
            2,
            2,
            1,
            0
        ],
        [
            "ETE101",
            "Sayısal Tasarım",
            4,
            6,
            0,
            0
        ],
        [
            "WIN091",
            "Endüstri Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "BAU091",
            "İnşaat Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "MAB091",
            "Makine Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "MAB109",
            "Statik",
            5,
            6,
            0,
            0
        ],
        [
            "MAB207",
            "Malzeme Teknolojisi I",
            5,
            6,
            0,
            0
        ],
        [
            "ÜSDBAU449",
            "Doğal Afetler",
            2,
            2,
            0,
            0
        ],
        [
            "ÜSDMBT371",
            "Doğa Bilimlerinde Girişimcilik",
            3,
            2,
            0,
            0
        ],
        [
            "EBT309",
            "Kuantum Enerjisi Sistemlerine Giriş",
            6,
            3,
            0,
            0
        ],
        [
            "ÜSDMBT369",
            "Doğa Bilimlerinde Yapay Zeka",
            3,
            2,
            0,
            0
        ]
    ]
},
        "pool_metadata": {
    "VWLSD": {},
    "ÜSD": {}
},
    },
    "İnşaat Müh": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "MAT103",
            "Analiz I",
            6,
            3,
            2,
            0
        ],
        [
            "BAU109",
            "Statik",
            6,
            3,
            2,
            0
        ],
        [
            "DEU121",
            "Teknik Almanca I",
            2,
            2,
            0,
            0
        ],
        [
            "BAU107",
            "Tasarım Teknikleri",
            6,
            1,
            2,
            1
        ],
        [
            "PHY103",
            "Modern Fizik",
            6,
            3,
            1,
            1
        ],
        [
            "ENG101",
            "İngilizce I",
            2,
            3,
            0,
            0
        ],
        [
            "BAU091",
            "İnşaat Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "MAT108",
            "Analiz II",
            6,
            3,
            2,
            0
        ],
        [
            "MAT106",
            "Lineer Cebir",
            6,
            2,
            2,
            1
        ],
        [
            "DEU122",
            "Teknik Almanca II",
            2,
            2,
            0,
            0
        ],
        [
            "BAU102",
            "Taşıyıcı Sistemlerin Esasları ve Yapı Tasarımı",
            6,
            3,
            1,
            0
        ],
        [
            "BAU092",
            "Bilimsel Çalışma Yöntemleri",
            2,
            2,
            0,
            0
        ],
        [
            "ENG102",
            "İngilizce II",
            2,
            3,
            0,
            0
        ],
        [
            "BAU112",
            "Mukavemet",
            6,
            3,
            2,
            0
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "MAT201",
            "Diferansiyel Denklemler",
            6,
            2,
            1,
            1
        ],
        [
            "BAU209",
            "Kinematik ve Dinamik",
            6,
            3,
            1,
            1
        ],
        [
            "ENG201",
            "İngilizce III",
            2,
            3,
            0,
            0
        ],
        [
            "AIT001",
            "Atatürk İlkeleri ve İnkılap Tarihi I",
            2,
            2,
            0,
            0
        ],
        [
            "BAU201",
            "Yapı Malzemesi ve Kimyası I",
            6,
            2,
            1,
            2
        ],
        [
            "BAU202",
            "Yapı Statiği I",
            6,
            3,
            1,
            1
        ],
        [
            "TUR001",
            "Türkçe I",
            2,
            2,
            0,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "AIT002",
            "Atatürk İlkeleri ve İnkılap Tarihi II",
            2,
            2,
            0,
            0
        ],
        [
            "ENG202",
            "İngilizce IV",
            2,
            3,
            0,
            0
        ],
        [
            "BAU203",
            "Yapı İnşaatı I",
            6,
            3,
            1,
            1
        ],
        [
            "BAU204",
            "Yapı Statiği II",
            6,
            3,
            1,
            1
        ],
        [
            "BAU205",
            "Akışkanlar Mekaniği",
            6,
            2,
            2,
            1
        ],
        [
            "BAU206",
            "Yapı Malzemesi ve Kimyası II",
            2,
            2,
            1,
            2
        ],
        [
            "ZSD",
            "Zorunlu Seçmeli Ders Alanı",
            6,
            0,
            0,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "BAU301",
            "Yapı İnşaatı II",
            6,
            4,
            2,
            0
        ],
        [
            "BAU303",
            "Ulaştırma",
            6,
            2,
            2,
            0
        ],
        [
            "ZSD",
            "Zorunlu Seçmeli Ders Alanı",
            18,
            0,
            0,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "ZSD",
            "Zorunlu Seçmeli Ders Alanı",
            24,
            0,
            0,
            0
        ],
        [
            "BAU302",
            "Zemin Mekaniği ve Temel İnşaatı I",
            6,
            2,
            2,
            1
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "ENG301",
            "İleri İngilizce I",
            2,
            3,
            0,
            0
        ],
        [
            "BSP201",
            "Şantiye Stajı",
            4,
            0,
            0,
            0
        ],
        [
            "ISG001",
            "İş Sağlığı ve Güvenliği I",
            2,
            2,
            0,
            0
        ],
        [
            "BUP403",
            "Büro Stajı",
            4,
            0,
            0,
            0
        ],
        [
            "ZSD",
            "Zorunlu Seçmeli Ders Alanı",
            6,
            0,
            0,
            0
        ],
        [
            "USD000",
            "Üniversite Seçmeli Ders Havuzu",
            6,
            0,
            0,
            0
        ],
        [
            "BAU304",
            "Zemin Mekaniği ve Temel İnşaatı II",
            6,
            2,
            2,
            1
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "ZSD",
            "Zorunlu Seçmeli Ders Alanı",
            10,
            0,
            0,
            0
        ],
        [
            "TUR002",
            "Türkçe II",
            2,
            2,
            0,
            0
        ],
        [
            "ISG002",
            "İş Sağlığı ve Güvenliği II",
            2,
            2,
            0,
            0
        ],
        [
            "BAU492",
            "Lisans Bitirme Çalışması",
            12,
            0,
            0,
            0
        ],
        [
            "ENG302",
            "İleri İngilizce II",
            2,
            3,
            0,
            0
        ]
    ]
},
        "pool_codes": {
    "ZSD": [
        "İnşaat Müh. Nümerik Metotlar",
        "Yapı Fiziği",
        "Veri Analizinin İstatistiksel Yöntemleri",
        "Stokastik Sistemler",
        "İnşaat Mühendisliği Projesi I",
        "Sistem Tekniği",
        "Bilg. Bil. ve Prog. Giriş",
        "Nesnel Programlama",
        "Yapı İşletmesi I",
        "İnşaat Hukuku",
        "Yapı Statiği III",
        "Geodezi ve Geoinformatik",
        "İnş. Müh. için Jeoloji",
        "İnşaat Bilişimi",
        "Su Kaynakları",
        "Kentsel Su Yönetiminin Temelleri",
        "İnş. Müh. 3D Hesaplamalar",
        "Yapı İnşaatı III",
        "Yapı İşletmesi II",
        "Yapı Kimyası ve Malzeme Testi",
        "Yapı Dinamiği I",
        "Demiryolu Ulaşımının Temelleri",
        "Yapı Bilgi Modellemesi",
        "Mühendisler İçin Hukuk",
        "İnşaat Mühendisliği Projesi II",
        "Deneysel Zemin Mekaniği",
        "Yığma Yapı İnşaatı",
        "Beton ve Kalıp İnşaatı",
        "Taşkın Koruma ve Yönetimi",
        "Liman Tasarımı",
        "Kıyı Mühendisliğine Giriş",
        "Kaya Mekaniği",
        "İşletmede Mesleki Eğitim",
        "Girişimcilik"
    ]
},
        "pools": {
    "ZSD": [
        [
            "BAU251",
            "İnşaat Müh. Nümerik Metotlar",
            6,
            2,
            1,
            2
        ],
        [
            "BAU252",
            "Yapı Fiziği",
            6,
            3,
            1,
            1
        ],
        [
            "MAT204",
            "Veri Analizinin İstatistiksel Yöntemleri",
            6,
            2,
            2,
            1
        ],
        [
            "BAU305",
            "Stokastik Sistemler",
            6,
            3,
            2,
            0
        ],
        [
            "BAU350",
            "İnşaat Mühendisliği Projesi I",
            6,
            2,
            2,
            0
        ],
        [
            "BAU351",
            "Sistem Tekniği",
            6,
            2,
            2,
            0
        ],
        [
            "INF101",
            "Bilg. Bil. ve Prog. Giriş",
            6,
            2,
            1,
            2
        ],
        [
            "INF102",
            "Nesnel Programlama",
            6,
            2,
            0,
            2
        ],
        [
            "BAU352",
            "Yapı İşletmesi I",
            6,
            3,
            1,
            1
        ],
        [
            "BAU353",
            "İnşaat Hukuku",
            6,
            2,
            1,
            0
        ],
        [
            "BAU354",
            "Yapı Statiği III",
            6,
            3,
            1,
            1
        ],
        [
            "BAU355",
            "Geodezi ve Geoinformatik",
            6,
            3,
            1,
            1
        ],
        [
            "BAU356",
            "İnş. Müh. için Jeoloji",
            6,
            3,
            1,
            1
        ],
        [
            "BAU357",
            "İnşaat Bilişimi",
            6,
            3,
            1,
            1
        ],
        [
            "BAU451",
            "Su Kaynakları",
            6,
            3,
            2,
            0
        ],
        [
            "BAU452",
            "Kentsel Su Yönetiminin Temelleri",
            6,
            3,
            2,
            0
        ],
        [
            "BAU453",
            "İnş. Müh. 3D Hesaplamalar",
            6,
            1,
            2,
            2
        ],
        [
            "BAU454",
            "Yapı İnşaatı III",
            6,
            3,
            1,
            1
        ],
        [
            "BAU456",
            "Yapı İşletmesi II",
            6,
            3,
            1,
            1
        ],
        [
            "BAU457",
            "Yapı Kimyası ve Malzeme Testi",
            6,
            3,
            1,
            1
        ],
        [
            "BAU458",
            "Yapı Dinamiği I",
            6,
            3,
            1,
            1
        ],
        [
            "BAU459",
            "Demiryolu Ulaşımının Temelleri",
            6,
            2,
            2,
            0
        ],
        [
            "BAU460",
            "Yapı Bilgi Modellemesi",
            6,
            2,
            2,
            0
        ],
        [
            "ING406",
            "Mühendisler İçin Hukuk",
            6,
            3,
            1,
            0
        ],
        [
            "BAU402",
            "İnşaat Mühendisliği Projesi II",
            4,
            2,
            2,
            0
        ],
        [
            "BAU425",
            "Deneysel Zemin Mekaniği",
            2,
            0,
            0,
            2
        ],
        [
            "BAU461",
            "Yığma Yapı İnşaatı",
            6,
            3,
            2,
            0
        ],
        [
            "BAU463",
            "Beton ve Kalıp İnşaatı",
            6,
            2,
            3,
            0
        ],
        [
            "BAU462",
            "Taşkın Koruma ve Yönetimi",
            6,
            3,
            2,
            0
        ],
        [
            "BAU464",
            "Liman Tasarımı",
            6,
            3,
            2,
            0
        ],
        [
            "BAU465",
            "Kıyı Mühendisliğine Giriş",
            6,
            3,
            2,
            0
        ],
        [
            "BAU466",
            "Kaya Mekaniği",
            6,
            3,
            2,
            0
        ],
        [
            "BAU490",
            "İşletmede Mesleki Eğitim",
            6,
            3,
            2,
            0
        ],
        [
            "ING404",
            "Girişimcilik",
            2,
            2,
            0,
            0
        ]
    ]
},
        "pool_metadata": {
    "ZSD": {}
},
    },
    "İşletme": {
        "curriculum": {
    "1. Dönem / 1. Yıl Güz Dönemi": [
        [
            "BWL101",
            "İşletmeye Giriş",
            6,
            2,
            6,
            0
        ],
        [
            "BWL103",
            "Muhasebe",
            5,
            2,
            5,
            0
        ],
        [
            "BWL105",
            "İstatistik 1",
            6,
            2,
            6,
            0
        ],
        [
            "BWL109",
            "Matematik 1",
            6,
            2,
            6,
            0
        ],
        [
            "HUK911",
            "Hukuka Giriş",
            3,
            0,
            3,
            0
        ],
        [
            "DEU111",
            "İşletme Almancası 1",
            2,
            0,
            2,
            0
        ],
        [
            "ENG101",
            "İngilizce 1",
            2,
            0,
            2,
            0
        ]
    ],
    "2. Dönem / 1. Yıl Bahar Dönemi": [
        [
            "BWL104",
            "Bilanço",
            5,
            2,
            5,
            0
        ],
        [
            "BWL106",
            "İstatistik 2",
            6,
            2,
            6,
            0
        ],
        [
            "BWL102",
            "Organizasyon",
            3,
            0,
            3,
            0
        ],
        [
            "BWL112",
            "Matematik 2",
            6,
            2,
            6,
            0
        ],
        [
            "VWL182",
            "İktisada Giriş",
            6,
            2,
            6,
            0
        ],
        [
            "DEU112",
            "İşletme Almancası 2",
            2,
            0,
            2,
            0
        ],
        [
            "ENG102",
            "İngilizce 2",
            2,
            0,
            2,
            0
        ]
    ],
    "3. Dönem / 2. Yıl Güz Dönemi": [
        [
            "BWL201",
            "Pazarlamanın Temelleri",
            5,
            2,
            5,
            0
        ],
        [
            "BWL205",
            "Maliyet Muhasebesi",
            6,
            2,
            6,
            0
        ],
        [
            "BWL207",
            "Yatırım ve Finansman",
            6,
            2,
            6,
            0
        ],
        [
            "BWL211",
            "İşletme Enformatiğinin Temelleri",
            3,
            0,
            3,
            0
        ],
        [
            "VWL281",
            "Mikroiktisat",
            5,
            2,
            5,
            0
        ],
        [
            "ENG201",
            "İngilizce 3",
            2,
            0,
            2,
            0
        ],
        [
            "BSD",
            "Seçmeli Ders",
            3,
            0,
            3,
            0
        ]
    ],
    "4. Dönem / 2. Yıl Bahar Dönemi": [
        [
            "BWL216",
            "E-İş",
            3,
            0,
            3,
            0
        ],
        [
            "BWL210",
            "İşletme Finansı",
            6,
            2,
            6,
            0
        ],
        [
            "BWL212",
            "Stratejik Pazarlama",
            5,
            2,
            5,
            0
        ],
        [
            "BWL214",
            "Operasyon Yönetimi",
            5,
            2,
            5,
            0
        ],
        [
            "VWL282",
            "Makroiktisat",
            6,
            2,
            6,
            0
        ],
        [
            "ENG202",
            "İngilizce 4",
            2,
            0,
            2,
            0
        ],
        [
            "BSD",
            "Seçmeli Ders",
            3,
            0,
            3,
            0
        ]
    ],
    "5. Dönem / 3. Yıl Güz Dönemi": [
        [
            "BWL309",
            "Stratejik Yönetim",
            4,
            1,
            4,
            0
        ],
        [
            "ENG303",
            "İleri İngilizce 1",
            3,
            0,
            3,
            0
        ],
        [
            "TUR001",
            "Türkçe 1",
            2,
            0,
            2,
            0
        ],
        [
            "SD",
            "Seçmeli Ders (SD) {SECIM:2}",
            12,
            0,
            12,
            0
        ],
        [
            "BSD",
            "Seçmeli Ders",
            3,
            0,
            3,
            0
        ],
        [
            "SD",
            "Seçmeli Ders",
            6,
            0,
            6,
            0
        ]
    ],
    "6. Dönem / 3. Yıl Bahar Dönemi": [
        [
            "BWL310",
            "Bilimsel Çalışma Yöntemleri",
            4,
            1,
            4,
            0
        ],
        [
            "BWL314",
            "Yönetim Muhasebesi",
            6,
            2,
            6,
            0
        ],
        [
            "TUR002",
            "Türkçe 2",
            2,
            0,
            2,
            0
        ],
        [
            "ENG304",
            "İleri İngilizce 2",
            3,
            0,
            3,
            0
        ],
        [
            "SD",
            "Seçmeli Ders (SD) {SECIM:2}",
            12,
            0,
            12,
            0
        ],
        [
            "BSD",
            "Seçmeli Ders",
            3,
            0,
            3,
            0
        ]
    ],
    "7. Dönem / 4. Yıl Güz Dönemi": [
        [
            "BWL407",
            "İşletme Stajı (BWL-Praktikum)*",
            5,
            0,
            5,
            0
        ],
        [
            "BWL417",
            "İş Etiği",
            3,
            1,
            3,
            0
        ],
        [
            "BWL413",
            "Pazar Araştırması",
            5,
            2,
            5,
            0
        ],
        [
            "BWL415",
            "Yöneylem Araştırması",
            6,
            2,
            6,
            0
        ],
        [
            "AIT001",
            "Atatürk İlkeleri ve Inkılap Tarihi 1",
            2,
            0,
            2,
            0
        ],
        [
            "SD",
            "Seçmeli Ders",
            6,
            0,
            6,
            0
        ],
        [
            "ÜSD000",
            "Üniversite Seçmeli Ders Havuzu",
            3,
            0,
            3,
            0
        ]
    ],
    "8. Dönem / 4. Yıl Bahar Dönemi": [
        [
            "BWL408",
            "Bitirme Tezi",
            10,
            0,
            10,
            0
        ],
        [
            "AIT002",
            "Atatürk İlkeleri ve Inkılap Tarihi 2",
            2,
            0,
            2,
            0
        ],
        [
            "SD",
            "Seçmeli Ders (SD) (Ders 1)** {SECIM:3}",
            18,
            0,
            18,
            0
        ]
    ]
},
        "pool_codes": {
    "BSD": [
        "Borçlar Hukuku (Schuldrecht)**",
        "Vergi Hukuku (Steuerrecht)***",
        "İş Hukuku (Labour Law)****",
        "Ticaret Hukuku (Commercial Law)*****",
        "Avrupa Ticaret Hukuku",
        "Sayısal Yöntemlerde Güncel Konular",
        "Muhasebe ve Finansta Güncel Konular",
        "Pazarlamada Güncel Konular",
        "Örgütsel Davranışta Güncel Konular",
        "Uluslararası Öğrenci Projesi"
    ],
    "SD": [
        "İşletme Semineri",
        "Karar Teorisi",
        "Çok Değişkenli İstatistik Yöntemler",
        "Bilgisayar Destekli İstatistik",
        "Lineer Cebir",
        "Ekonometri",
        "İşletmelerde Simülasyon Teknikleri",
        "Algoritma ve Programlamaya Giriş",
        "İşletmesel Veri Analizi",
        "Dijital Pazarlama",
        "Lojistik",
        "Tedarik Zinciri Yönetimi",
        "Hizmet Pazarlama",
        "Tüketici Davranışı",
        "Marka Yönetimi",
        "Uluslararası Muhasebe Standartları",
        "Sermaye Piyasaları",
        "İşletme Vergiciliği",
        "Mali Tablolar Analizi",
        "Denetim",
        "Para ve Banka",
        "Davranışsal Finans",
        "Örgütsel Davranış",
        "Girişimcilik",
        "Proje Yönetimi",
        "Kültürlerarası Yetkinlik Geliştirme",
        "Personel Yönetimi",
        "Uluslararası İşletme",
        "Kurumsal Kaynak Planlaması",
        "Şirket 2.0",
        "Süreç Yönetimi",
        "Blockchain ve Kripto Piyasaları",
        "İş Oyunu Semineri",
        "İnovasyon ve Yeni Ürün Geliştirme",
        "Örgütsel Dönüşüm Yönetimi",
        "Kültürlerarası Liderlik",
        "SAP Uygulamaları",
        "İşletmede Mesleki Eğitim (Vocational Education) ***"
    ],
    "ÜSD000": [
        "Akademik İngilizce",
        "Türkiye'de Gündelik Hayatın Tarihi",
        "Kariyer Planlama",
        "Fikrî Mülkiyet Hukuku",
        "İktisadi Tarih",
        "Felsefe Tarihi",
        "Bilgisayar Bilimine ve Programlamaya Giriş",
        "Elektrik Elektronik Mühendisliğine Giriş",
        "Endüstri Mühendisliğine Giriş",
        "İnşaat Mühendisliğine Giriş",
        "Statik",
        "Doğa Bilimlerinde Girişimcilik",
        "Kuantum Enerjisi Sistemlerine Giriş",
        "Kamu Maliyesi ve Kamu İktisadı"
    ]
},
        "pools": {
    "SD": [
        [
            "BWL011",
            "İşletme Semineri",
            6,
            1,
            1,
            0
        ],
        [
            "BWL017",
            "Karar Teorisi",
            6,
            1,
            1,
            0
        ],
        [
            "BWL018",
            "Çok Değişkenli İstatistik Yöntemler",
            6,
            1,
            1,
            0
        ],
        [
            "BWL019",
            "Bilgisayar Destekli İstatistik",
            6,
            1,
            1,
            0
        ],
        [
            "BWL020",
            "Lineer Cebir",
            6,
            1,
            1,
            0
        ],
        [
            "BWL021",
            "Ekonometri",
            6,
            1,
            1,
            0
        ],
        [
            "BWL022",
            "İşletmelerde Simülasyon Teknikleri",
            6,
            1,
            1,
            0
        ],
        [
            "BWL036",
            "Algoritma ve Programlamaya Giriş",
            6,
            1,
            1,
            0
        ],
        [
            "BWL037",
            "İşletmesel Veri Analizi",
            6,
            1,
            1,
            0
        ],
        [
            "BWL007",
            "Dijital Pazarlama",
            6,
            1,
            1,
            0
        ],
        [
            "BWL023",
            "Lojistik",
            6,
            1,
            1,
            0
        ],
        [
            "BWL005",
            "Tedarik Zinciri Yönetimi",
            6,
            1,
            1,
            0
        ],
        [
            "BWL010",
            "Hizmet Pazarlama",
            6,
            1,
            1,
            0
        ],
        [
            "BWL014",
            "Tüketici Davranışı",
            6,
            1,
            1,
            0
        ],
        [
            "BWL009",
            "Marka Yönetimi",
            6,
            1,
            1,
            0
        ],
        [
            "BWL024",
            "Uluslararası Muhasebe Standartları",
            6,
            1,
            1,
            0
        ],
        [
            "BWL012",
            "Sermaye Piyasaları",
            6,
            1,
            1,
            0
        ],
        [
            "BWL025",
            "İşletme Vergiciliği",
            6,
            1,
            1,
            0
        ],
        [
            "BWL026",
            "Mali Tablolar Analizi",
            6,
            1,
            1,
            0
        ],
        [
            "BWL027",
            "Denetim",
            6,
            1,
            1,
            0
        ],
        [
            "BWL028",
            "Para ve Banka",
            6,
            1,
            1,
            0
        ],
        [
            "BWL029",
            "Davranışsal Finans",
            6,
            1,
            1,
            0
        ],
        [
            "BWL030",
            "Örgütsel Davranış",
            6,
            1,
            1,
            0
        ],
        [
            "BWL002",
            "Girişimcilik",
            6,
            1,
            1,
            0
        ],
        [
            "BWL031",
            "Proje Yönetimi",
            6,
            1,
            1,
            0
        ],
        [
            "BWL032",
            "Kültürlerarası Yetkinlik Geliştirme",
            6,
            1,
            1,
            0
        ],
        [
            "BWL033",
            "Personel Yönetimi",
            6,
            1,
            1,
            0
        ],
        [
            "BWL034",
            "Uluslararası İşletme",
            6,
            1,
            1,
            0
        ],
        [
            "BWL035",
            "Kurumsal Kaynak Planlaması",
            6,
            1,
            1,
            0
        ],
        [
            "BWL013",
            "Şirket 2.0",
            6,
            1,
            1,
            0
        ],
        [
            "BWL001",
            "Süreç Yönetimi",
            6,
            1,
            1,
            0
        ],
        [
            "BWL038",
            "Blockchain ve Kripto Piyasaları",
            6,
            1,
            1,
            0
        ],
        [
            "BWL008",
            "İş Oyunu Semineri",
            6,
            1,
            1,
            0
        ],
        [
            "BWL003",
            "İnovasyon ve Yeni Ürün Geliştirme",
            6,
            1,
            1,
            0
        ],
        [
            "BWL004",
            "Örgütsel Dönüşüm Yönetimi",
            6,
            1,
            1,
            0
        ],
        [
            "BWL006",
            "Kültürlerarası Liderlik",
            6,
            1,
            1,
            0
        ],
        [
            "BWL040",
            "SAP Uygulamaları",
            6,
            2,
            0,
            2
        ],
        [
            "BWL039",
            "İşletmede Mesleki Eğitim (Vocational Education) ***",
            18,
            2,
            0,
            0
        ]
    ],
    "BSD": [
        [
            "BWL050",
            "Borçlar Hukuku (Schuldrecht)**",
            3,
            2,
            0,
            0
        ],
        [
            "BWL051",
            "Vergi Hukuku (Steuerrecht)***",
            3,
            2,
            0,
            0
        ],
        [
            "BWL052",
            "İş Hukuku (Labour Law)****",
            3,
            2,
            0,
            0
        ],
        [
            "BWL053",
            "Ticaret Hukuku (Commercial Law)*****",
            3,
            2,
            0,
            0
        ],
        [
            "BWL054",
            "Avrupa Ticaret Hukuku",
            3,
            2,
            0,
            0
        ],
        [
            "BWL055",
            "Sayısal Yöntemlerde Güncel Konular",
            3,
            2,
            0,
            0
        ],
        [
            "BWL056",
            "Muhasebe ve Finansta Güncel Konular",
            3,
            2,
            0,
            0
        ],
        [
            "BWL057",
            "Pazarlamada Güncel Konular",
            3,
            2,
            0,
            0
        ],
        [
            "BWL058",
            "Örgütsel Davranışta Güncel Konular",
            3,
            2,
            0,
            0
        ],
        [
            "BWL059",
            "Uluslararası Öğrenci Projesi",
            3,
            2,
            0,
            0
        ]
    ],
    "ÜSD000": [
        [
            "ÜSDENG111",
            "Akademik İngilizce",
            4,
            3,
            0,
            0
        ],
        [
            "ÜSDTGH 111",
            "Türkiye'de Gündelik Hayatın Tarihi",
            2,
            2,
            0,
            0
        ],
        [
            "ÜSDKAR123",
            "Kariyer Planlama",
            2,
            2,
            0,
            0
        ],
        [
            "HUK477",
            "Fikrî Mülkiyet Hukuku",
            2,
            2,
            0,
            0
        ],
        [
            "VWL203",
            "İktisadi Tarih",
            6,
            3,
            0,
            0
        ],
        [
            "KKW321",
            "Felsefe Tarihi",
            4,
            3,
            0,
            0
        ],
        [
            "INF101",
            "Bilgisayar Bilimine ve Programlamaya Giriş",
            6,
            2,
            2,
            0
        ],
        [
            "ETE091",
            "Elektrik Elektronik Mühendisliğine Giriş",
            2,
            2,
            1,
            0
        ],
        [
            "WIN091",
            "Endüstri Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "BAU091",
            "İnşaat Mühendisliğine Giriş",
            2,
            2,
            0,
            0
        ],
        [
            "MAB109",
            "Statik",
            6,
            3,
            2,
            0
        ],
        [
            "ÜSDMBT371",
            "Doğa Bilimlerinde Girişimcilik",
            3,
            3,
            0,
            0
        ],
        [
            "EBT309",
            "Kuantum Enerjisi Sistemlerine Giriş",
            6,
            3,
            1,
            0
        ],
        [
            "VWL303",
            "Kamu Maliyesi ve Kamu İktisadı",
            5,
            3,
            0,
            0
        ]
    ]
},
        "pool_metadata": {
    "SD": {},
    "BSD": {},
    "ÜSD000": {}
},
    },
}
