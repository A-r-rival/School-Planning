...
class ORToolsScheduler:
    ...
    def add_hard_constraints(self, include_teacher_unavailability=True):
        ...
        print("DEBUG: Calling add_teacher_room_preferences (SKIPPED)...", flush=True)
        # self.add_teacher_room_preferences()
        print("DEBUG: add_teacher_room_preferences DONE (SKIPPED).", flush=True)
        ...
        print("DEBUG: Skipped add_student_group_conflicts", flush=True)
    ...
    def add_student_group_conflicts(self):
        ...
        try:
            ...
            for (g_id, s_id), items in group_slot_vars.items():
                count += 1
                if count % 1000 == 0:
                     print(f"DEBUG: Processing group-slot {count}/{len(group_slot_vars)}", flush=True)
    ...
    def add_teacher_room_preferences(self):
        ...
        for t in self.teachers:
            t_id = t[0]
            # Use new model method (if available, else raw query or cached)
            # self.teachers is tuple list (id, name, room_req) from get_all_teachers_with_ids
            # Check if tuple has 3rd element
            print(f"DEBUG: Processing teacher {t_id}", flush=True)
            print(f"DEBUG: Type of t: {type(t)}, Value: {t}", flush=True)
            if len(t) < 3: 
                print(f"DEBUG: Teacher {t_id} tuple too short: {len(t)}", flush=True)
                continue
            
            request = t[2]
            if not request: 
                print(f"DEBUG: Teacher {t_id} has no request", flush=True)
                continue
            ...
            try:
                for c_idx, course in enumerate(self.courses):
                    if 'teacher_ids' not in course:
                         print(f"CRITICAL WARNING: Course {c_idx} missing 'teacher_ids'", flush=True)
                         continue
                    if t_id in course['teacher_ids']:
                        teacher_course_indices.append(c_idx)
            except Exception as e:
                print(f"CRITICAL ERROR in course lookup for teacher {t_id}: {e}", flush=True)
                continue
            
            if not teacher_course_indices: 
                print(f"DEBUG: Teacher {t_id} has no courses", flush=True)
                continue
            
            try:
                print(f"DEBUG: Logic 1 (Floor) start for {t_id}", flush=True)
                ...
                if target_floor is not None:
                    # Constrain all courses of this teacher to rooms on target_floor
                    for c_idx in teacher_course_indices:
                        for r in self.rooms:
                            r_id = r[0]
                            # r is (id, name, type, capacity, floor) length 5
                            # Safely get floor at index 4 if available
                ...
                print(f"DEBUG: Logic 2 (Lab) start for {t_id}", flush=True)
                # Logic 2: Lab Constraint
                # If teacher wants 'Lab', force Lab rooms (if not already handled by course type)
                if 'lab' in req_lower:
                    for c_idx in teacher_course_indices:
                        for r in self.rooms:
                            r_id = r[0]
                            r_name = r[1].lower()
                            is_lab = 'lab' in r_name
                            if not is_lab:
                                 if (c_idx, r_id) in self.room_vars:
                                    self.cp_model.Add(self.room_vars[(c_idx, r_id)] == 0)

                print(f"DEBUG: Logic 3 (Specific Room) start for {t_id}", flush=True)
                # Logic 3: Specific Room Name (e.g. "D205")
                # Heuristic: if request matches a room name exactly or closely
                # Simple check: if a room name is IN the request string
                for r in self.rooms:
                    r_id = r[0]
                    r_name = r[1]
                    if r_name.lower() in req_lower:
                        # Preference FOUND
                        pass 
                
                valid_room_ids = []
                for r in self.rooms:
                    if r[1].lower() in req_lower: # e.g. "d101" in "d101 istiyorum"
                         valid_room_ids.append(r[0])
                
                if valid_room_ids:
                    # Restrict to these rooms
                    for c_idx in teacher_course_indices:
                        for r in self.rooms:
                             r_id = r[0]
                ...
                print(f"DEBUG: Teacher {t_id} DONE", flush=True)

            except Exception as e:
                import traceback
                print(f"CRITICAL ERROR processing teacher {t_id}: {e}", flush=True)
                traceback.print_exc()
                # Don't re-raise, checking if partial schedule works
                # raise e 
    ...
    def _run_solver(self, mode_name: str, timeout: float = 120.0) -> bool:
        ...
        self.solver.parameters.log_to_stdout = False # Try disabling to fix crash
